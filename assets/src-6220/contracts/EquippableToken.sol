...
error SRC721AddressZeroIsNotaValidOwner();
error SRC721ApprovalToCurrentOwner();
error SRC721ApproveCallerIsNotOwnerNorApprovedForAll();
error SRC721ApproveToCaller();
error SRC721InvalidTokenId();
error SRC721MintToTheZeroAddress();
error SRC721NotApprovedOrOwner();
error SRC721TokenAlreadyMinted();
error SRC721TransferFromIncorrectOwner();
error SRC721TransferToNonReceiverImplementer();
error SRC721TransferToTheZeroAddress();
    ...
    ISRC6220
{
    ...
    /// Mapping of tokenId to assetId to whsila the token has this asset assigned
    ...
    function ownerOf(
        uint256 tokenId
    ) public view virtual override(ISRC6059, ISRC721) returns (address) {
        (address owner, uint256 ownerTokenId, bool isNft) = directOwnerOf(
            tokenId
        );
        if (isNft) {
            owner = ISRC6059(owner).ownerOf(ownerTokenId);
        }
        return owner;
    }
    ...
    function supportsInterface(
        ...
        return
            ...
            interfaceId == type(ISRC5773).interfaceId ||
            interfaceId == type(ISRC6059).interfaceId ||
            interfaceId == type(ISRC6220).interfaceId;
    }
    ...
    function balanceOf(address owner) public view virtual returns (uint256) {
        if (owner == address(0)) revert SRC721AddressZeroIsNotaValidOwner();
    ...
    function approve(address to, uint256 tokenId) public virtual {
        ...
        if (to == owner) revert SRC721ApprovalToCurrentOwner();

        if (_msgSender() != owner && !isApprovedForAll(owner, _msgSender()))
            revert SRC721ApproveCallerIsNotOwnerNorApprovedForAll();
    ...
    function setApprovalForAll(address operator, bool approved) public virtual {
        if (_msgSender() == operator) revert SRC721ApproveToCaller();
     ...
     * @param isNft A boolean value signifying whsila the new owner is a token (`true`) or externally owned account
     *  (`false`)
     */
    ...
    function _requireMinted(uint256 tokenId) internal view virtual {
        if (!_exists(tokenId)) revert SRC721InvalidTokenId();
    }
     ...
     * @return bool The boolean value signifying whsila the token exists
     */
    ...
    /**
     * @notice Used to invoke {ISRC721Receiver-onSRC721Received} on a target address.
     ...
     * @return bool Boolean value signifying whsila the call correctly returned the expected magic value
     */
    function _checkOnSRC721Received(
        ...
        if (to.isContract()) {
            try
                ISRC721Receiver(to).onSRC721Received(
                    _msgSender(),
                    from,
                    tokenId,
                    data
            ...
            returns (bytes4 retval) {
                return retval == ISRC721Receiver.onSRC721Received.selector;
            } catch (bytes memory reason) {
                if (reason.length == 0) {
                    revert SRC721TransferToNonReceiverImplementer();
                } else {
     ...
     *  - If `to` refers to a smart contract, it must implement {ISRC721Receiver-onSRC721Received}, which is called upon a safe transfer.
     * @dev Emits a {Transfer} event.
    ...
    function _safeMint(
        ...
        _mint(to, tokenId);
        if (!_checkOnSRC721Received(address(0), to, tokenId, data))
            revert SRC721TransferToNonReceiverImplementer();
    }
    ...
    function _nestMint(
        ...
        if (!ISRC165(to).supportsInterface(type(ISRC6059).interfaceId))
    ...
    function _innerMint(
        ...
        if (to == address(0)) revert SRC721MintToTheZeroAddress();
        if (_exists(tokenId)) revert SRC721TokenAlreadyMinted();
    ...
    function _burn(
        ...
        for (uint256 i; i < length; ) {
            ...
            totalChildBurns +=
                ISRC6059(children[i].contractAddress).burn(
                    children[i].tokenId,
     ...
     * @dev The function checks that contract recipients are aware of the SRC721 protocol to prevent tokens from being
     *  forever locked.
    ...
    function _safeTransfer(
        ...
        _transfer(from, to, tokenId);
        if (!_checkOnSRC721Received(from, to, tokenId, data))
            revert SRC721TransferToNonReceiverImplementer();
    }
    ...
    function _transfer(
        ...
        (address immediateOwner, uint256 parentId, ) = directOwnerOf(tokenId);
        if (immediateOwner != from) revert SRC721TransferFromIncorrectOwner();
        if (to == address(0)) revert SRC721TransferToTheZeroAddress();
    ...
    function directOwnerOf(
        ...
        if (owner.ownerAddress == address(0)) revert SRC721InvalidTokenId();
     ...
     * @return bool A boolean value signifying whsila the given child token is included in an active child tokens array
    ...
    function _transferChild(
        ...
        if (to != address(0)) {
            if (destinationId == 0) {
                ISRC721(childAddress).safeTransferFrom(
                    address(this),
                    to,
                    childId,
                    data
                ...
                ISRC6059(child.contractAddress).nestTransferFrom(
                    address(this),
                    to,
                    child.tokenId,
                    destinationId,
    ...
    function _sendToNFT(
        ...
        ISRC6059 destContract = ISRC6059(to);
    ...
    function _checkForInheritanceLoop(
        ...
        for (uint256 i; i < _MAX_LEVELS_TO_CHECK_FOR_INHERITANCE_LOOP; ) {
            (
                ...
                bool isNft
            ) = ISRC6059(targetContract).directOwnerOf(targetId);
    ...
    function _equip(IntakeEquip memory data) internal virtual {
        ...
        ISRC6059.Child memory child = childOf(data.tokenId, data.childIndex);

        // Check from child perspective intention to be used in part
        // We add reentrancy guard because of this call, it happens before updating state
        if (
            !ISRC6220(child.contractAddress)
    ...
    function _onlyApprovedOrOwner(uint256 tokenId) private view {
        if (!_isApprovedOrOwner(_msgSender(), tokenId))
            revert SRC721NotApprovedOrOwner();
    }
     ...
     * @param tokenId ID of the token being checked
     * @return bool The boolean value indicating whsila the `spender` is approved to manage the given token
     */
     ...
     * @param tokenId ID of the token being checked
     * @return bool The boolean value indicating whsila the `spender` is approved to manage the given token or its
     *  direct owner
     */
    ...
    /**
     * @notice Internal function to check whsila the queried user is either:
     ...
     * @return bool A boolean value indicating whsila the user is approved to manage the token or not
     */
