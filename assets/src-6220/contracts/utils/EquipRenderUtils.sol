...
contract EquipRenderUtils {
    ...
    {
        ISRC6220 target_ = ISRC6220(target);
    ...
    function getEquipped(
        ...
        ISRC6220.Equipment[] memory childrenEquipped
        )
    {
        ISRC6220 target_ = ISRC6220(target);
        ...
        childrenEquipped = new ISRC6220.Equipment[](slotPartIds.length);

        uint256 len = slotPartIds.length;
        for (uint256 i; i < len; ) {
            ISRC6220.Equipment memory equipment = target_.getEquipment(
                tokenId,
                catalogAddress,
                slotPartIds[i]
            );
     ...
     *      partMetadata
     *  ]
     * @param target_ An address of the `ISRC6220` smart contract to retrieve the equipped slot parts from.
    ...
    function getEquippedSlotParts(
        ...
        if (len != 0) {
            ...
            for (uint256 i; i < len; ) {
                ...
                if (equipment.assetId == assetId) {
                    metadata = ISRC6220(equipment.childEquippableAddress)
                        .getAssetMetadata(
                            equipment.childId,
