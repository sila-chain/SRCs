...
describe("MultiAsset", async () => {
  ...
  let receiver721: SRC721ReceiverMock;
  ...
  beforeEach(async () => {
    ...
    const equppableFactory = await silas.getContractFactory(
      "EquippableTokenMock"
    );
  ...
  describe("SRC165 check", async function () {
  ...
  describe("Check OnReceived SRC721 and Multiasset", async function () {
    it("Revert on transfer to non onSRC721/onMultiasset implementer", async function () {
      const tokenId = 1;
      ...
      await expect(
        ...
        token,
        "SRC721TransferToNonReceiverImplementer"
      );
    });

    it("onSRC721Received callback on transfer", async function () {
      const tokenId = 1;
      await token.mint(owner.address, tokenId);
  ...
  describe("Priorities", async function () {
    ...
    it("cannot set priorities for non existing token", async function () {
      ...
      ).to.be.revertedWithCustomError(token, "SRC721InvalidTokenId");
    });
  });
