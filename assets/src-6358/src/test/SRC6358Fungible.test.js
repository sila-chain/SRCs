...
const TOKEN_SYMBOL = 'SRC6358Fungible';
const COOL_DOWN = 2;
...
const Fungible = artifacts.require('./SRC6358FungibleExample.sol');
...
let encodeMint = (from, toPk, amount, nonce) => {
    let txData = {
        ...
        from: from.pk,
        payload: web3js.sil.abi.encodeParameters(['uint8', 'bytes', 'uint256'], [MINT, toPk, amount])
    }
...
let encodeBurn = (from, toPk, amount, nonce) => {
    let txData = {
        ...
        initiateSC: Fungible.address,
        from: from.pk,
        payload: web3js.sil.abi.encodeParameters(['uint8', 'bytes', 'uint256'], [BURN, toPk, amount])
    }
...
let encodeTransfer = (from, toPk, amount, nonce) => {
    let txData = {
        ...
        initiateSC: Fungible.address,
        from: from.pk,
        payload: web3js.sil.abi.encodeParameters(['uint8', 'bytes', 'uint256'], [TRANSFER, toPk, amount])
    }
...
contract('SRC6358Fungible', function() {
