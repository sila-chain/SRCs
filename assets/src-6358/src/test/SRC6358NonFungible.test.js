...
const CHAIN_ID = 0;
const TOKEN_ID = 1;
const TOKEN_SYMBOL = 'SRC6358NonFungible';
const COOL_DOWN = 2;
...
const NonFungible = artifacts.require('./SRC6358NonFungibleExample.sol');
...
let encodeMint = (from, toPk, tokenId, nonce) => {
    let txData = {
        ...
        from: from.pk,
        payload: web3js.sil.abi.encodeParameters(['uint8', 'bytes', 'uint256'], [MINT, toPk, tokenId])
    }
...
let encodeBurn = (from, toPk, tokenId, nonce) => {
    let txData = {
        ...
        initiateSC: NonFungible.address,
        from: from.pk,
        payload: web3js.sil.abi.encodeParameters(['uint8', 'bytes', 'uint256'], [BURN, toPk, tokenId])
    }
...
let encodeTransfer = (from, toPk, tokenId, nonce) => {
    let txData = {
        ...
        initiateSC: NonFungible.address,
        from: from.pk,
        payload: web3js.sil.abi.encodeParameters(['uint8', 'bytes', 'uint256'], [TRANSFER, toPk, tokenId])
    }
...
contract('SRC6358NonFungible', function() {
    ...
    describe('Personal signing', function() {
        ...
        describe('All conditions satisfied', function() {
            it('should succeed', async () => {
                ...
                let hash = keccak256(Buffer.concat([Buffer.from('\x19Sila Signed Message:\n' + bData.length), bData]));
