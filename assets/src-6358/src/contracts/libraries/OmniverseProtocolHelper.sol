...
import "../interfaces/ISRC6358.sol";
import "../interfaces/ISRC6358Application.sol";
...
library OmniverseProtocolHelper {
     ...
     */
    function getRawData(SRC6358TransactionData memory _data) internal view returns (bytes memory) {
        bytes memory payloadRawData = ISRC6358Application(address(this)).getPayloadRawData(_data.payload);
     ...
     */
    function getTransactionHash(SRC6358TransactionData memory _data) internal view returns (bytes32) {
     ...
     */
    function verifyTransaction(RecordedCertificate storage rc, SRC6358TransactionData memory _data) public returns (VerifyResult) {
        bytes memory rawData = getRawData(_data);
