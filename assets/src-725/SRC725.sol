...
interface ISRC165 {
     ...
     * `interfaceId`. See the corresponding [SRC165] standard.
...
interface ISRC725X is ISRC165 {
     ...
     * Requirements:
     *
     * - SHOULD only be callable by the owner of the contract set via SRC173.
...
// SRC165 INTERFACE IDs
bytes4 constant _INTERFACEID_SRC725X = 0x7545acac;

// SRC725X OPERATION TYPES
 ...
 * @param balance the balance of the SRC725X contract.
 * @param value the amount of native tokens sent via `SRC725X.execute(...)`.
 */
error SRC725X_InsufficientBalance(uint256 balance, uint256 value);
 ...
 */
error SRC725X_UnknownOperationType(uint256 operationTypeProvided);

/**
 * @dev the `value` parameter (= sending native tokens) is not allowed when making a staticcall
 * via `SRC725X.execute(...)` because sending native tokens is a state changing operation.
 */
error SRC725X_MsgValueDisallowedInStaticCall();

/**
 * @dev the `value` parameter (= sending native tokens) is not allowed when making a delegatecall
 * via `SRC725X.execute(...)` because msg.value is persisting.
 */
error SRC725X_MsgValueDisallowedInDelegateCall();
 ...
 */
error SRC725X_ExecuteParametersLengthMismatch();

contract SRC725X is SRC173, SRC165, ISRC725X {
    /**
     * @inheritdoc SRC165
     */
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(ISRC165, SRC165)
        returns (bool)
    {
        return
            interfaceId == _INTERFACEID_SRC725X ||
    ...
    function execute(
        ...
        if (address(this).balance < value) {
            revert SRC725X_InsufficientBalance(address(this).balance, value);
        }
    ...
    function _execute(
        ...
        if (operationType == uint256(OPERATION_1_CREATE)) {
            if (target != address(0))
                revert SRC725X_CreateOperationsRequireEmptyRecipientAddress();
        ...
        if (operationType == uint256(OPERATION_3_STATICCALL)) {
            if (value != 0) revert SRC725X_MsgValueDisallowedInStaticCall();
        ...
        if (operationType == uint256(OPERATION_4_DELEGATECALL)) {
            if (value != 0) revert SRC725X_MsgValueDisallowedInDelegateCall();
            return _executeDelegateCall(target, data);
        }

        revert SRC725X_UnknownOperationType(operationType);
    }
        ...
        returns (bytes memory newContract)
    {
        if (creationCode.length == 0) {
            revert SRC725X_NoContractBytecodeProvided();
        }
        ...
        if (contractAddress == address(0)) {
            revert SRC725X_ContractDeploymentFailed();
        }
...
/**
 * @title The interface for SRC725Y General data key/value store
 * @dev SRC725Y provides the ability to set arbitrary data key/value pairs that can be changed over time
...
// SRC165 INTERFACE IDs
bytes4 constant _INTERFACEID_SRC725Y = 0x629aa694;
 ...
 */
error SRC725Y_DataKeysValuesLengthMismatch(
    uint256 dataKeysLength,
    uint256 dataValuesLength
);

contract SRC725Y is SRC173, SRC165, ISRC725Y {
    // overrides
    ...
    {
        return
            interfaceId == _INTERFACEID_SRC725Y ||
            super.supportsInterface(interfaceId);
    }
        ...
        onlyOwner
    {
        if (dataKeys.length != dataValues.length) {
            revert SRC725Y_DataKeysValuesLengthMismatch(
                dataKeys.length,
                dataValues.length
            );
        }
...
contract SRC725 is SRC725X, SRC725Y {
    /**
     * @inheritdoc SRC165
     */
    function supportsInterface(bytes4 interfaceId)
