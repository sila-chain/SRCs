...
contract MultiOwnerNFT is Context, SRC165, ISRC721, Ownable {
    ...
    function safeTransferFrom(
        ...
        // 2. Call the internal function to check if `to` can handle SRC-721 tokens
        require(
            _checkOnSRC721Received(from, to, tokenId, ""),
            "MO-NFT: transfer to non SRC721Receiver implementer"
        );
    }
    ...
    function safeTransferFrom(
        ...
        // 2. Call the internal function to check if `to` can handle SRC-721 tokens
        require(
            _checkOnSRC721Received(from, to, tokenId, _data),
            "MO-NFT: transfer to non SRC721Receiver implementer"
        );
    }

    /**
     * @dev Private helper to call `onSRC721Received` on a target contract.
    ...
    function _checkOnSRC721Received(
        ...
        returns (bytes4 retval) {
            ...
            if (reason.length == 0) {
                revert("MO-NFT: transfer to non SRC721Receiver implementer");
            } else {
