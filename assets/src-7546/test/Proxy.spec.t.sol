...
/**
 *  @title A test to verify that the Proxy Contract meets the specifications of the SRC-7546 standard.
 *
 *  Proxy Specs
 ...
 *        (1-1-1) obtained as `bytes32(uint256(keccak256('src7546.proxy.dictionary')) - 1)`
...
contract ProxySpecTest is Test, Proxy {
    ...
    //  (1.1.1) Positive
    function test_SlotIsObtainedAsSRC1967Style() public {
        bytes32 slot = DICTIONARY_SLOT;
        assertEq(slot, 0x267691be3525af8a813d30db0c9e2bad08f63baecf6dceb85e2cf3676cff56f4);
        assertEq(slot, bytes32(uint256(keccak256('src7546.proxy.dictionary')) - 1));
    }
