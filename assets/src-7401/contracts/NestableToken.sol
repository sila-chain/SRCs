 ...
 *  gas limits allow it.
 */
contract NestableToken is Context, ISRC165, ISRC721, ISRC7401 {
    ...
    function supportsInterface(
        ...
        return
            ...
            interfaceId == type(ISRC7401).interfaceId;
    }
    ...
    function _nestTransfer(
        ...
        if (!ISRC165(to).supportsInterface(type(ISRC7401).interfaceId))
    ...
    function _sendToNFT(
        ...
        ISRC7401 destContract = ISRC7401(to);
    ...
    function _checkForInheritanceLoop(
        ...
        for (uint256 i; i < _MAX_LEVELS_TO_CHECK_FOR_INHERITANCE_LOOP; ) {
            (
                ...
                bool isNft
            ) = ISRC7401(targetContract).directOwnerOf(targetId);
    ...
    function ownerOf(
        uint256 tokenId
    ) public view virtual override(ISRC7401, ISRC721) returns (address) {
        (address owner, uint256 ownerTokenId, bool isNft) = directOwnerOf(
            tokenId
        );
        if (isNft) {
            owner = ISRC7401(owner).ownerOf(ownerTokenId);
        }
        return owner;
    }
    ...
    function _burn(
        ...
        for (uint256 i; i < length; ) {
            ...
            totalChildBurns +=
                ISRC7401(children[i].contractAddress).burn(
                    children[i].tokenId,
    ...
    function _transferChild(
        ...
        if (to != address(0)) {
            if (destinationId == uint256(0)) {
                ...
                ISRC7401(child.contractAddress).nestTransferFrom(
                    address(this),
                    to,
                    child.tokenId,
                    destinationId,
