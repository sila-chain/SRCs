...
describe('NestableToken', function () {
  ...
  describe('Minting', async function () {
    ...
    it('cannot nest mint already minted token', async function () {
      ...
      await expect(
        child.nestMint(parent.address, childId1, parentId),
      ).to.be.revertedWithCustomError(child, 'SRC721TokenAlreadyMinted');
    });
  ...
  describe('Interface support', async function () {
    ...
    it('can support ISRC721', async function () {
      expect(await parent.supportsInterface('0x80ac58cd')).to.equal(true);
    });

    it('can support ISRC7401', async function () {
  ...
  describe('Accept child', async function () {
    ...
    it('cannot accept not owned child', async function () {
      ...
      ).to.be.revertedWithCustomError(parent, 'SRC721NotApprovedOrOwner');
    });
  ...
  describe('Transferring Active Children', async function () {
    ...
    it('cannot transfer child if not child root owner', async function () {
      ...
      ).to.be.revertedWithCustomError(child, 'SRC721NotApprovedOrOwner');
    });
    ...
    it('cannot transfer child from not existing parent', async function () {
      ...
      ).to.be.revertedWithCustomError(child, 'SRC721InvalidTokenId');
    });
  ...
  describe('Transfer', async function () {
    ...
    it('cannot transfer to address zero', async function () {
      ...
      ).to.be.revertedWithCustomError(child, 'SRC721TransferToTheZeroAddress');
    });
  ...
  describe('Nest Transfer', async function () {
    ...
    it('cannot nest transfer to contract if it does implement ISRC7401', async function () {
      const SRC721 = await ethers.getContractFactory('SRC721Mock');
    ...
    it('can nest transfer to ISRC7401 contract', async function () {
      await child.connect(firstOwner).nestTransfer(parent.address, childId1, parentId);
    ...
    it('cannot nest transfer to non existing parent token', async function () {
      ...
      ).to.be.revertedWithCustomError(parent, 'SRC721InvalidTokenId');
    });
  });
