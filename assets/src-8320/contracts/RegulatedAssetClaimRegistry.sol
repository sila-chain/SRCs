...
/// @notice Reference SRC-8320 registry. Holds signed, versioned, role-gated claims about assets, keyed by
///         assetId. Claims move through a maker-checker lifecycle (propose -> validate -> activate, with
///         suspend/revoke) where every state change is authorized by an SIP-712 signature from the party
///         holding the matching role. The registry admin (DEFAULT_ADMIN_ROLE) registers assets and grants roles.
contract RegulatedAssetClaimRegistry is IRegulatedAssetClaimRegistry, AccessControl, SIP712 {
    ...
    /// @dev Per-signer nonce for SIP-712 replay protection.
    ...
    constructor(address admin) SIP712("RegulatedAssetClaimRegistry", "1") {
    ...
    /// @notice Returns the SIP-712 domain separator. Exposed for integrators and off-chain signers.
    function DOMAIN_SEPARATOR() external view returns (bytes32) {
        return _domainSeparatorV4();
    }

    function supportsInterface(bytes4 interfaceId) public view override(AccessControl, ISRC165) returns (bool) {
    ...
    /// @dev deadline -> nonce -> SIP-712 digest -> signature; bumps the signer nonce on success.
    ...
    /// @dev SIP-712 struct hash for Propose. Split into two encodes so the 15 static words never sit on the
