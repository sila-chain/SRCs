...
import { getTypesForSIP712Domain } from "viem";
import {
  delegatedErc721ABI
} from "./abi";
    ...
    async ({ creator, contractAddress }) => {
      ...
      const signature = await walletClient.signTypedData({
        ...
        domain: {
          ...
          // these two need to match the domain name and version in the src712 contract
          name: "SRC7015",
          version: "1"
        }
      });
      ...
      // get the CreatorAttribution event from the src1155 contract:
      ...
      // hash the sip712 domain based on the parameters emitted from the event:
      const hashedDomain = hashDomain({
        domain,
        types: {
          SIP712Domain: getTypesForSIP712Domain({ domain })
        }
      });

      // re-build the sip-712 typed data hash, consisting of the hashed domain and the structHash emitted from the event:
