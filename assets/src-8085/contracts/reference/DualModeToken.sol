...
/**
 * @title DualModeToken
 * @notice Dual-mode token (SRC-8085) combining SRC-20 and SRC-8086 privacy features
 * @dev This contract extends PrivacyToken (SRC-8086) with public mode (SRC-20) and
 *      mode conversion capabilities, creating a unified token that operates in two modes.
 *
* Architecture:
 *   - Public Mode: Standard SRC-20 (OpenZeppelin)
 *   - Privacy Mode: SRC-8086 IZRC20 compatible
 *   - Mode Conversion: toPrivate() / toPublic()
 *Layered Design:
 *   ┌─────────────────────────────────────────┐
 *   │    DualModeToken (SRC-8085 Layer)       │
 *   │  - Public mode (SRC-20)                 │
 *   │  - Mode conversion (toPrivate/toPublic) │
 *   │                                         │
 *   ├─────────────────────────────────────────┤
 *   │    PrivacyToken (SRC-8086 Layer)        │
 *   │  - Privacy mode (IZRC20)                │
 *   │  - ZK-SNARK proofs                      │
 ...
 *
 * ═══════════════════════════════════════════════════════════════════════════════
 * IMPORTANT: SRC-8085 Standard vs. Reference Implementation
 * ═══════════════════════════════════════════════════════════════════════════════
 *
 ...
 */
contract DualModeToken is PrivacyToken, SRC20, IDualModeToken {
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // State Variables (SRC-8085 / Public Mode Specific)
    // ═══════════════════════════════════════════════════════════════════════

    /// @dev Token metadata (stored separately for clone pattern compatibility)
    /// @custom:standard-required SRC-20 and SRC-8086 both require name/symbol
    string private _tokenName;
    string private _tokenSymbol;

    /// @dev Public mode configuration
    /// @custom:business-logic These are reference implementation choices, NOT required by SRC-8085
     ...
     * @dev Initializes both privacy layer (PrivacyToken) and public layer (SRC20)
     */
    function initialize(
        string memory name_,
        ...
        // Initialize public layer (SRC20 metadata)
        _tokenName = name_;
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // SRC-20 Metadata Overrides
    // ═══════════════════════════════════════════════════════════════════════

    function name() public view override(SRC20, IZRC20) returns (string memory) {
        return _tokenName;
    }

    function symbol() public view override(SRC20, IZRC20) returns (string memory) {
        return _tokenSymbol;
    }

    function decimals() public pure override(SRC20, IZRC20) returns (uint8) {
        return 18;
    }

    // ═══════════════════════════════════════════════════════════════════════
    // Supply Tracking (SRC-8085 Requirement)
    // ═══════════════════════════════════════════════════════════════════════

    /**
     * @notice Total supply across both modes (SRC-20 + Privacy)
     * @dev SRC-8085 requirement: totalSupply = publicSupply + privacySupply
     *      Overrides both SRC20.totalSupply() and PrivacyToken.totalSupply()
     */
    function totalSupply() public view override(SRC20, PrivacyToken, IDualModeToken) returns (uint256) {
        return SRC20.totalSupply() + PrivacyToken.totalSupply();
    }
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // Public Minting (REFERENCE IMPLEMENTATION - NOT PART OF SRC-8085)
    // ═══════════════════════════════════════════════════════════════════════

    /**
     * @notice Mint public tokens (standard SRC-20)
     * @dev Entry point for all tokens - ensures supply transparency
     *
     * ⚠️ IMPORTANT: This is NOT required by SRC-8085 standard!
     *
     * This is ONE possible token distribution mechanism.
     * Other SRC-8085 implementations MAY use:
     ...
     *
     * SRC-8085 standard does NOT mandate:
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // Mode Conversion: Public → Privacy (SRC-8085 Core)
    // ═══════════════════════════════════════════════════════════════════════

    /**
     * @notice Convert public balance to privacy mode
     * @dev Burns SRC-20 tokens and creates privacy commitment via PrivacyToken._privacyMint
    ...
    function toPrivate(
        ...
        // 1. Burn public tokens (SRC-20 layer)
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // Mode Conversion: Privacy → Public (SRC-8085 Core)
    // ═══════════════════════════════════════════════════════════════════════

    /**
     * @notice Convert privacy balance to public mode
     * @dev Spends privacy notes and mints SRC-20 tokens
    ...
    function toPublic(
        ...
        // 2. Mint public tokens (SRC-20 layer)
