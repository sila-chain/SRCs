...
const ISRC7303_ID = "0x4ee69337";
const SRC165_ID = "0x01ffc9a7";
const INVALID_ID = "0xffffffff";
const MINTER_ROLE = ethers.id("MINTER_ROLE");
const BURNER_ROLE = ethers.id("BURNER_ROLE");
const REVERT_NO_TOKEN = "SRC7303: not has a required token";

async function deployFixture() {
  const [issuer, alice, bob, carol] = await ethers.getSigners();
  const ct721 = await ethers.deployContract("SRC721ControlToken");
  const ct1155 = await ethers.deployContract("SRC1155ControlToken");
...
describe("SRC-7303 conformance fixture", function () {
  describe("interface identifier", function () {
    it("recomputes to 0x4ee69337 from the three ISRC7303 selectors", function () {
      const signatures = [
        "hasRole(bytes32,address)",
        "getSRC721ControlTokens(bytes32)",
        "getSRC1155ControlTokens(bytes32)",
      ];
      let acc = 0n;
      for (const sig of signatures) {
        acc ^= BigInt(ethers.dataSlice(ethers.id(sig), 0, 4));
      }
      expect("0x" + acc.toString(16).padStart(8, "0")).to.equal(ISRC7303_ID);
    });

    it("is declared via SRC-165 by the compliant fixture", async function () {
      const { target } = await loadFixture(deployFixture);
      expect(await target.supportsInterface(SRC165_ID)).to.equal(true);
      expect(await target.supportsInterface(ISRC7303_ID)).to.equal(true);
  ...
  describe("introspection getters", function () {
    it("enumerates the canonical MINTER_ROLE structure", async function () {
      ...
      expect(await target.getSRC721ControlTokens(MINTER_ROLE)).to.deep.equal([
        ct721.target,
      ]);
      const [contractIds, typeIds] =
        await target.getSRC1155ControlTokens(MINTER_ROLE);
      expect(contractIds).to.deep.equal([ct1155.target]);
      expect(typeIds).to.deep.equal([1n]);
    });

    it("enumerates the canonical BURNER_ROLE structure, including the empty SRC-721 list", async function () {
      const { target, ct1155 } = await loadFixture(deployFixture);
      expect(await target.getSRC721ControlTokens(BURNER_ROLE)).to.deep.equal([]);
      const [contractIds, typeIds] =
        await target.getSRC1155ControlTokens(BURNER_ROLE);
      expect(contractIds).to.deep.equal([ct1155.target]);
    ...
    it("answers empty lists for an unconfigured role", async function () {
      ...
      expect(await target.getSRC721ControlTokens(unknown)).to.deep.equal([]);
      const [contractIds, typeIds] =
        await target.getSRC1155ControlTokens(unknown);
  ...
  describe("association events", function () {
    it("emitted one SRC721ControlTokenAdded and two SRC1155ControlTokenAdded at deployment", async function () {
      const { target, ct721, ct1155 } = await loadFixture(deployFixture);

      const src721Events = await target.queryFilter(
        target.filters.SRC721ControlTokenAdded()
      );
      expect(src721Events).to.have.lengthOf(1);
      expect(src721Events[0].args.role).to.equal(MINTER_ROLE);
      expect(src721Events[0].args.contractId).to.equal(ct721.target);

      const src1155Events = await target.queryFilter(
        target.filters.SRC1155ControlTokenAdded()
      );
      expect(src1155Events).to.have.lengthOf(2);
      expect(src1155Events[0].args.role).to.equal(MINTER_ROLE);
      expect(src1155Events[0].args.contractId).to.equal(ct1155.target);
      expect(src1155Events[0].args.typeId).to.equal(1n);
      expect(src1155Events[1].args.role).to.equal(BURNER_ROLE);
      expect(src1155Events[1].args.contractId).to.equal(ct1155.target);
      expect(src1155Events[1].args.typeId).to.equal(2n);
    });
  });

  describe("role lifecycle (grant = mint, check = balanceOf, revoke = burn)", function () {
    it("tracks the SRC-721 control-token balance, including the issuer kill switch", async function () {
    ...
    it("tracks the SRC-1155 control-token balance per typeId", async function () {
    ...
    it("requires every stacked role for a doubly-guarded function (AND semantics across modifiers)", async function () {
      ...
      // Cross-standard AND: alice already holds MINTER_ROLE via the SRC-721
      // control token; granting BURNER_ROLE via the SRC-1155 control token
    ...
    it("grants MINTER_ROLE through either control token (OR semantics across entries)", async function () {
      ...
      await ct721.mint(alice.address); // SRC-721 path only
      await ct1155.mint(carol.address, 1, 1); // SRC-1155 path only
  ...
  describe("negative case: functionally identical legacy contract", function () {
    it("gates identically but does not declare ISRC7303", async function () {
      ...
      // ...but discovery must classify it as NOT implementing this SRC:
      expect(await legacy.supportsInterface(SRC165_ID)).to.equal(true);
      expect(await legacy.supportsInterface(ISRC7303_ID)).to.equal(false);
