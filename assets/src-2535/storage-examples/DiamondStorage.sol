...
library LibSRC721 {
    bytes32 constant SRC721_POSITION = keccak256("src721.storage");
    ...
    // 
    // bytes32 constant SRC721_POSITION = 
    //    keccak256(abi.encodePacked(
    //        SRC721.interfaceId, 
    //        SRC721.name
    //    ));

    struct SRC721Storage {
        // tokenId => owner
    ...
    // Return SRC721 storage struct for reading and writing
    function getStorage() internal pure returns (SRC721Storage storage storageStruct) {
        bytes32 position = SRC721_POSITION;
        assembly {
    ...
    function transferFrom(address _from, address _to, uint256 _tokenId) internal {
        SRC721Storage storage src721Storage = LibSRC721.getStorage();
        address tokenOwner = src721Storage.tokenIdToOwner[_tokenId];
        require(tokenOwner == _from);
        src721Storage.tokenIdToOwner[_tokenId] = _to;
        src721Storage.ownerToNFTokenCount[_from]--;
        src721Storage.ownerToNFTokenCount[_to]++;
        emit Transfer(_from, _to, _tokenId);
    }
}

// Note that this is not a full or correct SRC721 implementation.
// This is an example of using diamond storage.

// Note that the SRC721.name and SRC721.symbol storage variables would probably be set
// in an `init` function at deployment time or during an upgrade.


// Shows use of LibSRC721 and diamond storage
contract SRC721Facet {
    
    function name() external view returns (string memory name_) {
        name_ = LibSRC721.getStorage().name;
    }
    
    function symbol() external view returns (string memory symbol_) {
        symbol_ = LibSRC721.getStorage().symbol;
    }

    function transferFrom(address _from, address _to, uint256 _tokenId) external {
        LibSRC721.transferFrom(_from, _to, _tokenId);
    }
