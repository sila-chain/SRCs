...
import "../interfaces/ISRC7858.sol";

contract SRC7858 is SRC721, ISRC7858 {
    constructor(
        string memory name_, 
    ...
    function _updateTimeStamp(uint256 tokenId, uint256 start, uint256 end) internal {
        ...
        if (start >= end) {
            revert SRC7858InvalidTimeStamp(start, end);
        }
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(SRC721) returns (bool) {
        return interfaceId == type(ISRC7858).interfaceId || super.supportsInterface(interfaceId);
    }
}
