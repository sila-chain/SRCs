...
import {ISRC7858} from "../interfaces/ISRC7858.sol";
import {ISRC7858Epoch} from "../interfaces/ISRC7858Epoch.sol";
...
import {ISRC721Metadata} from "@openzeppelin/contracts/token/SRC721/extensions/ISRC721Metadata.sol";
import {ISRC721Errors} from "@openzeppelin/contracts/interfaces/draft-ISRC6093.sol";

abstract contract SRC7858Epoch is
    Context,
    SRC165,
    ISRC721,
    ISRC721Errors,
    ...
    ISRC7858Epoch
{
     ...
     * core SRC-721 logic MUST be matched with the use of {_increaseBalance} to keep balances
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view virtual override(SRC165, ISRC165) returns (bool) {
        return
            interfaceId == type(ISRC721).interfaceId ||
            interfaceId == type(ISRC721Metadata).interfaceId ||
            interfaceId == type(ISRC7858).interfaceId ||
            interfaceId == type(ISRC7858Epoch).interfaceId ||
            super.supportsInterface(interfaceId);
    }

    /**
     * @dev See {ISRC721-transferFrom}.
     */
    function transferFrom(
        ...
        if (to == address(0)) {
            revert SRC721InvalidReceiver(address(0));
        }
        // Setting an "auth" arguments enables the `_isAuthorized` check which verifies that the token exists
        // (from != 0). Therefore, it is not needed to verify that the return value is not 0 here.
        address previousOwner = _update(to, tokenId, _msgSender());
        if (previousOwner != from) {
            revert SRC721IncorrectOwner(from, tokenId, previousOwner);
        }
    }

    /**
     * @dev See {ISRC721-safeTransferFrom}.
     */
    ...
    function safeTransferFrom(
        ...
        SRC721Utils.checkOnSRC721Received(
            _msgSender(),
            from,
            to,
            tokenId,
    ...
    /**
     * @dev See {ISRC721-balanceOf}.
     */
    function balanceOf(address owner) public view virtual returns (uint256) {
        if (owner == address(0)) {
            revert SRC721InvalidOwner(address(0));
        }
    ...
    /**
     * @dev See {ISRC721Metadata-symbol}.
     */
    ...
    /**
     * @dev See {ISRC721-setApprovalForAll}.
     */
    ...
    /**
     * @dev See {ISRC721-isApprovedForAll}.
     */
    ...
    function _checkAuthorized(
        ...
        if (!_isAuthorized(owner, spender, tokenId)) {
            if (owner == address(0)) {
                ...
                revert SRC721InsufficientApproval(spender, tokenId);
            }
        }
    }
    ...
    function _mint(address account, uint256 tokenId) internal {
        ...
        if (previousOwner != address(0)) {
            revert SRC721InvalidSender(address(0));
        }
    }
    ...
    function _approve(
        ...
        if (emitEvent || auth != address(0)) {
            ...
            if (
                ...
                revert SRC721InvalidApprover(auth);
            }
    ...
    function _setApprovalForAll(
        ...
        if (operator == address(0)) {
            revert SRC721InvalidOperator(operator);
        }
        _operatorApprovals[owner][operator] = approved;
        emit ApprovalForAll(owner, operator, approved);
    }

    /// @dev See {ISRC7858-unexpiredBalanceOfAtEpoch}.
