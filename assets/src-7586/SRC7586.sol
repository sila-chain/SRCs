...
import "./ISRC7586.sol";
import "./Tokens/SRC20.sol";

contract SRC7586 is ISRC7586, SRC20 {
    constructor(string memory _name, string memory _symbol, IRS memory _irs) SRC20(_name, _symbol) {
        irsSymbol = _symbol;
        irs = _irs;

        _isActive = true;

        _balanceOf[_irs.payer] = _irs.paymentDates.length * 1 sila;
        _balanceOf[_irs.receiver] = _irs.paymentDates.length * 1 sila;
        _totalSupply = _irs.paymentDates.length * 2 * 1 sila;
    }
    ...
    function swap() external returns(bool) {
        ...
        burn(irs.payer, 1 sila);
        burn(irs.receiver, 1 sila);

        uint256 _paymentCount = paymentCount;
        paymentCount = _paymentCount + 1;

        ISRC20(irs.assetContract).transferFrom(_payer, _recipient, interestToTransfer * 1 sila / 10_000);
