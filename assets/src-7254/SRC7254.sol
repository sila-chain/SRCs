...
/**
* @dev Implementation of the {ISRC7254} interface.
*
...
* allowances. See {ISRC7254-approve}.
*/
contract SRC7254 is SRC20 {
    ...
    * construction.
    */
    constructor(string memory name_, string memory symbol_, address tokenReward_) SRC20(name_, symbol_){
    ...
    */
    function updateReward(address[] memory token, uint256[] memory amount) public virtual {   
        require(token.length == amount.length, "SRC7254: token and amount length mismatch");
        address owner = _msgSender();
        if(totalSupply() != 0){
            for( uint256 i = 0; i < token.length; ++i){
                require(isTokenReward[token[i]], "SRC7254: token reward is not approved");
                ISRC20(token[i]).transferFrom(owner, address(this), amount[i]);
    ...
    */
    function getReward(address[] memory token, address to) public virtual {
        ...
        for( uint256 i = 0; i < token.length; ++i){
            ...
            if(reward / MAX > 0){
                ISRC20(token[i]).transfer(to, reward / MAX);
            }  
    ...
    */
    function _withdraw(address token, address owner, uint256 reward) internal virtual {
        require(owner != address(0), "SRC7254: withdraw from the zero address");
    ...
    */
    function _updateRewardPerShare(address token, uint256 amount) internal virtual {
        require(token != address(0), "SRC7254: token the zero address");
        require(totalSupply() != 0, "SRC7254: totalSupply is zero");
    ...
    */
    function _add(address[] memory token) internal virtual {
        require(_tokenReward.length + token.length <= maxTokenReward(), "SRC7254: exceeds maxTokenReward");
        for ( uint256 i = 0; i < token.length; ++i ){
            require(!existsTokenReward(token[i]), "SRC7254: token already exists");
            require(token[i] != address(0), "SRC7254: token reward from the zero address");
