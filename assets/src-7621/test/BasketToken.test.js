...
describe("SRC-7621 Basket Token", function () {
  ...
  describe("Ownership (SRC-173)", function () {
    it("owner() and transferOwnership() conform to SRC-173", async function () {
      await deployAndSeed();
  ...
  describe("SRC-165", function () {
    it("reports support for ISRC7621 (0xc9c80f73)", async function () {
      await deployAndSeed();
      expect(await basket.supportsInterface("0xc9c80f73")).to.be.true;
    });

    it("reports support for ISRC173 (0x7f5828d0)", async function () {
      await deployAndSeed();
      expect(await basket.supportsInterface("0x7f5828d0")).to.be.true;
    });

    it("reports support for ISRC165 (0x01ffc9a7)", async function () {
      await deployAndSeed();
