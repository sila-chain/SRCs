...
import {ISRC7621} from "./interfaces/ISRC7621.sol";

/**
 * @title BasketToken
 * @notice Minimal reference implementation of SRC-7621.
 * @dev The basket contract IS the SRC-20 share token. Ownership follows SRC-173.
 *      Valuation uses sum-of-reserves (no oracle). First deposit mints dead shares
 *      to mitigate inflation attacks.
 */
contract BasketToken is SRC20, ISRC7621, ISRC165, ReentrancyGuard {
    ...
    function supportsInterface(bytes4 interfaceId) public pure override returns (bool) {
        return
            interfaceId == 0xc9c80f73 || // ISRC7621
            interfaceId == 0x7f5828d0 || // ISRC173
            interfaceId == 0x01ffc9a7;   // ISRC165
    }
    ...
    function contribute(
        ...
        for (uint256 i = 0; i < len; i++) {
            if (amounts[i] > 0) {
                ISRC20(_tokens[i]).safeTransferFrom(msg.sender, address(this), amounts[i]);
    ...
    function withdraw(
        ...
        for (uint256 i = 0; i < len; i++) {
            if (amounts[i] > 0) {
                _reserves[_tokens[i]] -= amounts[i];
                ISRC20(_tokens[i]).safeTransfer(receiver, amounts[i]);
            }
        }
