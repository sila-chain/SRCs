...
import {SRC721} from "./SRC721.sol";
import {ISRC721Bindable} from "../interfaces/ISRC721Bindable.sol";

/// @title SRC-721 Bindable Reference Implementation.
contract SRC721Bindable is SRC721, ISRC721Bindable {
    ...
    bytes4 private constant _SRC721_BINDER_INTERFACE_ID = 0x2ac2d2bc;
    bytes4 private constant _SRC721_BINDABLE_INTERFACE_ID = 0xd92c3ff0;
    ...
    function bind(
        ...
        if (
            ...
            ISRC721(bindAddress).ownerOf(bindId) == address(0)
        ) {
            revert BindInvalid();
        }
    ...
    function unbind(
		...
		if (
			to.code.length != 0 &&
				ISRC721Receiver(to).onSRC721Received(msg.sender, bindAddress, tokenId, "")
                !=
                ISRC721Receiver.onSRC721Received.selector
		) {
    ...
    function supportsInterface(bytes4 id) public pure override(SRC721, ISRC165) returns (bool) {
        return super.supportsInterface(id) || id == _SRC721_BINDABLE_INTERFACE_ID;
    }
