...
import {ISRC721Errors} from "../interfaces/ISRC721Errors.sol";

/// @title Reference Minimal SRC-721 Contract
contract SRC721 is ISRC721, ISRC721Errors {
    ...
    bytes4 private constant _SRC721_INTERFACE_ID = 0x80ac58cd;
    ...
    /// @param id The SRC-165 interface identifier.
    ...
    function supportsInterface(bytes4 id) public pure virtual returns (bool) {
        return
            id == _SRC165_INTERFACE_ID ||
            id == _SRC721_INTERFACE_ID;
    }
    ...
    function safeTransferFrom(
        ...
        if (
            to.code.length != 0 &&
                ISRC721Receiver(to).onSRC721Received(msg.sender, from, id, data)
                !=
    ...
    function safeTransferFrom(
        ...
        if (
            to.code.length != 0 &&
                ISRC721Receiver(to).onSRC721Received(msg.sender, from, id, "")
                !=
                ISRC721Receiver.onSRC721Received.selector
        ) {
            revert SafeTransferUnsupported();
