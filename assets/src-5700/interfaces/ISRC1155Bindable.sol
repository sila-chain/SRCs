...
import {ISRC1155BindableErrors} from "./ISRC1155BindableErrors.sol";

/// @title SRC-1155 Bindable Token Standard
/// @dev See https://sips.sila.org/SIPS/sip-5656
///  Note: the SRC-165 identifier for this interface is 0xd0d555c6.
interface ISRC1155Bindable is ISRC1155, ISRC1155BindableErrors {
    ...
    ///  valid contract (code size > 0), and if so, call `onSRC1155Bind` on the 
    ...
    ///  MUST emit both `Bind` and ISRC-1155 `TransferSingle` events to reflect 
	...
	/// @param bindId The identifier of the asset being bound to.
	/// @param bindAddress The contract address handling asset ownership.
    /// @param data Additional data sent with the `onSRC1155Bind` hook.
    function bind(
        address from,
        address to,
        uint256 tokenId,
    ...
    ///  `onSRC1155BatchBind` on the contract, throwing if the wrong identifier 
    ///  is returned (see "Binding Rules") or if the contract is invalid. On 
    ///  bind completion, the function MUST emit both `BindBatch` and ISRC-1155 
	...
	/// @param bindIds The identifiers of the assets being bound to.
	/// @param bindAddress The contract address handling asset ownership.
    /// @param data Additional data sent with the `onSRC1155BatchBind` hook.
    function batchBind(
        address from,
        address to,
    ...
    ///  so, call `onSRC1155Unbind` on the contract, throwing if the wrong 
    ...
    ///  call on it `onSRC1155Received`, throwing if the wrong identifier is 
    ///  returned. On unbind completion, the function MUST emit both `Unbind` 
    ///  and ISRC-1155 `TransferSingle` events to reflect delegated ownership change.
	/// @param from The address which owns the asset the token(s) are bound to.
	...
	/// @param bindId The identifier of the asset being unbound from.
	/// @param bindAddress The contract address handling bound asset ownership.
    /// @param data Additional data sent with the `onSRC1155Unbind` hook.
    function unbind(
        address from,
        address to,
        uint256 tokenId,
    ...
    ///  valid contract (code size >  0), and if so, call onSRC1155BatchUnbind` 
    ...
    ///  a valid contract, and if so, call `onSRC1155BatchReceived`, throwing if 
    ///  the wrong identifier is returned. On unbind completion, the function 
    ///  MUST emit the `BatchUnbind` and ISRC-1155 `TransferBatch` events to 
	...
	/// @param bindIds The identifier of the assets being unbound from.
	/// @param bindAddress The contract address handling bound asset ownership.
    /// @param data Additional data sent with the `onSRC1155BatchUnbind` hook.
