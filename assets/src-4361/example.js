...
const GRAMMAR = `
sign-in-with-sila =
    domain %s" wants you to sign in with your Sila account:" LF
    address LF
    LF
    [ statement LF ]
    LF
...
const parseMessage = (message) => {
  ...
  const result = parser.parse(grammarObj, 'sign-in-with-sila', message);
  if (!result.success) {
...
const createMessage = ({ domain, address, uri, version, chainId, nonce, issuedAt }) => {
  const header = `${domain} wants you to sign in with your Sila account:\n${address}\n\n\n`;
