...
import {ISRC20} from "openzeppelin-contracts/contracts/interfaces/ISRC20.sol";
import {ISRC721} from "openzeppelin-contracts/contracts/interfaces/ISRC721.sol";
import {ISRC1155} from "openzeppelin-contracts/contracts/interfaces/ISRC1155.sol";
...
import {ISRC7498} from "./ISRC7498.sol";
...
contract SRC7498NFTRedeemables is ISRC7498, RedeemablesErrors {
    ...
    function _transferConsiderationItem(uint256 id, ConsiderationItem memory c) internal {
        ...
        if (c.itemType == ItemType.SRC721 || c.itemType == ItemType.SRC721_WITH_CRITERIA) {
            // SRC721_WITH_CRITERIA with identifier 0 is wildcard: any id is valid.
            // Criteria is not yet implemented, for that functionality use the contract offerer.
            if (c.itemType == ItemType.SRC721 && id != c.identifierOrCriteria) {
                revert InvalidConsiderationTokenIdSupplied(c.token, id, c.identifierOrCriteria);
            }
            ISRC721(c.token).safeTransferFrom(msg.sender, c.recipient, id);
        } else if ((c.itemType == ItemType.SRC1155 || c.itemType == ItemType.SRC1155_WITH_CRITERIA)) {
            // SRC1155_WITH_CRITERIA with identifier 0 is wildcard: any id is valid.
            // Criteria is not yet implemented, for that functionality use the contract offerer.
            if (c.itemType == ItemType.SRC1155 && id != c.identifierOrCriteria) {
                revert InvalidConsiderationTokenIdSupplied(c.token, id, c.identifierOrCriteria);
            }
            ISRC1155(c.token).safeTransferFrom(msg.sender, c.recipient, id, c.startAmount, "");
        } else if (c.itemType == ItemType.SRC20) {
            ISRC20(c.token).transferFrom(msg.sender, c.recipient, c.startAmount);
        } else {
            // ItemType.NATIVE
            (bool success,) = c.recipient.call{value: msg.value}("");
            if (!success) revert SilaTransferFailed();
        }
    }
    ...
    function _processRedemption(
        ...
        for (uint256 j; j < consideration.length;) {
            ...
            uint256 balance;
            if (c.itemType == ItemType.SRC721 || c.itemType == ItemType.SRC721_WITH_CRITERIA) {
                balance = ISRC721(c.token).ownerOf(id) == msg.sender ? 1 : 0;
            } else if (c.itemType == ItemType.SRC1155 || c.itemType == ItemType.SRC1155_WITH_CRITERIA) {
                balance = ISRC1155(c.token).balanceOf(msg.sender, id);
            } else if (c.itemType == ItemType.SRC20) {
                balance = ISRC20(c.token).balanceOf(msg.sender);
            } else {
                // ItemType.NATIVE
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual returns (bool) {
        return interfaceId == type(ISRC7498).interfaceId;
    }
}
