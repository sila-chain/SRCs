...
contract CentoRouter { 
    ...
    /// @dev SRC-7201 namespace backing the shared `CentoStorage` layout.
    bytes32 private constant BASE_SLOT    = 0x69f90de95fb99742e875407e8b95a22f11141a7a0ca101bc562658f163a85b00;
    /// @dev Routing index of the facet that supports SRC-173.
    uint8   private constant SRC173_INDEX = 1;
    /// @dev Routing index of the facet that supports SRC-165.
    uint8   private constant SRC165_INDEX = 2;
    ...
    /// - index 1: Ownership *(with SRC-173)*
    /// - index 2: Observability *(with SRC-165)*
    ...
    constructor (address _contractOwner, address[3] memory facetAddresses) payable {
        ...
        cs.supportedInterfaces[type(ISRC165).interfaceId] = true; 
        cs.supportedInterfaces[type(ISRC173).interfaceId] = true; 
    ...
    fallback() external payable {
        assembly {
            ...
            case 0x01ffc9a7 { executeFacet(SRC165_INDEX, cds, 0) }
            case 0x8da5cb5b { executeFacet(SRC173_INDEX, cds, 0) }
            case 0xf2fde38b { executeFacet(SRC173_INDEX, cds, 0) }
    ...
    /// @notice Accepts plain SIL transfers.
