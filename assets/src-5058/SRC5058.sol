...
/**
 * @dev Implementation SRC721 Lockable Token
 */
abstract contract SRC5058 is SRC721, ISRC5058 {
    ...
    /**
     * @dev See {ISRC5058-lockApprove}.
     */
    function lockApprove(address to, uint256 tokenId) public virtual override {
        require(!isLocked(tokenId), "SRC5058: token is locked");
        address owner = SRC721.ownerOf(tokenId);
        require(to != owner, "SRC5058: lock approval to current owner");

        require(
            _msgSender() == owner || isLockApprovedForAll(owner, _msgSender()),
            "SRC5058: lock approve caller is not owner nor approved for all"
        );
    ...
    /**
     * @dev See {ISRC5058-getLockApproved}.
     */
    function getLockApproved(uint256 tokenId) public view virtual override returns (address) {
        require(_exists(tokenId), "SRC5058: lock approved query for nonexistent token");

        return _lockApprovals[tokenId];
    }

    /**
     * @dev See {ISRC5058-lockerOf}.
     */
    function lockerOf(uint256 tokenId) public view virtual override returns (address) {
        require(_exists(tokenId), "SRC5058: locker query for nonexistent token");
        require(isLocked(tokenId), "SRC5058: locker query for non-locked token");

        return _lockApprovals[tokenId];
    }

    /**
     * @dev See {ISRC5058-setLockApprovalForAll}.
     */
    ...
    /**
     * @dev See {ISRC5058-isLockApprovedForAll}.
     */
    ...
    /**
     * @dev See {ISRC5058-lockExpiredTime}.
     */
    ...
    /**
     * @dev See {ISRC5058-lock}.
     */
    function lock(uint256 tokenId, uint256 expired) public virtual override {
        //solhint-disable-next-line max-line-length
        require(_isLockApprovedOrOwner(_msgSender(), tokenId), "SRC5058: lock caller is not owner nor approved");
        require(expired > block.number, "SRC5058: expired time must be greater than current block number");
        require(!isLocked(tokenId), "SRC5058: token is locked");
    ...
    /**
     * @dev See {ISRC5058-unlock}.
     */
    function unlock(uint256 tokenId) public virtual override {
        require(lockerOf(tokenId) == _msgSender(), "SRC5058: unlock caller is not lock operator");

        address from = SRC721.ownerOf(tokenId);
    ...
    function _safeLockMint(
        ...
        require(expired > block.number, "SRC5058: lock mint for invalid lock block number");
    ...
    /**
     * @dev See {SRC721-_burn}. This override additionally clears the lock approvals for the token.
     */
    ...
    function _setLockApprovalForAll(
        ...
        require(owner != operator, "SRC5058: lock approve to caller");
    ...
    function _isLockApprovedOrOwner(address spender, uint256 tokenId) internal view virtual returns (bool) {
        require(_exists(tokenId), "SRC5058: lock operator query for nonexistent token");
        address owner = SRC721.ownerOf(tokenId);
    ...
    function _beforeTokenTransfer(
        ...
        require(!isLocked(tokenId), "SRC5058: token transfer while locked");
    }
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(ISRC165, SRC721) returns (bool) {
        return interfaceId == type(ISRC5058).interfaceId || super.supportsInterface(interfaceId);
    }
}
