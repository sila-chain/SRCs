...
import {ISRC20Checkpointed} from "./interfaces/ISRC20Checkpointed.sol";

/**
 * @dev A forked SRC20 contract that is forked from ISRC20Checkpointed at certain checkpoint nonce
 */
abstract contract SRC20Forked is Context, ISRC20, ISRC20Metadata, ISRC20Errors {
    using LibBitmap for LibBitmap.Bitmap;

    mapping(address account => uint256) private _balances;

    /**
     * @dev Tracks which accounts derive their balances from the checkpointed token.
     */
    ...
    ISRC20Checkpointed private immutable _sourceToken;
    ...
    constructor(string memory name_, string memory symbol_, uint48 forkedNonce_, address sourceToken_) {
        _sourceToken = ISRC20Checkpointed(sourceToken_);

        require(forkedNonce_ <= _sourceToken.checkpointNonce(), "SRC20Forked: checkpointed nonce is in the future");
    ...
    /**
     * @dev uses decimals of checkpointed token
     */
     ...
     * required by the SRC. See {xref-SRC20-_approve-address-address-uint256-bool-}[_approve].
     *
    ...
    function _update(address from, address to, uint256 value) internal virtual {
        if (from == address(0)) {
            ...
            if (fromBalance < value) {
                revert SRC20InsufficientBalance(from, fromBalance, value);
            }
            unchecked {
    ...
    function _spendAllowance(address owner, address spender, uint256 value) internal virtual {
        ...
        if (currentAllowance < type(uint256).max) {
            if (currentAllowance < value) {
                revert SRC20InsufficientAllowance(spender, currentAllowance, value);
            }
            unchecked {
