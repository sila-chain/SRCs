...
abstract contract SRC20Expirable is Context, ISRC20Errors, ISRC7818 {
    using SortedList for SortedList.List;
    using SlidingWindow for SlidingWindow.Window;

    error SRC7818TransferredExpiredToken();
    error SRC7818InvalidEpoch();
    ...
    function _update(
        ...
        if (from == address(0)) {
            ...
            if (balance < value) {
                revert SRC20InsufficientBalance(from, balance, value);
            }
    ...
    function _spendAllowance(
        ...
        if (currentAllowance != type(uint256).max) {
            if (currentAllowance < value) {
                revert SRC20InsufficientAllowance(
                    spender,
                    currentAllowance,
                    value
                );
    ...
    /// @dev See {ISRC20Metadata-name}.
    function name() public view virtual returns (string memory) {
        return _name;
    }

    /// @dev See {ISRC20Metadata-symbol}.
    function symbol() public view virtual returns (string memory) {
        return _symbol;
    }

    /// @dev See {ISRC20Metadata-decimals}.
    ...
    function epochType() public pure returns (EPOCH_TYPE) {
        return ISRC7818.EPOCH_TYPE.BLOCKS_BASED;
    }
    ...
    function transferAtEpoch(
        ...
        if (_expired(epoch)) {
            revert SRC7818TransferredExpiredToken();
        }
        address owner = _msgSender();
