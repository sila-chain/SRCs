...
contract RightsManagement is ISRC5218, SRC721URIStorage, Ownable {
  ...
  function supportsInterface(bytes4 interfaceId) public view virtual override(SRC721, ISRC165) returns (bool) {
    return
      interfaceId == type(ISRC5218).interfaceId ||
      super.supportsInterface(interfaceId);
  }

  function isLicenseActive(uint256 licenseId) public view virtual override(ISRC5218) returns (bool) {
  ...
  function getLicenseTokenId(uint256 licenseId) public view virtual override(ISRC5218) isActiveLicense(licenseId) returns (uint256) {
    return _licenses[licenseId].tokenId;   
  }

  function getParentLicenseId(uint256 licenseId) public view virtual override(ISRC5218) isActiveLicense(licenseId) returns (uint256) {        
    return _licenses[licenseId].parentLicenseId;
  }

  function getLicenseHolder(uint256 licenseId) public view virtual override(ISRC5218) isActiveLicense(licenseId) returns (address) {
    return _licenses[licenseId].licenseHolder;
  }

  function getLicenseURI(uint256 licenseId) public view virtual override(ISRC5218) isActiveLicense(licenseId) returns (string memory) {
    return _licenses[licenseId].uri;
  }

  function getLicenseRevoker(uint256 licenseId) public view virtual override(ISRC5218) isActiveLicense(licenseId) returns (address) {
    return _licenses[licenseId].revoker;
  }

  function getLicenseIdByTokenId(uint256 tokenId) public view virtual override(ISRC5218) returns (uint256) {
  ...
  function createLicense(
    ...
    public virtual override(ISRC5218)
    returns (uint256)
  {
  ...
  function revokeLicense(uint256 licenseId) public virtual override(ISRC5218) {
  ...
  function transferSublicense(uint256 licenseId, address licenseHolder) public virtual override(ISRC5218) {
    require(isLicenseActive(licenseId), "The license is not active");
