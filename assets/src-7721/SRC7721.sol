...
import "./interfaces/ISRC7721.sol";
...
abstract contract SRC7721 is SRC1155,ISRC7721{
    ...
    function lock(uint256 tokenId, address account, uint256 amount) public virtual override{
        require(
            ...
            "SRC1155: caller is not token owner or approved"
        );
    ...
    function _lock(uint256 tokenId, address account, address _locker, uint256 amount) internal {
        require(balanceOf(account, tokenId)>=amount,"SRC1155: Insufficient Balance");
    ...
    function unlock(uint256 tokenId, address account, uint256 amount) public virtual override{
        require(locker[tokenId][account][_msgSender()]>=amount,"SRC1155: Insufficient Locked Amount");
   ...
   /**
    ...
    function setApprovalForId(uint256 tokenId, address operator, uint256 amount) public virtual {
        require (amount>locker[tokenId][_msgSender()][operator], "SRC1155: Insufficient Locked Amount");
    ...
    function safeTransferFrom(
        ...
        uint256 amount,
        bytes memory data
    ) public virtual override(SRC1155,ISRC1155) {
        require(
        ...
        if (from != address(0)) { 
            for (uint256 i = 0; i < ids.length; i++) {
                ...
                if(locker[tokenId][from][operator]>0){
                    require( getApprovalForId(tokenId,from,operator) >= amount,"SRC7066: Locked");
                } else{
                    require(balanceOf(from, tokenId)-amount >= lockedAmount[tokenId][from],"SRC7066: Can't Spend Locked");
                }
            }
        }
     ...
     */
    function supportsInterface(bytes4 interfaceId) public view virtual override(ISRC165, SRC1155) returns (bool) {
         return
            interfaceId == type(ISRC7066SFT).interfaceId ||
