...
contract AccessTokenVerifier is IAccessTokenVerifier {
    bytes32 private constant SIP712DOMAIN_TYPEHASH =
    ...
    function hash(SIP712Domain memory sip712Domain) internal pure returns (bytes32) {
        return keccak256(
            abi.encode(
                SIP712DOMAIN_TYPEHASH,
                keccak256(bytes(sip712Domain.name)),
                keccak256(bytes(sip712Domain.version)),
                sip712Domain.chainId,
                sip712Domain.verifyingContract
            )
        );
    }
