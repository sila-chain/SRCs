...
import "./interfaces/ISRC20charity.sol";
...
abstract contract SRC20Charity is ISRC20charity, SRC20, Ownable {
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view virtual override(ISRC165) returns (bool) {
        return
            interfaceId == type(ISRC20charity).interfaceId ||
    ...
    function setSpecificDefaultAddress(
        ...
        require(
            ...
            "SRC20Charity: invalid whitelisted rate"
        );
    ...
    function setSpecificDefaultAddressAndRate(
        ...
        require(
            ...
            "SRC20Charity: rate must be between 0 and _feeDenominator"
        );
        require(
            rate >= _defaultRate(),
            "SRC20Charity: rate fee must exceed default rate"
        );
        require(
            rate >= whitelistedRate[whitelistedAddr],
            "SRC20Charity: rate fee must exceed the fee set by the owner"
        );
        require(
            whitelistedRate[whitelistedAddr] != 0,
            "SRC20Charity: invalid whitelisted address"
        );
        _defaultAddress[msg.sender] = whitelistedAddr;
