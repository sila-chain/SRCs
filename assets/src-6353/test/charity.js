...
describe('Compile: Test charity donation configuration ', function() {
    ...
    it("Fails: User Whitelist a charity address that is not whitelisted ", async function () {
        await  expect(charity.connect(user).setSpecificDefaultAddressAndRate(charity3.address,20)).to.be.revertedWith( "SRC20Charity: invalid whitelisted address");
    });

    it("Fails: User Whitelist a charity address with an insufficient rate", async function () {
        await  expect(charity.connect(user).setSpecificDefaultAddressAndRate(charity1.address,5)).to.be.revertedWith( "SRC20Charity: rate fee must exceed default rate");
        await  expect(charity.connect(user).setSpecificDefaultAddressAndRate(charity2.address,100)).to.be.revertedWith( "SRC20Charity: rate fee must exceed the fee set by the owner");
    });
