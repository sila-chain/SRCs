...
describe("Multi-Fractional Non-Fungible Token", async () => {
    ...
    describe("NFT Ownership", async () => {
        ...
        it("should revert if given parentNFTContractAddress doesn't support SRC-721", async() => {
            const { MFNFT } = await setupTests();
    ...
    describe("onSRC721Received", async () => {

        it("should be able to accept SRC-721 token with safeTransferFrom()", async () => {
            const { MFNFT, NFT } = await setupTests()
        ...
        it("should return expected value for onSRC721Received()", async () => {
            const { MFNFT } = await setupTests()
            expect(await MFNFT.onSRC721Received(admin.address, user1.address, tokenId, 0x0)).to.be.equal("0x150b7a02")
        });
