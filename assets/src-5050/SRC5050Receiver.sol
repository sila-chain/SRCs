...
contract SRC5050Receiver is ISRC5050Receiver {
        ...
        virtual
    {
        if (action.state != address(0)) {
            require(action.state.isContract(), "SRC5050: invalid state");
            try
                ISRC5050Receiver(action.state).onActionReceived{
                    value: msg.value
                }(action, nonce)
            {} catch (bytes memory reason) {
                if (reason.length == 0) {
                    revert("SRC5050: call to non SRC5050Receiver");
                } else {
                    assembly {
