...
contract SRC5050Sender is ISRC5050Sender {
    ...
    function approveForAction(
        ...
        require(_approved != _account, "SRC5050: approve to caller");

        require(
            msg.sender == _account ||
                isApprovedForAllActions(_account, msg.sender),
            "SRC5050: approve caller is not account nor approved for all"
        );
    ...
    function setApprovalForAllActions(address _operator, bool _approved)
        ...
        override
    {
        require(msg.sender != _operator, "SRC5050: approve to caller");
    ...
    function _sendAction(Action memory action) internal {   
        ...
        if (next.isContract()) {
            try
                ISRC5050Receiver(next).onActionReceived{value: msg.value}(
                    action,
                    nonce
                )
