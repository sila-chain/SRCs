...
import {ISRC7943Fungible} from "../contracts/interfaces/ISRC7943.sol";
import {ISRC20} from "@openzeppelin/contracts/token/SRC20/ISRC20.sol";
import {IAccessControl} from "@openzeppelin/contracts/access/IAccessControl.sol";
import {ISRC20Errors} from "@openzeppelin/contracts/interfaces/draft-ISRC6093.sol";
...
contract uRWA20Test is Test {
    ...
    function test_SendWhitelist_ChangeStatus() public {
        ...
        emit uRWA20.SendWhitelisted(otherUser, true);
        token.changeSendWhitelist(otherUser, true);
        assertTrue(token.canSend(otherUser));

        vm.prank(whitelister);
        vm.expectEmit(true, false, false, true);
        emit uRWA20.SendWhitelisted(otherUser, false);
    ...
    function test_ReceiveWhitelist_ChangeStatus() public {
        ...
        vm.prank(whitelister);
        vm.expectEmit(true, false, false, true);
        emit uRWA20.ReceiveWhitelisted(otherUser, true);
        token.changeReceiveWhitelist(otherUser, true);
        assertTrue(token.canReceive(otherUser));

        vm.prank(whitelister);
        vm.expectEmit(true, false, false, true);
        emit uRWA20.ReceiveWhitelisted(otherUser, false);
    ...
    function test_Mint_Success() public {
        ...
        emit ISRC20.Transfer(address(0), user2, mintAmount);
    ...
    function test_Revert_Mint_ToNonWhitelisted() public {
        vm.prank(minter);
        vm.expectRevert(abi.encodeWithSelector(ISRC7943Fungible.SRC7943CannotReceive.selector, nonWhitelistedUser));
        token.mint(nonWhitelistedUser, TRANSFER_AMOUNT);
    }

    function test_Revert_Mint_ToZeroAddress() public {
        vm.prank(minter);
        vm.expectRevert(abi.encodeWithSelector(ISRC20Errors.SRC20InvalidReceiver.selector, address(0)));
        token.mint(address(0), TRANSFER_AMOUNT);
    }

    function test_Mint_ToAccountThatCanReceiveButNotSend() public {
        // Account that can receive but not send should still be mintable to
        vm.startPrank(whitelister);
        token.changeSendWhitelist(otherUser, false);
        token.changeReceiveWhitelist(otherUser, true);
        vm.stopPrank();
        ...
        vm.prank(minter);
        token.mint(otherUser, TRANSFER_AMOUNT);
        assertEq(token.balanceOf(otherUser), TRANSFER_AMOUNT);
    }
    ...
    function test_Burn_Success() public {
        ...
        vm.prank(user1);
        vm.expectEmit(true, true, true, true);
        emit ISRC20.Transfer(user1, address(0), BURN_AMOUNT);
    ...
    function test_Revert_Burn_CannotSend() public {
        ...
        // Remove burner from send whitelist
        vm.prank(whitelister);
        token.changeSendWhitelist(burner, false);

        vm.prank(burner);
        vm.expectRevert(abi.encodeWithSelector(ISRC7943Fungible.SRC7943CannotSend.selector, burner));
        token.burn(BURN_AMOUNT);
    }
    ...
    function test_Revert_Burn_InsufficientBalance() public {
        ...
        uint256 burnAmount = available + 1;
        vm.expectRevert(abi.encodeWithSelector(ISRC20Errors.SRC20InsufficientBalance.selector, user1, available, burnAmount));
        vm.prank(user1);
    ...
    function test_Burn_Success_ReducesFrozenWhenExceedsUnfrozen() public {
        ...
        uint256 burnAmount = unfrozenBalance + 100 * 1e18;
        ...
        vm.prank(user1);
        vm.expectEmit(true, true, true, true);
        emit ISRC7943Fungible.Frozen(user1, expectedNewFrozenAmount);
        vm.expectEmit(true, true, true, true);
        emit ISRC20.Transfer(user1, address(0), burnAmount);
    ...
    function test_Burn_Success_DoesNotChangeFrozenWhenWithinUnfrozen() public {
        ...
        uint256 unfrozenBalance = token.balanceOf(user1) - frozenAmount;
        uint256 burnAmount = unfrozenBalance - 50 * 1e18;
        ...
        assertEq(token.balanceOf(user1), initialBalance - burnAmount);
        assertEq(token.totalSupply(), initialSupply - burnAmount);
        assertEq(token.getFrozenTokens(user1), frozenAmount);
    }
    ...
    function test_CanTransfer_Success_WhenNoFrozenTokens() public view {
        ...
        // It only checks canSend/canReceive
        assertTrue(token.canTransfer(user1, user2, INITIAL_MINT_AMOUNT + 1));
    }

    function test_Revert_ForcedTransfer_FromZeroAddress() public {
        vm.prank(forceTransferrer);
        vm.expectRevert(abi.encodeWithSelector(ISRC20Errors.SRC20InvalidSender.selector, address(0)));
    ...
    function test_Freeze_MoreThanBalance_Success() public {
        ...
        vm.prank(user1);
        vm.expectRevert(abi.encodeWithSelector(ISRC7943Fungible.SRC7943InsufficientUnfrozenBalance.selector, user1, 1, 0));
    ...
    function test_Revert_SelfTransfer_WhenFrozen() public {
        ...
        uint256 transferAmount = balance - freezeAmount + 1;
        vm.prank(user1);
        vm.expectRevert(abi.encodeWithSelector(ISRC7943Fungible.SRC7943InsufficientUnfrozenBalance.selector, user1, transferAmount, balance - freezeAmount));
    ...
    function test_Transfer_Success_WhitelistedToWhitelisted() public {
        ...
        vm.prank(user1);
        vm.expectEmit(true, true, true, true);
        emit ISRC20.Transfer(user1, user2, TRANSFER_AMOUNT);
    ...
    function test_Revert_Transfer_FromCannotSend() public {
        ...
        vm.prank(user1);
        vm.expectRevert(abi.encodeWithSelector(ISRC7943Fungible.SRC7943CannotSend.selector, user1));
    ...
    function test_Revert_Transfer_InsufficientBalance() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(ISRC20Errors.SRC20InsufficientBalance.selector, user1, INITIAL_MINT_AMOUNT, transferAmount));
        vm.prank(user1);
    ...
    function test_Approve_Success() public {
        vm.prank(user1);
        vm.expectEmit(true, true, true, true);
        emit ISRC20.Approval(user1, user2, APPROVE_AMOUNT);
    ...
    function test_TransferFrom_Success() public {
        ...
        vm.prank(user2);
        vm.expectEmit(true, true, true, true);
        emit ISRC20.Transfer(user1, otherUser, APPROVE_AMOUNT);
    ...
    function test_Revert_TransferFrom_FromCannotSend() public {
        vm.prank(user1);
        token.approve(user2, APPROVE_AMOUNT);
    ...
    function test_Revert_TransferFrom_ToCannotReceive() public {
        vm.prank(user1);
        token.approve(user2, APPROVE_AMOUNT);
    ...
    function test_Revert_TransferFrom_InsufficientAllowance() public {
        vm.prank(user1);
        token.approve(user2, APPROVE_AMOUNT - 1);

        vm.prank(user2);
        vm.expectRevert(abi.encodeWithSelector(ISRC20Errors.SRC20InsufficientAllowance.selector, user2, APPROVE_AMOUNT - 1, APPROVE_AMOUNT));
    ...
    function test_ForcedTransfer_Success_WhitelistedToWhitelisted() public {
        ...
        vm.prank(forceTransferrer);
        vm.expectEmit(true, true, true, true);
        emit ISRC20.Transfer(user1, user2, FORCE_TRANSFER_AMOUNT);
        vm.expectEmit(true, true, true, true);
        emit ISRC7943Fungible.ForcedTransfer(user1, user2, FORCE_TRANSFER_AMOUNT);
    ...
    function test_ForcedTransfer_Success_FromCannotSendToWhitelisted() public {
        vm.prank(whitelister);
        token.changeSendWhitelist(user1, false);
        assertFalse(token.canSend(user1));
    ...
    function test_Revert_ForcedTransfer_InsufficientBalance() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(ISRC20Errors.SRC20InsufficientBalance.selector, user1, available, forceTransferAmount));
        vm.prank(forceTransferrer);
    ...
    function test_ForcedTransfer_Success_ReducesFrozenWhenExceedsUnfrozen() public {
        ...
        emit ISRC7943Fungible.Frozen(user1, expectedNewFrozenAmount);
        vm.expectEmit(true, true, true, true);
        emit ISRC20.Transfer(user1, user2, forceTransferAmount);
        vm.expectEmit(true, true, true, true);
        emit ISRC7943Fungible.ForcedTransfer(user1, user2, forceTransferAmount);
    ...
    function test_ForcedTransfer_Success_DoesNotChangeFrozenWhenWithinUnfrozen() public {
        ...
        uint256 unfrozenBalance = token.balanceOf(user1) - frozenAmount;
        uint256 forceTransferAmount = unfrozenBalance - 50 * 1e18;
    ...
    function test_ForcedTransfer_Success_ExactlyUnfrozenAmount() public {
        ...
        uint256 unfrozenBalance = token.balanceOf(user1) - frozenAmount;
        uint256 forceTransferAmount = unfrozenBalance;
    ...
    function test_ForcedTransfer_Success_AllTokensFrozenThenForceTransferAll() public {
        ...
        vm.prank(forceTransferrer);
        vm.expectEmit(true, true, true, true);
        emit ISRC7943Fungible.Frozen(user1, 0);
        vm.expectEmit(true, true, true, true);
        emit ISRC20.Transfer(user1, user2, allTokens);
        vm.expectEmit(true, true, true, true);
        emit ISRC7943Fungible.ForcedTransfer(user1, user2, allTokens);
    ...
    function test_Freeze_Success() public {
        vm.expectEmit(true, true, true, true);
        emit ISRC7943Fungible.Frozen(user1, FREEZE_AMOUNT) ;
        vm.prank(freezer);
    ...
    function test_Revert_Transfer_When_AllSenderBalanceFrozen() public {
        vm.prank(freezer);
        token.setFrozenTokens(user1, INITIAL_MINT_AMOUNT);

        vm.expectRevert(abi.encodeWithSelector(ISRC7943Fungible.SRC7943InsufficientUnfrozenBalance.selector, user1, TRANSFER_AMOUNT, 0));
        vm.prank(user1);
        assertFalse(token.transfer(user2, TRANSFER_AMOUNT));
    }
    ...
    function test_SetFrozenTokens_EmitsCorrectEvents() public {
        ...
        vm.prank(freezer);
        vm.expectEmit(true, true, true, true);
        emit ISRC7943Fungible.Frozen(user1, newFrozenAmount);
        token.setFrozenTokens(user1, newFrozenAmount);

        uint256 updatedFrozenAmount = FREEZE_AMOUNT * 2;
        vm.prank(freezer);
        vm.expectEmit(true, true, true, true);
        emit ISRC7943Fungible.Frozen(user1, updatedFrozenAmount);
    ...
    function test_EdgeCase_ForceTransferExactBalance() public {
        ...
        vm.prank(forceTransferrer);
        vm.expectEmit(true, true, true, true);
        emit ISRC20.Transfer(user1, user2, userBalance);
        vm.expectEmit(true, true, true, true);
        emit ISRC7943Fungible.ForcedTransfer(user1, user2, userBalance);
    ...
    function test_Revert_Transfer_When_TransferExceedsAvailableAfterFreeze() public {
        ...
        vm.prank(user1);
        vm.expectRevert(abi.encodeWithSelector(ISRC7943Fungible.SRC7943InsufficientUnfrozenBalance.selector, user1, TRANSFER_AMOUNT, available));
        assertFalse(token.transfer(user2, TRANSFER_AMOUNT));
    }
    ...
    function test_Transfer_Success_When_TransferIsWithinAvailableAfterPartialFreeze() public {
        ...
        vm.prank(user1);
        vm.expectEmit(true, true, true, true);
        emit ISRC20.Transfer(user1, user2, availableAmountToTransfer);
    ...
    function test_Revert_TransferFrom_When_FromBalanceFrozenAndTransferExceedsAvailable() public {
        ...
        uint256 amountToFreeze = INITIAL_MINT_AMOUNT - (APPROVE_AMOUNT / 2);
        ...
        vm.prank(user2);
        vm.expectRevert(abi.encodeWithSelector(ISRC7943Fungible.SRC7943InsufficientUnfrozenBalance.selector, user1, APPROVE_AMOUNT, available));
        assertFalse(token.transferFrom(user1, otherUser, APPROVE_AMOUNT));
    }
    ...
    function test_Interface_SupportsISRC7943() public view {
        assertTrue(token.supportsInterface(type(ISRC7943Fungible).interfaceId));
    }

    function test_Interface_SupportsISRC20() public view {
        assertTrue(token.supportsInterface(type(ISRC20).interfaceId));
    }
