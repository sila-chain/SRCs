...
import {ISRC7943Fungible} from "./interfaces/ISRC7943.sol";
...
contract uRWA20 is Context, SRC20, AccessControlEnumerable, ISRC7943Fungible {
    ...
    function _update(address from, address to, uint256 amount) internal virtual override {
        if (from != address(0) && to != address(0)) { // Transfer
            ...
            require(amount <= unfrozenFromBalance, SRC7943InsufficientUnfrozenBalance(from, amount, unfrozenFromBalance));
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(AccessControlEnumerable, ISRC165) returns (bool) {
        return interfaceId == type(ISRC7943Fungible).interfaceId ||
