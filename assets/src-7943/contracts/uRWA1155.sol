...
import {ISRC7943MultiToken} from "./interfaces/ISRC7943.sol";
import {SRC1155} from "@openzeppelin/contracts/token/SRC1155/SRC1155.sol";
...
/// @notice An SRC-1155 token implementation adhering to the ISRC-7943 interface for Real World Assets.
/// @dev Combines standard SRC-1155 functionality with RWA-specific features like whitelisting,
/// controlled minting/burning, asset forced transfers, and freezing. Managed via AccessControl.
contract uRWA1155 is Context, SRC1155, AccessControlEnumerable, ISRC7943MultiToken {
    ...
    /// @dev Initializes the SRC-1155 token with a URI and grants all roles
    ...
    constructor(string memory uri, address initialAdmin) SRC1155(uri) {
    ...
    function canTransfer(address from, address to, uint256 tokenId, uint256 amount) public view virtual override returns (bool allowed) {
        uint256 frozen = getFrozenTokens(from, tokenId);
        if (frozen > 0) {
    ...
    function forcedTransfer(address from, address to, uint256 tokenId, uint256 amount) public virtual override onlyRole(FORCE_TRANSFER_ROLE) returns(bool result) {
        require(to != address(0), SRC1155InvalidReceiver(address(0)));
        require(from != address(0), SRC1155InvalidSender(address(0)));
        ...
        if (to != address(0)) {
            ...
            if (ids.length == 1) {
                ...
                SRC1155Utils.checkOnSRC1155Received(operator, from, to, id, value, "");
            } else {
                SRC1155Utils.checkOnSRC1155BatchReceived(operator, from, to, ids, values, "");
            }
        }
    ...
    /// @dev Overrides the SRC-1155 `_update` hook. Enforces transfer restrictions based on {canTransfer}, {canSend} and {canReceive} logic.
    /// Reverts with {SRC7943CannotSend} | {SRC7943CannotReceive} | {SRC7943InsufficientUnfrozenBalance} | {SRC1155InsufficientBalance}
    /// if any check fails.
    ...
    function _update(address from, address to, uint256[] memory ids, uint256[] memory values) internal virtual override {
        ...
        if (from != address(0) && to != address(0)) { // Transfer
            ...
            for (uint256 i = 0; i < ids.length; ++i) {
                ...
                require(value <= balanceOf(from, id), SRC1155InsufficientBalance(from, balanceOf(from, id), value, id));
                require(value <= unfrozenBalance, SRC7943InsufficientUnfrozenBalance(from, id, value, unfrozenBalance));
            }
    ...
    /// @dev Indicates support for the {ISRC7943MultiToken} interface in addition to inherited interfaces.
    /// @param interfaceId The interface identifier, as specified in SRC-165.
    /// @return True if the contract implements `interfaceId`, false otherwise.
    function supportsInterface(bytes4 interfaceId) public view virtual override(AccessControlEnumerable, SRC1155, ISRC165) returns (bool) {
        return interfaceId == type(ISRC7943MultiToken).interfaceId ||
