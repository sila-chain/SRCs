...
import {ISRC7943NonFungible} from "./interfaces/ISRC7943.sol";
import {SRC721} from "@openzeppelin/contracts/token/SRC721/SRC721.sol";
import {SRC721Utils} from "@openzeppelin/contracts/token/SRC721/utils/SRC721Utils.sol";
...
/// @notice An SRC-721 token implementation adhering to the ISRC-7943 interface for Real World Assets.
/// @dev Combines standard SRC-721 functionality with RWA-specific features like whitelisting,
/// controlled minting/burning, asset forced transfers, and freezing. Managed via AccessControl.
contract uRWA721 is Context, SRC721, AccessControlEnumerable, ISRC7943NonFungible {
    ...
    /// @dev Initializes the SRC-721 token with name and symbol, and grants all roles
    ...
    constructor(string memory name, string memory symbol, address initialAdmin) SRC721(name, symbol) {
    ...
    /// @inheritdoc ISRC7943NonFungible
    ...
    /// Performs an SRC721 receiver check on `to` if it is a contract.
    ...
    function burn(uint256 tokenId) external onlyRole(BURNER_ROLE) {
        ...
        if (previousOwner == address(0)) revert SRC721NonexistentToken(tokenId);
    }
    ...
    function forcedTransfer(address from, address to, uint256 tokenId) public virtual override onlyRole(FORCE_TRANSFER_ROLE) returns(bool result) {
        require(to != address(0), SRC721InvalidReceiver(address(0)));
        require(from != address(0), SRC721InvalidSender(address(0)));
        require(canReceive(to), SRC7943CannotReceive(to));
        require(ownerOf(tokenId) == from, SRC721IncorrectOwner(from, tokenId, ownerOf(tokenId)));
        _excessFrozenUpdate(from , tokenId);
        super._update(to, tokenId, address(0)); // Skip _update override
        SRC721Utils.checkOnSRC721Received(_msgSender(), from, to, tokenId, "");
    ...
    /// @dev Overrides the SRC-721 `_update` hook. Enforces transfer restrictions based on {canTransfer}, {canSend} and {canReceive} logic.
    /// Reverts with {SRC721IncorrectOwner} | {SRC7943InsufficientUnfrozenBalance} | {SRC7943CannotSend} | {SRC7943CannotReceive} if any check fails.
    ...
    function _update(address to, uint256 tokenId, address auth) internal virtual override returns(address) {
        ...
        if (from == address(0) && to != address(0)) { // Mint
            ...
            require(from == _ownerOf(tokenId), SRC721IncorrectOwner(from, tokenId, _ownerOf(tokenId)));
            require(!getFrozenTokens(from, tokenId), SRC7943InsufficientUnfrozenBalance(from, tokenId));
            require(canSend(from), SRC7943CannotSend(from));
            require(canReceive(to), SRC7943CannotReceive(to));
        } else {
            revert SRC721NonexistentToken(tokenId);
        }
    ...
    /// @dev Indicates support for the {ISRC7943NonFungible} interface in addition to inherited interfaces.
    /// @param interfaceId The interface identifier, as specified in SRC-165.
    /// @return True if the contract implements `interfaceId`, false otherwise.
    function supportsInterface(bytes4 interfaceId) public view virtual override(AccessControlEnumerable, SRC721, ISRC165) returns (bool) {
        return interfaceId == type(ISRC7943NonFungible).interfaceId ||
