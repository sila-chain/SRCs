...
import "./interfaces/ISRC3475.sol";

contract SRC3475 is ISRC3475 {
    /** 
    ...
    struct Nonce {
        ...
        mapping(uint256 => ISRC3475.Values) _values;
    ...
    struct Class {
        mapping(uint256 => ISRC3475.Values) _values;
        mapping(uint256 => ISRC3475.Metadata) _nonceMetadata;
    ...
    mapping(uint256 => ISRC3475.Metadata) _classMetadata;
    ...
    function transferFrom(
        ...
        require(
            _from != address(0),
            "SRC3475: can't transfer from the zero address"
        );
        require(
            _to != address(0),
            "SRC3475:use burn() instead"
        );
        require(
            msg.sender == _from ||
            isApprovedFor(_from, msg.sender),
            "SRC3475:caller-not-owner-or-approved"
        );
    ...
    function transferAllowanceFrom(
        ...
        require(
            _from != address(0),
            "SRC3475: can't transfer allowed amt from zero address"
        );
        require(
            _to != address(0),
            "SRC3475: use burn() instead"
    ...
    override
    {
        ...
        for (uint256 i = 0; i < len; i++) {
            require(
                _to != address(0),
                "SRC3475: can't issue to the zero address"
            );
    ...
    override
    {
        require(
            _from != address(0),
            "SRC3475: can't redeem from the zero address"
        );
        uint256 len = _transactions.length;
        for (uint256 i = 0; i < len; i++) {
    ...
    override
    {
        require(
            _from != address(0),
            "SRC3475: can't burn from the zero address"
        );
        require(
            msg.sender == _from ||
            isApprovedFor(_from, msg.sender),
    ...
    function balanceOf(
        ...
        require(
            account != address(0),
            "SRC3475: balance query for the zero address"
        );
