...
describe("Transferable", async function () {
  let nonTransferable: SRC721TransferableMock;
  ...
  it("reverts if token does not exist", async function () {
    await expect(nonTransferable['isTransferable(uint256,address,address)'](10, owner.address, otherOwner.address)).to.be.revertedWith("SRC721: invalid token ID");
  });
