...
// SRC2266 is compatible with SRC20 standard: https://thesila.wiki/w/index.php/SRC20_Token_Standard
...
contract Example
{
    ...
    modifier canInitiate(bytes32 secretHash) {
        ...
        require(initiatorAsset[secretHash].state == AssetState.Empty);
        require(SRC20(swap[secretHash].tokenA).balanceOf(msg.sender) >= initiatorAsset[secretHash].amount);
        _;
    }
    ...
    modifier canFillPremium(bytes32 secretHash) {
        require(swap[secretHash].initiator == msg.sender);
        require(premium[secretHash].state == AssetState.Empty);
        require(SRC20(swap[secretHash].tokenB).balanceOf(msg.sender) >= premium[secretHash].amount);
        _;
    }
    ...
    modifier canParticipate(bytes32 secretHash) {
        ...
        require(SRC20(swap[secretHash].tokenB).balanceOf(msg.sender) >= participantAsset[secretHash].amount);
        _;
    }
    ...
    {
        SRC20(swap[secretHash].tokenA).transferFrom(swap[secretHash].initiator, address(this), initiatorAsset[secretHash].amount);
    ...
    {   
        SRC20(swap[secretHash].tokenB).transferFrom(swap[secretHash].initiator, address(this), premium[secretHash].amount);
    ...
    function participate(bytes32 secretHash, uint256 assetRefundTime)
        ...
        checkRefundTimestampOverflow(assetRefundTime)
    {
        SRC20(swap[secretHash].tokenB).transferFrom(swap[secretHash].participant, address(this), participantAsset[secretHash].amount);
        ...
        isAssetRedeemable(secretHash, secret)
    {
        ...
        if (swap[secretHash].initiator == msg.sender) {
            SRC20(swap[secretHash].tokenB).transfer(msg.sender, participantAsset[secretHash].amount);
            ...
            SRC20(swap[secretHash].tokenA).transfer(msg.sender, initiatorAsset[secretHash].amount);
    ...
    {
        SRC20(swap[secretHash].tokenB).transfer(msg.sender, premium[secretHash].amount);
