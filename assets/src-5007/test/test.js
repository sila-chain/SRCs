...
contract("test SRC5007", async accounts => {

    it("test SRC5007", async () => {
        const Alice = accounts[0];

        const instance = await SRC5007Demo.deployed("SRC5007Demo", "SRC5007Demo");
        const demo = instance;
        ...
        console.log("ISRC5007 InterfaceId:", await demo.getInterfaceId())
        let isSupport = await demo.supportsInterface('0x7a0cdf92');
        assert.equal(isSupport, true , "supportsInterface error");
        
    });

    it("test SRC5007Composable", async () => {
        const Alice = accounts[0];
        const Bob = accounts[1];
        const Carl = accounts[2];

        const instance = await SRC5007ComposableTest.deployed("SRC5007ComposableTest", "SRC5007ComposableTest");
        const demo = instance;
        ...
        console.log("ISRC5007Composable InterfaceId:", await demo.getInterfaceId())
