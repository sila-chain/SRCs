...
describe("AttributesRepository", async function () {
  ...
  let ownedCollection1: OwnableMintableSRC721Mock;
  let ownedCollection2: OwnableMintableSRC721Mock;
  ...
  it("can support ISRC165", async function () {
    expect(await tokenAttributes.supportsInterface(ISRC165)).to.equal(true);
  });

  it("can support ISRC7508", async function () {
    expect(await tokenAttributes.supportsInterface(ISRC7508)).to.equal(true);
  });
