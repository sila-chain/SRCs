...
/// @dev SRC-165 interfaceId = XOR of this interface's own 5 function selectors (inherited
/// ISRC165.supportsInterface is excluded per the language rule). Value: 0xd6da8150.
interface IConfidentialPolicyVerdict is ISRC165 {
    ...
    /// @dev The domain declares an SRC-8004 Identity Registry and `agentId` does not exist in it.
    ...
    /// @notice SIP-712 digest an executor signs to authorize RELAYED submission of this exact
    ...
    /// SIP-712 signature (ECDSA or SRC-1271) by v.executor over verdictDigest(v). Because the
