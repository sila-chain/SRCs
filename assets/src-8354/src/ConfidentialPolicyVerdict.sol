...
contract ConfidentialPolicyVerdict is IConfidentialPolicyVerdict, SIP712, SRC165 {
    ...
    constructor(IPolicyDomainRegistry _registry) SIP712("ConfidentialPolicyVerdict", "1") {
        registry = _registry;
    }
    ...
    /// @inheritdoc ISRC165
    function supportsInterface(bytes4 interfaceId) public view override(SRC165, ISRC165) returns (bool) {
    ...
    function _consume(Verdict calldata v, bytes calldata proof, bytes memory executorAuth) internal {
        ...
        IPolicyDomainRegistry.Domain memory d = registry.domain(v.domainId);
        // Conditional on the domain declaring an SRC-8004 Identity Registry: a domain on a chain
    ...
    /// @dev Existence is an SRC-721 ownership read: the agent exists when `ownerOf` returns a
    ...
    /// SIP-712 signature (ECDSA or SRC-1271) over this verdict's digest.
