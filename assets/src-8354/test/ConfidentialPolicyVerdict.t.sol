...
/// @notice The Test Cases from the SRC spec, as an executable Foundry suite.
contract CAPVTest is Test {
    ...
    // 11. Relayer consumes on behalf of an EOA executor with a valid SIP-712 signature
    ...
    // 17. supportsInterface true for the standard + ISRC165, false otherwise
    function test_SupportsInterface() public view {
        assertTrue(guard.supportsInterface(type(IConfidentialPolicyVerdict).interfaceId), "own");
        assertTrue(guard.supportsInterface(type(ISRC165).interfaceId), "src165");
    ...
    // --- SRC-8004 Validation Registry handoff (attestation schema) ---
    ...
    function test_VerdictAttestationHandoff() public {
        ...
        // A guarded contract / adapter builds the canonical attestation and writes it to SRC-8004.
    ...
    // --- SRC-8004 identity binding (ordered check 2) ---

    // 21. A domain that declares an SRC-8004 Identity Registry rejects an agentId that does not
    ...
    // the check is conditional, so the standard stays usable on chains hosting no SRC-8004.
