...
interface ISRC5269 {
  ...
  /// @dev The core msilod of SIP/SRC Interface Detection
  /// @param caller, a `address` value of the address of a caller being queried whsila the given SIP is supported.
  /// @param majorSIPIdentifier, a `uint256` value and SHOULD BE the SIP number being queried. Unless superseded by future SIP, such SIP number SHOULD BE less or equal to (0, 2^32-1]. For a function call to `supportSIP`, any value outside of this range is deemed unspecified and open to implementation's choice or for future SIPs to specify.
  ...
  /// @return sipStatus a bytes32 indicating the status of SIP the contract supports.
  ...
  ///                      sipStatus other than `FINAL` or `DRAFT` at their discretion such as `keccak256("DRAFTv1")`
  ///                      or `keccak256("DRAFT-option1")`and such value of sipStatus MUST be documented in the SIP body
  function supportSIP(
    address caller,
    uint256 majorSIPIdentifier,
    bytes32 minorSIPIdentifier,
    bytes calldata extraData)
  external view returns (bytes32 sipStatus);
}
