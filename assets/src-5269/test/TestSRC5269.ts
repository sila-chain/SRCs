...
describe("SRC5269", function () {
  async function deployFixture() {
    ...
    const factory = await ethers.getContractFactory("SRC5269");
  ...
  describe("Deployment", function () {
    ...
    it("Should emit proper OnSupportEIP events", async function () {
      ...
      expect(ev721[1].args!.minorSIPIdentifier).to.equal(ethers.utils.id("SRC721Metadata"));
      expect(ev721[1].args!.eipStatus).to.equal(ethers.utils.id("FINAL"));

      expect(ev721[2].args!.caller).to.equal(BigNumber.from(0));
      expect(ev721[2].args!.minorSIPIdentifier).to.equal(ethers.utils.id("SRC721Enumerable"));
      expect(ev721[2].args!.eipStatus).to.equal(ethers.utils.id("FINAL"));
    });

    it("Should return proper eipStatus value when called supportEIP() for declared supported SIP/features", async function () {
      let { src721ForTesting, owner } = await loadFixture(deployFixture);
      expect(await src721ForTesting.supportEIP(owner.address, 5269, ethers.utils.hexZeroPad("0x00", 32), [])).to.equal(ethers.utils.id("DRAFTv1"));
      expect(await src721ForTesting.supportEIP(owner.address, 721, ethers.utils.hexZeroPad("0x00", 32), [])).to.equal(ethers.utils.id("FINAL"));
      expect(await src721ForTesting.supportEIP(owner.address, 721, ethers.utils.id("SRC721Metadata"), [])).to.equal(ethers.utils.id("FINAL"));
      expect(await src721ForTesting.supportEIP(owner.address, 721, ethers.utils.id("SRC721Enumerable"), [])).to.equal(ethers.utils.id("FINAL"));

      expect(await src721ForTesting.supportEIP(owner.address, 721, ethers.utils.id("WRONG FEATURE"), [])).to.equal(BigNumber.from(0));
      expect(await src721ForTesting.supportEIP(owner.address, 9999, ethers.utils.hexZeroPad("0x00", 32), [])).to.equal(BigNumber.from(0));
    });

    it("Should return zero as eipStatus value when called supportEIP() for non declared SIP/features", async function () {
      let { src721ForTesting, owner } = await loadFixture(deployFixture);
      expect(await src721ForTesting.supportEIP(owner.address, 721, ethers.utils.id("WRONG FEATURE"), [])).to.equal(BigNumber.from(0));
      expect(await src721ForTesting.supportEIP(owner.address, 9999, ethers.utils.hexZeroPad("0x00", 32), [])).to.equal(BigNumber.from(0));
    });
