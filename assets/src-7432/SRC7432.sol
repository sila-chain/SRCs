...
import { ISRC7432 } from "./interfaces/ISRC7432.sol";
import { ISRC721 } from '@openzeppelin/contracts/token/SRC721/ISRC721.sol';

contract SRC7432 is ISRC7432 {
    struct RoleData {
        address recipient;
    ...
    function grantRole(ISRC7432.Role calldata _role) external override onlyAllowedRole(_role.roleId) {
    ...
    function unlockToken(address _tokenAddress, uint256 _tokenId) external override {
        ...
        ISRC721(_tokenAddress).transferFrom(address(this), originalOwner, _tokenId);
    ...
    function supportsInterface(bytes4 interfaceId) external view virtual override returns (bool) {
        return interfaceId == type(ISRC7432).interfaceId;
    }
    ...
    function _depositNft(address _tokenAddress, uint256 _tokenId) internal returns (address originalOwner_) {
        address _currentOwner = ISRC721(_tokenAddress).ownerOf(_tokenId);
        ...
        if (_currentOwner == address(this)) {
            ...
            ISRC721(_tokenAddress).transferFrom(_currentOwner, address(this), _tokenId);
