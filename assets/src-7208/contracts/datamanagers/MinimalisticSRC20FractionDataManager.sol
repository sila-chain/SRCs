...
/**
 * @title Minimalistic SRC20 Fraction Data Manager
 * @notice Contract for managing SRC20 fractions of an SRC1155 token
 * @dev This contract is used to manage fractions of a MinimalisticSRC1155WithSRC20FractionsDataManager
 *      contract as a SRC20 token, exposing the SRC20 functionalities and emitting the Transfer event.
 *      NOTE: This implementation is minimalistic and does not include minting and burning functionalities.
 */
contract MinimalisticSRC20FractionDataManager is Initializable, IFractionTransferEventEmitter, ISRC20, ISRC20Metadata, ISRC20Errors, OwnableUpgradeable {
    /// @dev Decimals for the SRC20 token (set to 0)
    ...
    /// @dev SRC1155 data manager contract
    address public src1155dm;

    /// @dev SRC1155 token ID
    uint256 public src1155ID;

    /// @dev SRC20 token name
    string private _name;

    /// @dev SRC20 token symbol
    ...
    /// @notice Modifier to check if the caller is the SRC1155 data manager
    modifier onlyTransferNotifier() {
        if (_msgSender() != src1155dm) revert WrongTransferNotificationSource();
        _;
    }

    /// @dev Initializes the SRC20 Fraction Data Manager
    function initialize(
        bytes32 datapoint_,
        address dataIndex_,
        ...
        __MinimalisticSRC20FractionDataManager_init_unchained(datapoint_, dataIndex_, fungibleFractionsDO_, src1155dm_, src1155ID_, name_, symbol_);
    }

    function __MinimalisticSRC20FractionDataManager_init_unchained(
        bytes32 datapoint_,
        address dataIndex_,
        address fungibleFractionsDO_,
        address src1155dm_,
        ...
        if (datapoint_ == bytes32(0) || dataIndex_ == address(0) || fungibleFractionsDO_ == address(0) || src1155dm_ == address(0)) {
            revert WrongParams();
        }
        ...
        src1155dm = src1155dm_;
        src1155ID = src1155ID_;
        _name = name_;
        _symbol = symbol_;
    }

    /// @notice SRC20 token decimals
    function decimals() external pure returns (uint8) {
        return DECIMALS;
    }

    /// @notice SRC20 token name
    function name() external view returns (string memory) {
        return _name;
    }

    /// @notice SRC20 token symbol
    function symbol() external view returns (string memory) {
        return _symbol;
    }

    /**
     * @notice Total supply of the SRC20 token
     * @return The amount of SRC20 tokens in circulation
     * @dev This function reads the total supply from the fungible fractions data object
     *      NOTE: This total supply is equal to the amount of fractions of the SRC1155 token with the id `src1155ID`
     */
    function totalSupply() external view override returns (uint256) {
        if (address(fungibleFractionsDO) == address(0)) revert ContractNotInitialized();
        return abi.decode(fungibleFractionsDO.read(_datapoint, IFungibleFractionsOperations.totalSupply.selector, abi.encode(src1155ID)), (uint256));
    }

    /**
     * @notice Balance of an account
     * @param account The account to check the balance of
     * @return The amount of SRC20 tokens the account has
     * @dev This function reads the balance of the account from the fungible fractions data object
     *      NOTE: This balance is equal to the amount of fractions of the SRC1155 token with the id `src1155ID` the account has
     */
    function balanceOf(address account) external view override returns (uint256) {
        if (address(fungibleFractionsDO) == address(0)) revert ContractNotInitialized();
        return abi.decode(fungibleFractionsDO.read(_datapoint, IFungibleFractionsOperations.balanceOf.selector, abi.encode(account, src1155ID)), (uint256));
    }

    /// @dev Function only callable by the SRC1155 data manager to notify a fraction transfer
    ...
    function approve(address spender, uint256 value) public returns (bool) {
        if (_msgSender() == address(0)) {
            revert SRC20InvalidApprover(address(0));
        }
        if (spender == address(0)) {
            revert SRC20InvalidSpender(address(0));
        }
    ...
    function transfer(address to, uint256 amount) external virtual override returns (bool) {
        if (to == address(0)) {
            revert SRC20InvalidReceiver(address(0));
        }
    ...
    function transferFrom(address from, address to, uint256 amount) external virtual override returns (bool) {
        ...
        if (from == address(0)) {
            revert SRC20InvalidSender(address(0));
        }
        if (to == address(0)) {
            revert SRC20InvalidReceiver(address(0));
        }
    ...
    function _spendAllowance(address owner_, address spender, uint256 amount) internal {
        ...
        if (currentAllowanceAmount != type(uint256).max) {
            if (currentAllowanceAmount < amount) {
                revert SRC20InsufficientAllowance(spender, currentAllowanceAmount, amount);
            }
            unchecked {
    ...
    function _writeTransfer(address from, address to, uint256 amount) internal {
        ...
        // dataIndex.write(fungibleFractionsDO, _datapoint, IFungibleFractionsOperations.mint.selector, abi.encode(to, src1155ID, amount));
        // dataIndex.write(fungibleFractionsDO, _datapoint, IFungibleFractionsOperations.burn.selector, abi.encode(from, src1155ID, amount));

        dataIndex.write(fungibleFractionsDO, _datapoint, IFungibleFractionsOperations.transferFrom.selector, abi.encode(from, to, src1155ID, amount));

        IFractionTransferEventEmitter(src1155dm).fractionTransferredNotify(from, to, amount);
    }
