 ...
 * @notice DataObject with base functionality of Fungible Fractions (Can be used for SRC1155-Compatible DataManagers)
 ...
 *      access control, metadata management, etc. As may be an SRC1155 token.
 *
...
contract MinimalisticFungibleFractionsDO is IDataObject {
     ...
     * @param ids Enumerable set of object (SRC1155 token) ids
     * @param balances Mapping of object (SRC1155 token) id to balance of the user owning diid
    ...
    function _diid(DataPoint dp, address account) internal view returns (bytes32) {
        DataPointStorage storage dps = _dataPointStorage(dp);
        return dps.dataIndexImplementation.diid(account, dp);
    }
