...
describe("SRC7641", function () {
  ...
  beforeEach(async function () {
    ...
    const SRC7641 = await ethers.getContractFactory("SRC7641");
    src7641 = await SRC7641.deploy("SRC7641", "ERCX", supply, percentClaimable);
  ...
  describe("Deployment", function () {
    ...
    it("Should set the right symbol", async function () {
      expect(await src7641.symbol()).to.equal("ERCX");
    });
    ...
    it("Should assign the total supply to the owner", async function () {
      expect(await src7641.balanceOf(await ethers.provider.getSigner(0))).to.equal(supply);
    });
  });
  ...
  describe("Deposit", function () {
    it("Should deposit SIL to the contract", async function () {
      ...
      expect(await ethers.provider.getBalance(src7641Address)).to.equal(1000);
    });
  });
  ...
  describe("Burn", function () {
    it("Should burn tokens", async function () {
      ...
      expect(await src7641.balanceOf(await ethers.provider.getSigner(0))).to.equal(supply-10000);
    });

    it("Should burn tokens and receive SIL", async function () {
      await addr0.sendTransaction({ to: src7641Address, value: ethers.parseEther("1000") });
      expect(await ethers.provider.getBalance(src7641Address)).to.equal(ethers.parseEther("1000"));
      expect(await src7641.redeemableOnBurn(10000)).to.equal(ethers.parseEther("1000")*BigInt(10000)*BigInt(100-percentClaimable)/BigInt(supply)/BigInt(100));
    ...
    it("Should snapshot, deposit, and burn", async function () {
      ...
      await src7641.snapshot();
      await addr0.sendTransaction({ to: src7641Address, value: ethers.parseEther("1000") });
      expect(await src7641.redeemableOnBurn(10000)).to.equal(ethers.parseEther("2000")*BigInt(10000)*BigInt(100-percentClaimable)/BigInt(supply)/BigInt(100));
      const balanceBefore = await ethers.provider.getBalance(await ethers.provider.getSigner(0));
      await src7641.burn(10000);
      const balanceAfter = await ethers.provider.getBalance(await ethers.provider.getSigner(0));
  ...
  describe("Claim", function () {
    ...
    it("Should claim after snapshot", async function () {
      ...
      await network.provider.send("hardhat_mine", ["0x400"]);
      await src7641.snapshot();
      expect(await src7641.claimableRevenue(addr0, 1)).to.equal(ethers.parseEther("1000")*BigInt(percentClaimable)/BigInt(100));
      const balanceBefore = await ethers.provider.getBalance(await ethers.provider.getSigner(0));
    ...
    it("Should claim correctly after snapshot with two holders", async function () {
      ...
      await network.provider.send("hardhat_mine", ["0x400"]);
      await src7641.snapshot();
      expect(await src7641.claimableRevenue(addr0, 1)).to.equal(ethers.parseEther("1000")*BigInt(supply-100000)*BigInt(percentClaimable)/BigInt(100)/BigInt(supply));
      expect(await src7641.claimableRevenue(addr1, 1)).to.equal(ethers.parseEther("1000")*BigInt(100000)*BigInt(percentClaimable)/BigInt(100)/BigInt(supply));
    ...
    it("Should claim multiple snapshots correctly", async function () {
      ...
      await network.provider.send("hardhat_mine", ["0x400"]);
      await src7641.snapshot();
      await addr0.sendTransaction({ to: src7641Address, value: ethers.parseEther("2000") });
      await network.provider.send("hardhat_mine", ["0x400"]);
      await src7641.snapshot();
      expect(await src7641.claimableRevenue(addr0, 1)).to.equal(ethers.parseEther("1000")*BigInt(percentClaimable)/BigInt(100));
      expect(await src7641.claimableRevenue(addr0, 2)).to.equal(ethers.parseEther("2000")*BigInt(percentClaimable)/BigInt(100));
      const balanceBefore = await ethers.provider.getBalance(await ethers.provider.getSigner(0));
  ...
  describe("Mixed operations", function () {
    it("deposit -> snapshot -> deposit -> burn -> deposit -> burn -> snapshot -> claim -> burn", async function () {
      await addr0.sendTransaction({ to: src7641Address, value: ethers.parseEther("100") });
      await network.provider.send("hardhat_mine", ["0x400"]);
      await src7641.snapshot();
      await addr0.sendTransaction({ to: src7641Address, value: ethers.parseEther("100") });
      ...
      await network.provider.send("hardhat_mine", ["0x400"]);
      await src7641.snapshot();
      expect(await src7641.claimableRevenue(addr0, 2)).to.equal(ethers.parseEther("200")*BigInt(percentClaimable)/BigInt(100));
      const balanceBefore = await ethers.provider.getBalance(await ethers.provider.getSigner(0));
