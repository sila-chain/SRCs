...
/** 
* @dev Contract instance for the EPS Genesis SRC-1155. This contract provides two token
...
*/ 
contract EPSGenesis is SRC1155, Ownable, SRC1155Burnable, SRC1155Supply, Proxiable {

/** 
* @dev Not required, but provided to give consistency with SRC-721 in terms of display
* in tools like silascan:
*/     
  ...
  */ 
  function proxyMintGated(address _receivedAddress) internal hasNotAlreadyMintedGated(_receivedAddress) isProxyAddress(_receivedAddress) supplyNotExhaustedGated() {
    ...
    require((ISRC721(GATE_1).balanceOf(nominator) >= 1 || ISRC721(GATE_2).balanceOf(nominator) >= 1 || ISRC721(GATE_3).balanceOf(nominator) >= 1), "Must hold an eligible token for this mint");
  ...
  */ 
  ...
  function hasNominatorWithEligibleToken(address _receivedAddress) public view returns (bool) {
    ...
    bool isProxied;
    (nominator, delivery, isProxied) = getAddresses(_receivedAddress);
    return((ISRC721(GATE_1).balanceOf(nominator) >= 1 || ISRC721(GATE_2).balanceOf(nominator) >= 1 || ISRC721(GATE_3).balanceOf(nominator) >= 1));
  }
  ...
  /** 
  * @dev name, max and total supply for tools like silascan:
  */ 
  ...
  function _beforeTokenTransfer(address operator, address from, address to, uint256[] memory ids, uint256[] memory amounts, bytes memory data)
      internal
      override(SRC1155, SRC1155Supply)
  {
