...
/**
 * @notice Reference implementation of the src-5516 interface.
...
contract SRC5516 is Context, SRC165, ISRC5516 {
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view virtual override(SRC165) returns (bool) {
        return
            interfaceId == type(ISRC5516).interfaceId ||
            super.supportsInterface(interfaceId);
    }

    /**
     * @dev See {ISRC5516-issue}.
     */
    function issue(
        address[] calldata recipients,
        string calldata metadataURI
    ) external virtual override returns (uint256 tokenId) {
        require(recipients.length > 0, "SRC5516: Empty recipients list");
        require(bytes(metadataURI).length > 0, "SRC5516: Empty metadataURI");
        ...
        for (uint256 i = 0; i < recipients.length; ) {
            ...
            require(
                recipient != address(0),
                "SRC5516: Transfer to address zero"
            );
            require(
                !_holdings[recipient][tokenId],
                "SRC5516: Token already owned"
            );
            require(
                !_renounced[tokenId][recipient],
                "SRC5516: Recipient renounced this token"
            );
    ...
    function uri(
        ...
        require(
            ...
            "SRC5516: Token does not exist"
        );
        return string(abi.encodePacked(_uri, _tokenURIs[tokenId]));
    }

    /**
     * @dev See {ISRC5516-deriveTokenId}.
     */
