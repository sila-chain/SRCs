...
contract SRC7092 is ISRC7092, BondStorage {
    constructor(
    ...
    function _issue(IssueData[] memory _issueData, Bond memory _bondInfo) internal virtual {
        ...
        for(uint256 i; i < _issueData.length; i++) {
            ...
            require(investor != address(0), "SRC7092: ZERO_ADDRESS_INVESTOR");
            require(principal != 0 && (principal * _denomination) % _denomination == 0, "SRC: INVALID_PRINCIPAL_AMOUNT");
        ...
        require(_maturityDate > block.timestamp, "SRC7092: INVALID_MATURITY_DATE");
        require(volume == _issueVolume, "SRC7092: INVALID_ISSUE_VOLUME");
    ...
    function _redeem(IssueData[] memory _bondsData) internal virtual {
        uint256 _maturityDate = _bonds[bondISIN].maturityDate;
        require(block.timestamp > _maturityDate, "SRC2721: WAIT_MATURITY");
    ...
    function _approve(address _owner, address _spender, uint256 _amount) internal virtual {
        require(_owner != address(0), "SRC7092: OWNER_ZERO_ADDRESS");
        require(_spender != address(0), "SRC7092: SPENDER_ZERO_ADDRESS");
        require(_amount > 0, "SRC7092: INVALID_AMOUNT");
        require(block.timestamp < _bonds[bondISIN].maturityDate, "SRC7092: BONDS_MATURED");

        uint256 _balance = _principals[_owner] / _bonds[bondISIN].denomination;
        uint256 _denomination = _bonds[bondISIN].denomination;

        require(_amount <= _balance, "SRC7092: INSUFFICIENT_BALANCE");
        require((_amount * _denomination) % _denomination == 0, "SRC7092: INVALID_AMOUNT");
    ...
    function _decreaseAllowance(address _owner, address _spender, uint256 _amount) internal virtual {
        ...
        require(block.timestamp < _bonds[bondISIN].maturityDate, "SRC7092: BONDS_MATURED");
        require(_amount <= _allowance, "SRC7092: NOT_ENOUGH_APPROVAL");
        require((_amount * _denomination) % _denomination == 0, "SRC7092: INVALID_AMOUNT");
    ...
    function _transfer(
        ...
        require(_from != address(0), "SRC7092: OWNER_ZERO_ADDRESS");
        require(_to != address(0), "SRC7092: SPENDER_ZERO_ADDRESS");
        require(_amount > 0, "SRC7092: INVALID_AMOUNT");
    ...
    function _spendAllowance(address _from, address _spender, uint256 _amount) internal virtual {
        ...
        require(_amount <= currentAllowance, "SRC7092: INSUFFICIENT_ALLOWANCE");
    ...
    function _batchApprove(address _owner, address[] calldata _spender, uint256[] calldata _amount) internal virtual  {
        ...
        for(uint256 i = 0; i < _spender.length; i++) {
            require(_spender[i] != address(0), "SRC7092: SPENDER_ZERO_ADDRESS");
            require(_amount[i] > 0, "SRC7092: INVALID_AMOUNT");
            require(_balance >= _amount[i], "SRC7092: INSUFFICIENT_BALANCE");
            require((_amount[i] * _denomination) % _denomination == 0, "SRC7092: INVALID_AMOUNT");
    ...
    function _decreaseAllowance(address _owner, address[] calldata _spender, uint256[] calldata _amount) internal virtual {
        ...
        for(uint256 i = 0; i < _spender.length; i++) {
            ...
            uint256 _approval = _allowed[_owner][_spender[i]];
            require(_amount[i] <= _approval, "SRC7092: NOT_ENOUGH_APPROVAL");
    ...
    function _batchTransfer(address[] memory _from, address[] calldata _to, uint256[] calldata _amount, bytes[] calldata _data) internal virtual {
        ...
        for(uint256 i = 0; i < _from.length; i++) {
            require(_from[i] != address(0), "SRC7092: OWNER_ZERO_ADDRESS");
            require(_to[i] != address(0), "SRC7092: SPENDER_ZERO_ADDRESS");
            require(_amount[i] > 0, "SRC7092: INVALID_AMOUNT");
    
            uint256 principal = _principals[_from[i]];
            uint256 _balance = principal / _denomination;

            require(_amount[i] <= _balance, "SRC7092: INSUFFICIENT_BALANCE");
            require((_amount[i] * _denomination) % _denomination == 0, "SRC7092: INVALID_AMOUNT");
    ...
    function _batchSpendAllowance(address[] calldata _from, address _spender, uint256[] calldata _amount) internal virtual {
        for(uint256 i = 0; i < _from.length; i++) {
            ...
            require(_amount[i] <= currentAllowance, "SRC7092: INSUFFICIENT_ALLOWANCE");
