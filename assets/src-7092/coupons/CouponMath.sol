...
library CouponMath {
    ...
    */
    function annualInterest(
        ...
        uint256 couponRate = ISRC7092(_bondContract).couponRate();
        uint256 principal = ISRC7092(_bondContract).principalOf(_investor);
    ...
    */
    function interest(
        ...
        uint256 couponRate = ISRC7092(_bondContract).couponRate();
        uint256 principal = ISRC7092(_bondContract).principalOf(_investor);
        uint256 frequency = ISRC7092(_bondContract).couponFrequency();
    ...
    */
    function _numberOfDays(address _bondContract) private view returns(uint256) {
        uint256 dayCountBasis = ISRC7092(_bondContract).dayCountBasis();
