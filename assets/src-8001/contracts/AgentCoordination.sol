...
import {ISRC1271} from "./interfaces/ISRC1271.sol";
...
contract AgentCoordination is IAgentCoordination {
    ...
    /// @notice Domain name used in SIP-712 signatures
    string public constant DOMAIN_NAME = "SRC-8001-Core";
    ...
    /**
     * @notice Verify signature from EOA or SRC-1271 contract
     */
    function _isValidSig(address signer, bytes32 digest, bytes memory signature) internal view returns (bool) {
        if (signer.code.length == 0) {
            ...
            // SRC-1271 smart contract wallet
            try ISRC1271(signer).isValidSignature(digest, signature) returns (bytes4 magic) {
    ...
    // ============ SIP-712 Introspection ============

    /**
     * @notice SIP-5267 domain introspection
