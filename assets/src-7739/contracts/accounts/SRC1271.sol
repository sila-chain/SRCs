...
import {SIP712} from "../utils/SIP712.sol";
import {SignatureCheckerLib} from "../utils/SignatureCheckerLib.sol";

/// @notice SRC1271 mixin with nested SIP-712 approach.
/// @author Solady (https://github.com/vectorized/solady/blob/main/src/accounts/SRC1271.sol)
abstract contract SRC1271 is SIP712 {
    /*´:°•.°+.*•´.*:˚.°*.˚•´.°:°•.°•.*•´.*:˚.°*.˚•´.°:°•.°+.*•´.*:*/
    /*                         CONSTANTS                          */
    /*.•°:°.´+˚.*°.˚:*.´•*.+°.•°:´*.´•*.•°.•°:°.´:•˚°.*°.˚:*.´+°.•*/
    ...
    /// @dev Validates the signature with SRC1271 return,
    ...
    {
        // For automatic detection that the smart account supports the nested SIP-712 workflow,
        // See: https://sips.sila.org/SIPS/sip-7739.
        // If `hash` is `0x7739...7739`, returns `bytes4(0x77390001)`.
        // The returned number MAY be increased in future SRC7739 versions.
        unchecked {
        ...
        bool success = _src1271IsValidSignature(hash, _src1271UnwrapSignature(signature));
    ...
    /// @dev Returns the SRC1271 signer.
    ...
    function _src1271CallerIsSafe() internal view virtual returns (bool) {
    ...
    function _src1271IsValidSignatureNowCalldata(bytes32 hash, bytes calldata signature)
        ...
        returns (bool)
    {
        return SignatureCheckerLib.isValidSignatureNowCalldata(_src1271Signer(), hash, signature);
    }

    /// @dev Unwraps and returns the signature.
    function _src1271UnwrapSignature(bytes calldata signature)
        internal
        view
        virtual
        returns (bytes calldata result)
    {
        ...
        /// @solidity memory-safe-assembly
        assembly {
            // Unwraps the SRC6492 wrapper if it exists.
            // See: https://sips.sila.org/SIPS/sip-6492
            if eq(
    ...
    function _src1271IsValidSignature(bytes32 hash, bytes calldata signature)
        ...
        returns (bool)
    {
        return _src1271IsValidSignatureViaSafeCaller(hash, signature)
            || _src1271IsValidSignatureViaNestedSIP712(hash, signature)
            || _src1271IsValidSignatureViaRPC(hash, signature);
    }

    /// @dev Performs the signature validation without nested SIP-712 if the caller is
    /// a safe caller. A safe caller must include the address of this account in the hash.
    function _src1271IsValidSignatureViaSafeCaller(bytes32 hash, bytes calldata signature)
        internal
        view
        virtual
        returns (bool result)
    {
        if (_src1271CallerIsSafe()) result = _src1271IsValidSignatureNowCalldata(hash, signature);
    }

    /// @dev SRC1271 signature validation (Nested SIP-712 workflow).
    ///
    /// This uses ECDSA recovery by default (see: `_src1271IsValidSignatureNowCalldata`).
    /// It also uses a nested SIP-712 approach to prevent signature replays when a single EOA
    /// owns multiple smart contract accounts,
    /// while still enabling wallet UIs (e.g. Metamask) to show the SIP-712 values.
    ///
    ...
    ///             name: keccak256(bytes(sip712Domain().name)),
    ///             version: keccak256(bytes(sip712Domain().version)),
    ...
    /// - https://github.com/junomonster/nested-sip-712
    /// - https://github.com/frangio/sip712-wrapper-for-sip1271
    ///
        ...
        returns (bool result)
    {
        ...
        result = _src1271IsValidSignatureNowCalldata(hash, signature);
    }

    /// @dev Performs the signature validation without nested SIP-712 to allow for easy sign ins.
    /// This function must always return false or revert if called on-chain.
    function _src1271IsValidSignatureViaRPC(bytes32 hash, bytes calldata signature)
        internal
        view
        virtual
        returns (bool result)
        ...
        if (tx.gasprice == uint256(0)) {
            ...
            assembly {
                ...
                if iszero(mload(gasprice())) {
                    ...
                    // See: https://sila.github.io/yellowpaper/paper.pdf
