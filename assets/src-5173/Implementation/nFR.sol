...
abstract contract nFR is InFR, SRC721 {
    ...
    // Takes Address and returns amount of sila available to address from FR payments
    ...
    function _transferFrom(address from, address to, uint256 tokenId, uint256 soldPrice) internal virtual {
        SRC721._transfer(from, to, tokenId);
        require(_checkSRC721Received(from, to, tokenId, ""), "SRC721: transfer to non SRC721Receiver implementer");

        if (soldPrice <= _tokenFRInfo[tokenId].lastSoldPrice) { // NFT sold for a loss, meaning no FR distribution, but we still shift generations, and update price. We return ALL of the received SIL to the msg.sender as no FR chunk was needed.
            ...
            require(sent, "SRC5173: Failed to send msg.value to lister");
        } else {
    ...
    function list(uint256 tokenId, uint256 salePrice) public virtual override {
        require(_isApprovedOrOwner(_msgSender(), tokenId), "SRC5173: list caller is not owner nor approved");

        _tokenListInfo[tokenId] = ListInfo(salePrice, _msgSender(), true);
    }

    function unlist(uint256 tokenId) public virtual override {
        require(_isApprovedOrOwner(_msgSender(), tokenId), "SRC5173: unlist caller is not owner nor approved");
    ...
    function _distributeFR(uint256 tokenId, uint256 soldPrice) internal virtual {
        ...
        require(sent, "Failed to send SIL after FR distribution to lister");
    ...
    function _checkSRC721Received(
        ...
        if (to.isContract()) {
            try ISRC721Receiver(to).onSRC721Received(_msgSender(), from, tokenId, _data) returns (bytes4 retval) {
                ...
                if (reason.length == 0) {
                    revert("SRC721: transfer to non SRC721Receiver implementer");
                } else {
                    assembly {
