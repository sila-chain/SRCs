...
library Forest {
    ...
    struct DAG {
        ...
        mapping(uint256 => ISRC8047.Token) tokens;
    }

    /**
     * @dev See {ISRC8047.TokenCreated}
    ...
    function _createToken(
        DAG storage self,
        ISRC8047.Token memory newToken,
        address spender
        ...
        self.tokens[newId] = ISRC8047.Token(rootId, newToken.parent, newToken.value, newToken.depth, newToken.owner);
        unchecked {
            self.nonces[spender]++;
        }
     ...
     */
    function getToken(DAG storage self, uint256 id) internal view returns (ISRC8047.Token memory) {
    ...
    function getTokenHierarchy(DAG storage self, uint256 id) internal view returns (uint256) {
        ISRC8047.Token storage ptr = self.tokens[id];
        if (ptr.parent != 0) {
     ...
     */
    function createToken(DAG storage self, ISRC8047.Token memory newToken, address spender) internal returns (uint256) {
    ...
    function spendToken(
        ...
        unchecked {
            ...
            if (to != address(0)) {
                newId = _createToken(self, ISRC8047.Token(currentRoot, id, value, newDepth, to), spender);
    ...
    function defaultMerge(DAG storage self, uint256[] memory ids, address spender) internal returns (uint256 newId) {
        ...
        ISRC8047.Token storage mainPtr = self.tokens[mainId];
        ...
        for (uint256 i = 1; i < ids.length; i++) {
            ...
            ISRC8047.Token storage ptr = self.tokens[currentId];
