...
describe("SRC-8047", function () {
  async function deployTokenFixture() {
    const [owner, alice, bob, charlie, dave, otherAccount] = await ethers.getSigners();
    const src8047 = await ethers.getContractFactory(CONTRACT_NAME.SRC8047);
    const contract = await src8047.deploy(TOKEN_METADATA.URI);
  ...
  describe("SRC1155 Conformance Test", function () {
    it("setApprovalForAll", async function () {
      ...
      await expect(contract.connect(alice).setApprovalForAll(ZeroAddress, true))
        .to.be.revertedWithCustomError(contract, "SRC1155InvalidOperator")
    ...
    it("uri", async function () {
      ...
      // Note: In SRC-8047 using same URI for all token ids.
  ...
  describe("SRC-8047", function () {
    it("tracks DAG depths, roots, parents, ownership, and supply correctly", async function () {
      ...
      expect(await contract.supportsInterface(SRC165InterfaceId)).to.equal(true);
      expect(await contract.supportsInterface(SRC1155InterfaceId)).to.equal(true);
      expect(await contract.supportsInterface(SRC5615InterfaceId)).to.equal(true);
      expect(await contract.supportsInterface(SRC8047InterfaceId)).to.equal(true);
      ...
      await expect(contract.balanceOfBatch([alice.address], [rootTokenId, tokenId]))
        .to.be.revertedWithCustomError(contract, "SRC1155InvalidArrayLength")
        .withArgs(2, 1);
