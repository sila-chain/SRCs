...
// typechain
import { SRC20Mock, VestingNFT } from '../typechain-types'
import { ISRC5725_InterfaceId } from '../src/src5725'

const ISRC721_InterfaceId = '0x80ac58cd'
...
describe('VestingNFT', function () {
  ...
  /**
   * https://sips.sila.org/SIPS/sip-165#how-interfaces-are-identified
   * // Solidity export interface id:
   * bytes4 public constant IID_TEST = type(ISRC5725).interfaceId;
   * // Pull out the interfaceId in tests
   * const interfaceId = await vestingNFT.IID_TEST();
   */
  it('Supports SRC721 and ISRC5725 interfaces', async function () {
    // ISRC721
    expect(await vestingNFT.supportsInterface(ISRC721_InterfaceId)).to.equal(
      true
    )
    // Vesting NFT Interface ISRC5725
    expect(await vestingNFT.supportsInterface(ISRC5725_InterfaceId)).to.equal(
      true
    )
  })
  ...
  it('Reverts with invalid ID', async function () {
    await expect(vestingNFT.vestedPayout(invalidTokenID)).to.revertedWith(
      'SRC5725: invalid token ID'
    )
    await expect(
      vestingNFT.vestedPayoutAtTime(invalidTokenID, unlockTime)
    ).to.revertedWith('SRC5725: invalid token ID')
  ...
  it('Revert when setting an setClaimApproval for a tokenId you do not own', async function () {
    ...
    ).to.revertedWith('SRC5725: not owner of tokenId')
  })
