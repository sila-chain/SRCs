...
import "./ISRC7007Updatable.sol";
import "./IOpmlLib.sol";

/**
 * @dev Implementation of the {ISRC7007} interface.
 */
contract SRC7007Opml is SRC165, ISRC7007Updatable, SRC721URIStorage {
    ...
    function addAigcData(
        ...
        require(ownerOf(tokenId) != address(0), "SRC7007: nonexistent token");
        require(tokenIdToRequestId[tokenId] == 0, "SRC7007: requestId already exists");
    ...
    /**
     * @dev See {ISRC7007Updatable-update}.
     */
    function update(
        bytes calldata prompt,
        bytes calldata aigcData
    ) public virtual override {
        require(verify(prompt, aigcData, prompt), "SRC7007: invalid aigcData"); // proof argument is not used in verify() function for opML, so we can pass prompt as proof
        uint256 tokenId = uint256(keccak256(prompt));
        require(ownerOf(tokenId) != address(0), "SRC7007: nonexistent token");
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view virtual override(SRC165, SRC721, ISRC165) returns (bool) {
        return
