 ...
 * @dev Specifying the role in the ecosystem. Used in conjunction with ISRC6956.Authorization
 */
enum Role {
 ...
 * E7    | Not authorized based on SRC6956Authorization
    ...
    ISRC6956 
{
    ...
    modifier onlyMaintainer() {
        require(isMaintainer(msg.sender), "SRC6956-E1");
        _;
    }

    /**
     * @notice Behaves like SRC721 burn() for wallet-cleaning purposes. Note only the tokenId (as a wrapper) is burned, not the ASSET represented by the ANCHOR.
     * @dev 
    ...
    function burn(uint256 tokenId) public override
    {
        ...
        require(_roleBasedAuthorization(anchor, createAuthorizationMap(burnAuthorization)), "SRC6956-E2");
        _burn(tokenId);
    }
        ...
        authorized(Role.ASSET, createAuthorizationMap(approveAuthorization))
    {
        ...
        (to, anchor, attestationHash) = decodeAttestationIfValid(attestation, data);
        _commitAttestation(to, anchor, attestationHash);
        require(tokenByAnchor[anchor]>0, "SRC6956-E3");
        _approve(to, tokenByAnchor[anchor]);
    }

    // approveAuth == ISSUER does not really make sense.. so no separate implementation, since SRC-721.approve already implies owner...

    function approve(address to, uint256 tokenId) public virtual override(SRC721,ISRC721)
     ...
     *      When derived contracts extend SRC6956 contract, this function may be overridden
     ...
     *      implementation is fully SRC-6956 compatible 
     */
    ...
    {
        require(batchSize == 1, "SRC6956-E4");
        bytes32 anchor = anchorByToken[tokenId];
        ...
        if(to == address(0)) {
            ...
            require(_anchorIsReleased[anchor], "SRC6956-E5");
        }
    ...
    modifier authorized(Role _role, uint256 _authMap) {
        require(hasAuthorization(_role, _authMap), "SRC6956-E7");
        _;
    }

    // The following functions are overrides required by Solidity, SIP-165.
    ...
    {
        return
            interfaceId == type(ISRC6956).interfaceId ||
    ...
    function decodeAttestationIfValid(bytes memory attestation, bytes memory data) public view returns (address to, bytes32 anchor, bytes32 attestationHash) {
        ...
        require(isTrustedOracle(signer), "SRC6956-E8");
        require(_anchorByUsedAttestation[attestationHash] <= 0, "SRC6956-E9");

        // Check expiry
        uint256 timestamp = block.timestamp;
        require(timestamp > validStartTime, "SRC6956-E10");
        require(attestationTime + maxAttestationExpireTime > block.timestamp, "SRC6956-E11");
        require(validEndTime > block.timestamp, "SRC6956-E112");
    ...
    /// @notice Compatible with SRC721.tokenURI(). Returns {baseURI}{anchor}
    ...
    function _baseURI() internal view virtual override(SRC721) returns (string memory) {
        return _baseUri;
    }
   ...
   function _extractSigner(bytes32 messageHash, bytes memory sig) internal pure returns (address) {
        require(sig.length == 65, "SRC6956-E13");
        /*
        Signature is produced by signing a keccak256 hash with the following format:
        "\x19Sila Signed Message\n" + len(msg) + msg
        */
        bytes32 silSignedMessageHash = keccak256(abi.encodePacked("\x19Sila Signed Message:\n32", messageHash));
        ...
        address signer = ecrecover(silSignedMessageHash, v, r, s);       
        return signer;
    }
}
