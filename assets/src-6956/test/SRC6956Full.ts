...
import { SRC6956Authorization, merkleTestAnchors, NULLADDR, AttestedTransferLimitUpdatePolicy, invalidAnchor, createAttestationWithData} from "./commons";
...
describe("SRC6956: Asset-Bound NFT --- Full", function () {
  ...
  describe("Deployment & Settings", function () {
    it("Should implement SIP-165 support the SIP-6956 interface", async function () {
      ...
      expect(await abnftContract.supportsInterface(ISRC6956FloatableInterfaceId)).to.equal(true);
      expect(await abnftContract.supportsInterface(ISRC6956ValidAnchorsInterfaceId)).to.equal(true);
      expect(await abnftContract.supportsInterface(ISRC6956AttestationLimitedInterfaceId)).to.equal(true);
    });
  });
...
describe("Anchor-Floating", function () {
  it("SHOULD only allow maintainer to specify canStartFloating and canStopFloating", async function () {
    ...
    await expect(abnftContract.connect(mallory).updateFloatingAuthorization(SRC6956Authorization.ALL, SRC6956Authorization.ALL))
    .to.revertedWith("SRC6956-E1");

    await expect(abnftContract.connect(maintainer).updateFloatingAuthorization(SRC6956Authorization.OWNER_AND_ASSET, SRC6956Authorization.ISSUER))
    .to.emit(abnftContract, "FloatingAuthorizationChange")
    .withArgs(SRC6956Authorization.OWNER_AND_ASSET, SRC6956Authorization.ISSUER, maintainer.address);
  });
  ...
  it("SHOULD allow owner to float token only when OWNER is allowed", async function () {
    ...
    await expect(abnftContract.connect(maintainer).updateFloatingAuthorization(SRC6956Authorization.ASSET_AND_ISSUER, SRC6956Authorization.ASSET_AND_ISSUER))
    .to.emit(abnftContract, "FloatingAuthorizationChange")
    .withArgs(SRC6956Authorization.ASSET_AND_ISSUER, SRC6956Authorization.ASSET_AND_ISSUER, maintainer.address);

    await expect(abnftContract.connect(alice).float(anchor, FloatState.Floating))
    .to.revertedWith("SRC6956-E21")

    await expect(abnftContract.connect(maintainer).updateFloatingAuthorization(SRC6956Authorization.OWNER_AND_ASSET, SRC6956Authorization.OWNER_AND_ASSET))
    .to.emit(abnftContract, "FloatingAuthorizationChange")
    .withArgs(SRC6956Authorization.OWNER_AND_ASSET, SRC6956Authorization.OWNER_AND_ASSET, maintainer.address);
  ...
  it("SHOULDN'T allow owner to transfer token when explicitly marked anchored", async function () {
    ...
    await expect(abnftContract.connect(maintainer).updateFloatingAuthorization(SRC6956Authorization.ISSUER, SRC6956Authorization.ISSUER))
    .to.emit(abnftContract, "FloatingAuthorizationChange")
    .withArgs(SRC6956Authorization.ISSUER, SRC6956Authorization.ISSUER, maintainer.address);
  ...
  it("SHOULD allow maintainer to float HIS OWN token when OWNER is allowed", async function () {
    ...
    await expect(abnftContract.connect(maintainer).updateFloatingAuthorization(SRC6956Authorization.OWNER, SRC6956Authorization.OWNER))
    .to.emit(abnftContract, "FloatingAuthorizationChange")
    .withArgs(SRC6956Authorization.OWNER, SRC6956Authorization.OWNER, maintainer.address);
...
describe("Attested Transfer Limits", function () {
  ...
  it("SHOULD allow maintainer to update global attestation limit", async function () {
    ...
    .to.emit(abnftContract, "GlobalAttestationLimitUpdate") // Standard SRC721 event
  ...
  it("Should allow maintainer to update anchor-based attestation limit w/o changing global limits", async function () {
    ...
    .to.emit(abnftContract, "AttestationLimitUpdate") // Standard SRC721 event
