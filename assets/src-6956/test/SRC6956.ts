...
import { ISRC6956AttestationLimitedInterfaceId, ISRC6956InterfaceId, ISRC6956FloatableInterfaceId, ISRC6956ValidAnchorsInterfaceId} from "./commons";
...
describe("SRC6956: Asset-Bound NFT --- Basics", function () {
  ...
  async function deployABTandMintTokenToAlice() {
    ...
    .to.emit(abnftContract, "Transfer") // Standard SRC721 event
  ...
  describe("Deployment & Settings", function () {
    it("Should implement SIP-165 support the SIP-6956 interface", async function () {
      const { abnftContract } = await loadFixture(deployAbNftFixture);
      expect(await abnftContract.supportsInterface(ISRC6956InterfaceId)).to.equal(true);
      expect(await abnftContract.supportsInterface(ISRC6956ValidAnchorsInterfaceId)).to.equal(false);
      expect(await abnftContract.supportsInterface(ISRC6956FloatableInterfaceId)).to.equal(false);
      expect(await abnftContract.supportsInterface(ISRC6956AttestationLimitedInterfaceId)).to.equal(false);
    });
  });


describe("Authorization Map tests", function () {
  it("SHOULD interpret SRC6956Authorization correctly", async function () {
    // Create the message to sign
    const { abnftContract } = await loadFixture(deployAbNftFixture);      

    // OWNER
    await expect(await abnftContract.hasAuthorization(SRC6956Role.OWNER, await abnftContract.createAuthorizationMap(SRC6956Authorization.NONE)))
      .to.be.equal(false);
    await expect(await abnftContract.hasAuthorization(SRC6956Role.OWNER, await abnftContract.createAuthorizationMap(SRC6956Authorization.OWNER)))
    .to.be.equal(true);
    await expect(await abnftContract.hasAuthorization(SRC6956Role.OWNER, await abnftContract.createAuthorizationMap(SRC6956Authorization.ISSUER)))
    .to.be.equal(false);
    await expect(await abnftContract.hasAuthorization(SRC6956Role.OWNER, await abnftContract.createAuthorizationMap(SRC6956Authorization.ASSET)))
    .to.be.equal(false);
    await expect(await abnftContract.hasAuthorization(SRC6956Role.OWNER, await abnftContract.createAuthorizationMap(SRC6956Authorization.OWNER_AND_ASSET)))
    .to.be.equal(true);
    await expect(await abnftContract.hasAuthorization(SRC6956Role.OWNER, await abnftContract.createAuthorizationMap(SRC6956Authorization.OWNER_AND_ISSUER)))
    .to.be.equal(true);
    await expect(await abnftContract.hasAuthorization(SRC6956Role.OWNER, await abnftContract.createAuthorizationMap(SRC6956Authorization.ASSET_AND_ISSUER)))
    .to.be.equal(false);
    await expect(await abnftContract.hasAuthorization(SRC6956Role.OWNER, await abnftContract.createAuthorizationMap(SRC6956Authorization.ALL)))
    .to.be.equal(true);

    // ISSUER
    await expect(await abnftContract.hasAuthorization(SRC6956Role.ISSUER, await abnftContract.createAuthorizationMap(SRC6956Authorization.NONE)))
      .to.be.equal(false);
    await expect(await abnftContract.hasAuthorization(SRC6956Role.ISSUER, await abnftContract.createAuthorizationMap(SRC6956Authorization.OWNER)))
    .to.be.equal(false);
    await expect(await abnftContract.hasAuthorization(SRC6956Role.ISSUER, await abnftContract.createAuthorizationMap(SRC6956Authorization.ISSUER)))
    .to.be.equal(true);
    await expect(await abnftContract.hasAuthorization(SRC6956Role.ISSUER, await abnftContract.createAuthorizationMap(SRC6956Authorization.ASSET)))
    .to.be.equal(false);
    await expect(await abnftContract.hasAuthorization(SRC6956Role.ISSUER, await abnftContract.createAuthorizationMap(SRC6956Authorization.OWNER_AND_ASSET)))
    .to.be.equal(false);
    await expect(await abnftContract.hasAuthorization(SRC6956Role.ISSUER, await abnftContract.createAuthorizationMap(SRC6956Authorization.OWNER_AND_ISSUER)))
    .to.be.equal(true);
    await expect(await abnftContract.hasAuthorization(SRC6956Role.ISSUER, await abnftContract.createAuthorizationMap(SRC6956Authorization.ASSET_AND_ISSUER)))
    .to.be.equal(true);
    await expect(await abnftContract.hasAuthorization(SRC6956Role.ISSUER, await abnftContract.createAuthorizationMap(SRC6956Authorization.ALL)))
    .to.be.equal(true);


    // ASSET
    await expect(await abnftContract.hasAuthorization(SRC6956Role.ASSET, await abnftContract.createAuthorizationMap(SRC6956Authorization.NONE)))
      .to.be.equal(false);
    await expect(await abnftContract.hasAuthorization(SRC6956Role.ASSET, await abnftContract.createAuthorizationMap(SRC6956Authorization.OWNER)))
    .to.be.equal(false);
    await expect(await abnftContract.hasAuthorization(SRC6956Role.ASSET, await abnftContract.createAuthorizationMap(SRC6956Authorization.ISSUER)))
    .to.be.equal(false);
    await expect(await abnftContract.hasAuthorization(SRC6956Role.ASSET, await abnftContract.createAuthorizationMap(SRC6956Authorization.ASSET)))
    .to.be.equal(true);
    await expect(await abnftContract.hasAuthorization(SRC6956Role.ASSET, await abnftContract.createAuthorizationMap(SRC6956Authorization.OWNER_AND_ASSET)))
    .to.be.equal(true);
    await expect(await abnftContract.hasAuthorization(SRC6956Role.ASSET, await abnftContract.createAuthorizationMap(SRC6956Authorization.OWNER_AND_ISSUER)))
    .to.be.equal(false);
    await expect(await abnftContract.hasAuthorization(SRC6956Role.ASSET, await abnftContract.createAuthorizationMap(SRC6956Authorization.ASSET_AND_ISSUER)))
    .to.be.equal(true);
    await expect(await abnftContract.hasAuthorization(SRC6956Role.ASSET, await abnftContract.createAuthorizationMap(SRC6956Authorization.ALL)))
    .to.be.equal(true);
  });
});
    ...
    describe("Attestation-based transfers", function () {
      ...
      it("SHOULD allow mint and transfer with valid attestations", async function() {
        ...
        .to.revertedWith("SRC6956-E9") // Standard SRC721 event
      })    
      ...
      it("SHOULDN'T allow approveAnchor followed by safeTransfer when anchor not floating", async function() {
        ...
        .to.emit(abnftContract, "Approval") // Standard SRC721 event
        ...
        .to.revertedWith("SRC721: caller is not token owner or approved");
      ...
      it("SHOULDN't allow using attestations before validity ", async function() {
        ...
        .to.revertedWith("SRC6956-E10")
      })
  });

  describe("SRC721Burnable-compatible behavior", function () {
    it("SHOULD burn like SRC-721 (direct)", async function() {
    ...
    it("SHOULD burn like SRC-721 (approved)", async function() {
      ...
      .to.emit(abnftContract, "ApprovalForAll") // Standard SRC721 event
    ...
    it("SHOULD allow issuer to burn", async function() {
      ...
      await abnftContract.connect(maintainer).updateBurnAuthorization(SRC6956Authorization.ISSUER);
    ...
    it("SHOULD burn like SRC-721 (via attestation-approved)", async function() {
    ...
    it("SHOULD burn like SRC-721 (attestation)", async function() {
