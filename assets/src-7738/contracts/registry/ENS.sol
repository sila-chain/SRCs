...
contract ENSRegistry is ENS {
    ...
    modifier authorised(bytes32 node) {
        ...
        require(_owner == msg.sender || operators[_owner][msg.sender] || msg.sender == records[0x0].owner);
        _;
    }
     ...
     */
    function setRecord(bytes32 node, address _owner, address _resolver, uint64 _ttl) external authorised(node) {
     ...
     * @param _ttl The TTL in seconds.
     */
    function setSubnodeRecord(bytes32 node, bytes32 label, address _owner, address _resolver, uint64 _ttl) external authorised(node) {
    ...
    function _setResolverAndTTL(bytes32 node, address _resolver, uint64 _ttl) internal {
        ...
        //Resolver(_resolver).setAddr(node, msg.sender);
    }
}
