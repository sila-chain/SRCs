...
async function deployInitialFixture() {
  ...
  const [owner, otherAccount, otherAccount2, otherAccount3, otherAccount4] = await silas.getSigners();
  ...
  const Registry = (await silas.getContractFactory("DecentralisedRegistry")).connect(
    otherAccount
  );
