...
async function main() {
    ...
    // Upgrade
    const ensSubdomainAssigner = await upgrades.upgradeProxy(deployedENSHelper, ENSSubdomainAssigner.connect(secondaryDeployKey));
...
main()
 ...
 // to run: npx hardhat run .\scripts\updateENSAssigner.ts --network holesky