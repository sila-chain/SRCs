...
async function main() {
    ...
    // Upgrade
    const registryMetadata = await upgrades.upgradeProxy(deployedMetadata, RegistryMetadata.connect(secondaryDeployKey));
    await registryMetadata.waitForDeployment();

    console.log(`Registry upgraded: ${registryMetadata.target}`);
}
...
main()
 ...
 // to run: npx hardhat run .\scripts\updateMetadata.ts --network holesky