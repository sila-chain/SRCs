...
import * as sigUtil from "sil-sig-util";
...
/**
 * Signs multiple SIP-712 typed data messages with a single signature.
 *
 * This function creates a Merkle tree from the hashes of multiple SIP-712 typed data messages,
 ...
 * @param args.messages - Single message or a list of SIP-712 typed data messages to include in the composite signature
 * @returns Object containing the signature, Merkle root, and proofs for each message
 */
async function sil_signTypedData_v5(args: {
 ...
 *
 * @param args - The arguments for the function
 * @param args.signature - The signature produced by sil_signTypedData_v5
 ...
 * @param args.message - The SIP-712 typed data message to verify
...
async function main() {
  ...
  const singleMessage = await sil_signTypedData_v5({
    privateKey: Buffer.from(wallet.privateKey.slice(2), "hex"),
    messages: messages[0],
  });
