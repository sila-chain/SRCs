...
import {GenericSIP712} from "./GenericSIP712.sol";

abstract contract AbstractToken is IAbstractToken, Ownable, GenericSIP712 {
    event SetSigner(address signer);
    mapping(bytes32 => bool) used;
    address private signer;

    // The type of content signed in the SIP-712 signature
    ...
    constructor(address _signer) GenericSIP712('AbstractToken', '1') {
        _setSigner(_signer);
    }
        ...
        returns (AbstractTokenMessageStatus)
    {
        ...
        // message must include a valid proof (SIP-712 signature)
        // 
    ...
    function messageId(AbstractTokenMessage calldata message)
        ...
        returns (bytes32)
    {
        // Note that the message ID is also the SIP-712 hash of the message struct - the fields must match the contents of TYPE_HASH
        return keccak256(
            abi.encode(
                TYPE_HASH,
                message.chainId,
