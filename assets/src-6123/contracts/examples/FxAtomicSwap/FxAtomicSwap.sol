...
import "./SRC20AtomicSwapSettlementRouter.sol";

/**
 * @notice Interface for exposing an SRC-8100-style XML state template.
 */
 ...
 * @notice Single-use payment-versus-payment exchange of two SRC-20 tokens.
...
contract FxAtomicSwap is ISDCTrade, IXMLRepresentableState {
    ...
    ISRC20RouterToken public immutable baseToken;
    ISRC20RouterToken public immutable quoteToken;
     ...
     * @param baseToken_ SRC-20 token represented by `position`.
     * @param quoteToken_ SRC-20 token represented by `paymentAmount`.
     ...
     */
    constructor(
        ISRC20RouterToken baseToken_,
        ISRC20RouterToken quoteToken_,
        address partyA_,
        address partyB_,
    ...
    function stateXmlTemplate() external pure override returns (string memory) {
        return string.concat(
            ...
            "<Lifecycle>",
            "<TradeId>1</TradeId>",
            "<Phase savmstate:call='phase()(uint8)' savmstate:format='decimal'/>",
            "<Inceptor savmstate:call='inceptor()(address)' savmstate:format='address'/>",
            "</Lifecycle>",
            "<Parties>",
            "<Party role='A' savmstate:call='partyA()(address)' savmstate:format='address'/>",
            "<Party role='B' savmstate:call='partyB()(address)' savmstate:format='address'/>",
            "</Parties>",
            "<Tokens>",
            "<Token role='base' savmstate:call='baseToken()(address)' savmstate:format='address'/>",
            "<Token role='quote' savmstate:call='quoteToken()(address)' savmstate:format='address'/>",
            "</Tokens>",
            "<SettlementRouter savmstate:call='settlementRouter()(address)' savmstate:format='address'/>",
            "<Terms view='inceptor'>",
            "<Position token='base' unit='smallest-token-unit' ",
            "savmstate:call='position()(int256)' savmstate:format='decimal'/>",
            "<PaymentAmount token='quote' unit='smallest-token-unit' ",
            "savmstate:call='paymentAmount()(int256)' savmstate:format='decimal'/>",
            "<TradeData savmstate:call='tradeData()(string)' savmstate:format='string'/>",
            "<InitialSettlementData savmstate:call='initialSettlementData()(string)' ",
