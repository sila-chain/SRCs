...
import {IEvalSIP712Buffer, SeaPortMock} from "src/SeaPort/SeaPortMock.sol";
...
contract SeaPort712ParserTest is Test, OrderGenerator {
    ...
    function testEvalSIP712BufferSeaport() public view {
        OrderComponents memory order = generateOrder();
        bytes memory encodedOrder = abi.encode(order);
        string[] memory translatedSig = seaPortMock.evalSIP712Buffer(seaPortMock.DOMAIN_SEPARATOR(), "OrderComponents", encodedOrder);
