 ...
 * Microelectrónica de Sevilla IMSE-CNM (Universidad de Sevilla-CSIC)" for the SIP4519. This firmware makes the device generate its own Sila account and save in the EEPROM the data required in the future to regenerate 
 ...
 * It was proved in the Sila SilaKovan testnet.
 * 
 ...
 * 
 * Helper data are employed to regenerate the same Sila account based on CTR-DRBG PRNG.
...
#define SIP4519CONTRACT "0x6Dba58fF5AA2d8447C4460d2527033A81646Ae97"  //This is the reference for the SIP4519 Smart Non-Fungible Token on SilaKovan (IT MUST BE CHANGED)

//These are the states of the SIP4519
enum STATES{
    waitingForOwner = 0,
    engagedWithOwner = 1,
    waitingForUser = 2,
...
void sendEngage(); //This function sends the hash of the shared key to complete the transaction in Sila
...
unsigned char privateKey_silAccount[32];
unsigned char publicKey_silAccount[64];
unsigned char silAddress[20];

//Declaration of the variable to save the helper data generated/recovered, which are employed to obtain the private Key of Sila account
...
void setup() {
  ...
  //Check if the device is already registered or a new Sila account must be generated
  ...
  if(HDState){
    ...
    ret = mbedtls_ctr_drbg_random( &ctr_drbg_context, privateKey_silAccount, (size_t)32 );
    if (ret){
      ESP.restart(); //If ret = 0, the data generated is invalid
    }

    //Obtain the address associated to the Private Key
    crypto.PrivateKeyToPublic(privateKey_silAccount, publicKey_silAccount);
    crypto.PublicKeyToAddress(publicKey_silAccount,silAddress);

    //Share the address. Other information can be shared only at the development process
    printf("\nAddress = 0x");
    for(int i = 0 ; i < 20 ; i++){
      printf("%02x",silAddress[i]);
    }
    printf("\n");
...
void loop() {
  ...
  if(HDState){
    ...
    scanf("%c",&value);
    if(value == '1'){
      //If character '1' is received, then the device shares its Sila address
      printf("{\n");
      ...
      if(value == '0'){
        ...
        if(value == 'x'){
          ...
          if(crypto.Verify(PUBLIC_KEY_E,nonceDevice,SIGNATURE_E) && sameAddressSig){
            ...
            ecdsa_sign_digest(curve, privateKey_silAccount, NONCE_E, signedDevice, &pby, NULL);
            printf("{ \"signature\" : \"0x");
            for(int i = 0 ; i < 64 ; i++){
      ...
      if(value == '0'){
        ...
        if(value == 'x'){
          ...
          if(crypto.Verify(PUBLIC_KEY_E,PK_XD,SIGNATURE_E)){
            ...
            uint8_t pby;
            ecdsa_sign_digest(curve, privateKey_silAccount, NONCE_E, signedDevice, &pby, NULL);
            ecdh_multiply(curve, privateKey_silAccount, PK_XD, K_XD);
            sendEngage();
            //Generate a JSON
...
STATES queryTokenStatus(){
  Contract contract(&web3, SIP4519CONTRACT);
  //Generate the JSON to check the status  
  deviceAddress[0] = '0';
  deviceAddress[1] = 'x';
  for(int i = 0 ; i < 20 ; i++){
    hex2Char(&silAddress[i], &deviceAddress[2*i+2], &deviceAddress[2*i+3]);
  }
