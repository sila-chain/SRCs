...
const TOKEN_NAME1 = "SRC_5521_NAME1";
const TOKEN_SYMBOL1 = "SRC_5521_SYMBOL1";
const TOKEN_NAME2 = "SRC_5521_NAME2";
const TOKEN_SYMBOL2 = "SRC_5521_SYMBOL2";
...
describe("SRC_5521 - single token contract scenario", function () {
    let tokenContract1;
    ...
    it("must not mint two tokens with the same token id", async function () {
        await tokenContract1.safeMint(1, [], []);
        await expect(tokenContract1.safeMint(1, [], [])).to.be.revertedWith("SRC721: token already minted");
    })
    ...
    it("should revert when attempting to create an invalid relationship", async function () {
        ...
        await expect(tokenContract1.safeMint(2, [tokenContract1.address], [[1, 2, 3]])).to.be.revertedWith("SRC_5521: self-reference not allowed");
...
describe("SRC_5521 - multi token contracts scenario", function () {
    let tokenContract1;
    let tokenContract2;
    ...
    it("can mint a token referring to another minted token", async function () {
        ...
        // relationships of token 2 can be queried using any SRC5521 contract, not necessarily the contract that minted token 2
