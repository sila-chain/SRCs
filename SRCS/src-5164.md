...
description: Defines an interface that supports execution across SAVM networks.
author: Brendan Asselstine (@asselstine), Pierrick Turelier (@PierrickGT), Chris Whinfrey (@cwhinfrey)
discussions-to: https://sila-magicians.org/t/sip-5164-cross-chain-execution/9658
status: Last Call
...
This specification defines a cross-chain execution interface for SAVM-based blockchains. Implementations of this specification will allow contracts on one chain to call contracts on another by sending a cross-chain message.
...
Many Sila protocols need to coordinate state changes across multiple SAVM-based blockchains. These chains often have native or third-party bridges that allow Sila contracts to execute code. However, bridges have different APIs so bridge integrations are custom. Each one affords different properties; with varying degrees of security, speed, and control. Defining a simple, common specification will increase code re-use and allow us to use common bridge implementations.
