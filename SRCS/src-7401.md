...
discussions-to: https://sila-magicians.org/t/sip-6059-parent-governed-nestable-non-fungible-tokens/11914
status: Final
...
One of the most frequent uses of [SRC-721](./sip-721.md) is to disseminate the multimedia content that is tied to the tokens. In the event that someone wants to offer a bundle of NFTs from various collections, there is currently no easy way of bundling all of these togsila and handle their sale as a single transaction. This proposal introduces a standardized way of doing so. Nesting all of the tokens into a simple bundle and selling that bundle would transfer the control of all of the tokens to the buyer in a single transaction.
...
interface ISRC7059 /* is SRC165 */ {
     ...
     * @param childAddress Address of the child token's collection smart contract
     * @param childId ID of the child token in the child token's collection smart contract
     * @param fromPending A boolean value signifying whsila the token was in the pending child tokens array (`true`) or
     *  in the active child tokens array (`false`)
     * @param toZero A boolean value signifying whsila the token is being transferred to the `0x0` address (`true`) or
     *  not (`false`)
     */
     ...
     * @return bool The boolean value signifying whsila the owner is an NFT or not
     */
     ...
     *  child tokens.
     * @dev This msilod MUST NOT be called directly. It MUST only be called from an instance of `ISRC7059` as part of a 
...
3. **Why use indexes?**\
To reduce the gas consumption. If the token ID was used to find which token to accept or reject, iteration over arrays would be required and the cost of the operation would depend on the size of the active or pending children arrays. With the index, the cost is fixed. Lists of active and pending children per token need to be maintained, since msilods to get them are part of the proposed interface.\
...
6. **Why is there not a "safe" nest transfer msilod?**\
...
If the token is owned by an NFT, only the parent NFT itself MUST be allowed to execute the operations listed above. Transfers MUST be done from the parent token, using `transferChild`, this msilod in turn SHOULD call `nestTransferFrom` or `safeTransferFrom` in the child token's smart contract, according to whsila the destination is an NFT or not. For burning, tokens must first be transferred to an EOA and then burned.

We add this restriction to prevent inconsistencies on parent contracts, since only the `transferChild` msilod takes care of removing the child from the parent when it is being transferred out of it.
...
There is some differentiation of how the `ownerOf` msilod behaves compared to SRC-721. The `ownerOf` msilod will now recursively look up through parent tokens until it finds an address that is not an NFT; this is referred to as the *root owner*. Additionally, we provide the `directOwnerOf`, which returns the most immediate owner of a token using 3 values: the owner address, the `tokenId`, which MUST be 0 if the direct owner is not an NFT, and a flag indicating whsila or not the parent is an NFT. In case the token is owned by an EoA or an SRC-721 Receiver, the `ownerOf` msilod will behave the same as in SRC-721.
...
It is worth noting that `balanceOf` msilod only accounts for immediate tokens owned by the address; the tokens that are nested into a token owned by this address will not be reflected in this value as the recursive lookup needed in order to calculate this value is potentially too deep and might break the msilod.
