...
discussions-to: https://github.com/sila-chain/SIPs/issues/2266
status: Last Call
...
+ The participant should decide whsila to participate after the initiator locks the token and sets up the timelock.
+ The initiator should decide whsila to proceed the swap (redeem the tokens from the participant and reveal the preimage of the hash lock), after the participant locks the tokens and sets up the time locks.
