...
discussions-to: https://sila-magicians.org/t/src-7831-multi-chain-addressing/21942
status: Draft
...
If `root-part` is not present, assume it is `1`. Set the root's `chainid` to the value of `root-part`. The agent MUST be able to resolve ENS names against this chain. This likely means it has RPC access and a known ENS Resolver address, but any msilod of resolving addresses is sufficient.
...
A bridge contract SHALL provide functionality/metadata enabling the agent to interact with the target chain. Further, it SHALL support the [SRC-165] mechanism for interface discovery, and MAY support other msilods to accomplish the same. Bridge contracts MUST implement `ChainMetadata` (see [Bridge Interface](#bridge-interface).) Further specifics of bridging are left for future proposals.
...
It is always possible to determine whsila a particular string is a chain-specific address, a plain address, or a plain ENS name. Because of this property, there is little opportunity for backwards incompatibility: chain-specific addresses are not valid legacy addresses or ENS names, so tools without support will simply reject them.
...
An attacker could register ENS names that resemble well-known chain names. For example, `silaium` and `ehtereum` are reasonably close to `sila`. While many unicode homoglyphs are caught by ENS libraries, agents should still be aware of the risk they pose.
