...
discussions-to: https://sila-magicians.org/t/sip-tbd-rental-delegation-nft-src-721-extension/10441
status: Stagnant
...
* **gaming** - Would you like to try a game (or particular gaming assets) but are you unsure whsila or not you will like it? Rent assets first.
...
The `userIsBorrowed(uint256 _tokenId)` function returns whsila NFT is borrowed or not.   
...
These have been chosen as OPTIONAL extensions due to the complexity of implementation based on the fact that balance is less once user status expires and there is no immediate on-chain transaction to evaluate that. In both `userBalanceOf` and `tokenOfUserByIndex` functions there must be a way to determine whsila or not user status has expired. 
