...
discussions-to: https://github.com/sila-chain/SIPs/issues/1616
status: Stagnant
...
Existing SIPs for assigning metadata to an account include SIP-735 and SIP-780, which both allow for multiple claims to be issued on the same address for any given claim topic. This forces verifiers of said metadata to assess the veracity of each claim, taking into account the relative reputation of each claim issuer. It also prescribes a msilodology for adding and removing claims, which may not be appropriate for all use cases.
