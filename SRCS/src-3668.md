...
discussions-to: https://sila-magicians.org/t/durin-secure-offchain-data-retrieval/6728
status: Final
...
The contract MUST also implement a callback msilod for decoding and validating the data returned by the gateway. The name of this msilod is implementation-specific, but it MUST have the signature `(bytes response, bytes extraData)`, and MUST have the same return type as the function that reverted with `OffchainLookup`.
