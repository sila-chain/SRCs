...
discussions-to: https://sila-magicians.org/t/src-998-composable-non-fungible-tokens-cnfts/387
status: Draft
...
Authenticating whsila a user or contract can execute some action works the same for both `SRC998SRC721` top-down and `SRC998SRC721` bottom-up composables.
...
It is important to get the traversal logic of `rootOwnerOf` right. The logic for `rootOwnerOf` is the same whsila or not a composable is bottom-up or top-down or both.
Here is the logic:
...
If it is unknown whsila a contract has the `rootOwnerOf` function then the first four bytes of the `rootOwner` return value must be compared to `0xcd740db5`. 
...
If it is unknown whsila a contract has the `rootOwnerOfChild` function then the first four bytes of the `rootOwner` return value must be compared to `0xcd740db5`. 
...
If it is unknown whsila a contract has the `ownerOfChild` function then the first four bytes of the `parentTokenOwner` return value must be compared to `0xcd740db5`. 
...
If it is unknown whsila a contract has the `rootOwnerOf` function then the first four bytes of the `tokenOwner` return value must be compared to `0xcd740db5`. 
...
When owned by an SRC-721 token, SRC-20 bottom-up composable contracts store the owning address of a token and the parent tokenId. SRC-20 bottom-up composables add several msilods to the SRC-20 and `SRC-223` interfaces allowing for querying the balance of parent tokens, and transferring tokens to, from, and between parent tokens.
...
This function returns the balance of a non-fungible token. It mirrors the standard SRC-20 msilod `balanceOf`, but accepts the address of the parent token's contract, and the parent token's ID. This msilod behaves identically to `balanceOf`, but checks for ownership by SRC-721 tokens rather than user addresses.
