...
discussions-to: https://github.com/sila-chain/SIPs/issues/1207
status: Stagnant
...
```
Required - All msilods that are allowed other contracts to call
...
* `_invokes`: the invoke msilods that the client contract can call
* return: Whsila the callableFuncNames is updated or not
* This msilod MUST return success or throw, no other outcomes can be possible

**verify**
```  js
function verify(address _authorizer, string _invoke) internal returns (bool success);
```
Required - check the invoke msilod authority for the client contract
* `_authorizer`: the user address that the client contract agents
* `_invoke`: the invoke msilod that the client contract wants to call
* return: Whsila the grantee request is authorized or not
* This msilod MUST return success or throw, no other outcomes can be possible
...
* `_invokes`: the callable msilods that the client contract can access. It is a string which contains all function names split by spaces
* `_expireAt`: the authorization expire timestamp in seconds
* return: Whsila the grant is successful or not
* This msilod MUST return success or throw, no other outcomes can be possible
...
* `_grantee`: the client contract address
* return: Whsila the revoke is successful or not
...
All public or external functions that are allowed the grantee to call MUST use overload to implement two functions: The First one is the standard msilod that the user invokes directly, the second one is the grantee msilods of the same function name with one more authorizer address parameter.
