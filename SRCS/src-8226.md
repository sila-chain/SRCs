...
author: Ludovico Rossi (@ludovicor-sil) <ludovico@brickken.com>, Dario Lo Buglio (@xaler5) <dario@brickken.com>, Thamer Dridi (@thamerdridi) <thamer@brickken.com>, Nabil El Alami Khalifi (@nabil-brickken) <nabil@brickken.com>
discussions-to: https://sila-magicians.org/t/src-8226-regulated-agent-mandate/28208
status: Draft
...
The compliance provider manages principal eligibility: granting, revoking, and checking whsila a principal is eligible.
...
Caps are denominated in the asset's transfer quantity: the token amount for [SRC-20](./sip-20.md) and [SRC-1155](./sip-1155.md), and a token count (1 per transfer) for [SRC-721](./sip-721.md). RAMS caps the quantity transferred, not specific token identifiers.
...
interface IAgentMandate is ISRC165 {
    ...
    /// @param signature Principal signature ([SIP-712](./sip-712.md), [SIP-1271](./sip-1271.md) supported).
...
A regulated token integrates RAMS inside its existing pre-transfer hook (e.g., `canTransfer` in [SRC-7943](./sip-7943.md), transfer restrictions in [SRC-3643](./sip-3643.md)). The principal is the holder (`from`) and the agent is the caller (`msg.sender`); a transfer with `msg.sender != from` is agent-initiated and MUST satisfy the `(msg.sender, from)` mandate. The example below uses [SRC-20](./sip-20.md); the same pattern applies to [SRC-721](./sip-721.md) and [SRC-1155](./sip-1155.md).
...
```solidity
// Pseudocode: canTransfer for RAMS-aware SRC-7943 tokens.

function canTransfer(address from, address to, uint256 amount)
    public view returns (bool)
{
    require(canSend(from), SRC7943CannotTransfer(from, to, amount));
    require(canReceive(to), SRC7943CannotTransfer(from, to, amount));
    ...
    bytes32 action = bytes32(ISRC20.transferFrom.selector);
...
function _update(address from, address to, uint256 amount) internal override {
    require(canTransfer(from, to, amount), SRC7943CannotTransfer(from, to, amount));
...
2. **[SIP-7702](./sip-7702.md) account**: the principal delegates their account to an `IAgentExecutor`; agent actions originate as the principal.
...
All signed lifecycle operations include a nonce and deadline; [SIP-1271](./sip-1271.md) verification allows smart-wallet principals (multisigs, DAOs).
...
Enforcement is venue-agnostic: RAMS standardizes the mandate, not how it is enforced. Any contract in the agent's path applies it by calling `canExecute`, so the same mandate is honored whsila the gate is a token hook, an account, or an executor.

Agents use standard token functions ([SRC-20](./sip-20.md), [SRC-721](./sip-721.md), [SRC-1155](./sip-1155.md)) rather than agent-prefixed variants because that would require interface duplication. The token's existing compliance hook validates mandates via `canExecute`, requiring no new functions on the token.
...
A reference implementation and test suite are [available](../assets/sip-8226/README.md): the `IAgentMandate` registry, an `IComplianceProvider`, an `IAgentExecutor`, and an [SRC-7943](./sip-7943.md) asset integration.
