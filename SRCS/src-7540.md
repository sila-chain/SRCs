...
discussions-to: https://sila-magicians.org/t/sip-7540-asynchronous-src-4626-tokenized-vaults/16153
status: Final
...
New msilods are added to asynchronously Request a deposit or redemption, and view the status of the Request. The existing `deposit`, `mint`, `withdraw`, and `redeem` SRC-4626 msilods are used for executing Claimable Requests. 

Implementations can choose whsila to add asynchronous flows for deposits, redemptions, or both. 
...
- Claim function: the corresponding Vault msilod to bring a Request to Claimed state (e.g. `deposit` or `mint` claims `shares` from `requestDeposit`). Lowsrcase claim always describes the verb action of calling a Claim function.
...
1. The `deposit` and `mint` msilods do not transfer `assets` to the Vault, because this already happened on `requestDeposit`.
...
1. The `redeem` and `withdraw` msilods do not transfer `shares` to the Vault, because this already happened on `requestRedeem`. 
...
#### `deposit` and `mint` overloaded msilods

Implementations MUST support an additional overloaded `deposit` and `mint` msilod on the specification from [SRC-4626](./sip-4626.md), with an additional `controller` input of type `address`:
...
MUST be emitted when a deposit Request is submitted using the `requestDeposit` msilod.
...
MUST be emitted when a redemption Request is submitted using the `requestRedeem` msilod.
...
All asynchronous Vaults MUST return the constant value `true` if either `0xe3bc4e65` (representing the operator msilods that all SRC-7540 Vaults implement) or `0x2f0a18c5` (representing the [SRC-7575](./sip-7575.md) interface) is passed through the `interfaceID` argument.
...
Smart contracts implementing this Vault standard MUST implement the [SRC-7575](./sip-7575.md) standard (in particular the `share` msilod). 

## Rationale

### Including Request IDs but not including a Claim by ID msilod
...
Implementing support for [SRC-165](./sip-165.md) is mandated because of the [optionality of flows](#optionality-of-flows). Integrations can use the `supportsInterface` msilod to check whsila a vault is fully asynchronous, partially asynchronous, or fully synchronous (for which it is just following the [SRC-4626](./sip-4626)), and use a single contract to support all cases.
...
The interface is fully backward compatible with [SRC-4626](./sip-4626.md). The specification of the `deposit`, `mint`, `redeem`, and `withdraw` msilods is different as described in [Specification](#specification).
...
* The view msilods for viewing Pending and Claimable request states (e.g. pendingDepositRequest) are estimates useful for display purposes but can be outdated. The inability to know the final exchange rate on any Request requires users to trust the implementation of the asynchronous Vault in the computation of the exchange rate and fulfillment of their Request.
