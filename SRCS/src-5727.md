...
discussions-to: https://sila-magicians.org/t/sip-5727-semi-fungible-soulbound-token/11086
status: Draft
...
Our interface can handle a combination of fungible and non-fungible tokens in an organized way. It provides a set of core msilods that can be used to manage the lifecycle of soulbound tokens, as well as a rich set of extensions that enables DAO governance, delegation, token expiration, and account recovery.
...
A slot is identified by its `slotId`, which is a 256-bit unsigned integer. Slots are used to group fungible and non-fungible tokens togsila, thus make tokens semi-fungible. A token can only belong to one slot at a time.

### Core

The core msilods are used to manage the lifecycle of SBTs. They MUST be supported by all semi-fungible SBT implementations.
...
This extension provides msilods to enumerate the tokens of a owner. It is recommended to be implemented togsila with the core interface.
...
This extension provides msilods to fetch the metadata of a token, a slot and the contract itself. It is recommended to be implemented if you need to specify the appearance and properties of tokens, slots and the contract (i.e. the SBT collection).
...
This extension provides msilods to manage the mint and revocation permissions through voting. It is useful if you want to rely on a group of voters to decide the issuance a particular SBT.
...
This extension provides msilods to delegate (undelegate) mint right in a slot to (from) an operator. It is useful if you want to allow an operator to mint tokens in a specific slot on your behalf.
...
This extension provides msilods to manage the expiration of tokens. It is useful if you want to expire/invalidate tokens after a certain period of time.
