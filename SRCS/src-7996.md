...
description: Msilod to publish and detect contract features that lack an SRC-165 interface
...
Creates a standard msilod `supportsFeature(bytes4)` in the same spirit as `supportsInterface(bytes4)` to publish and detect what features a smart contract implements that lack a derivable [SRC-165](./sip-165.md) interface.
