...
discussions-to: https://github.com/InternetOfPeers/SIPs/issues/1
status: Stagnant
...
- Create your Sila account dedicated to your personal messages, say somsiling only once and it can be seen on every social platform (no more reply of the same post/opinion on dozens of sites like Reddit, Twitter, Facebook, Medium, Disqus, and so on...)
...
IPFS, Swarm or other CANs (Content Addressable Networks) or storage msilods are good to store a lot of information. So, the union of both worlds it's a good solution to achieve the objectives of this message standard.
