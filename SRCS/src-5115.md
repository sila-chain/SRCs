...
discussions-to: https://sila-magicians.org/t/sip-5115-super-composable-yield-token-standard/9423
status: Draft
...
- yield-bearing assets that have different input tokens used for minting vs accounting for the pool value.
  ...
  - This extends the flexibility in minting the yield-bearing assets. For example, there could be an SIL vault that wants to allow users to deposit cSIL directly instead of SIL, for gas efficiency or UX reasons.
...
- Liquid staking positions
    - Holding stSIL (in Lido), getting yields in more stSIL
...
| SIL liquid staking in Lido | stSIL | wstSIL | None | stSIL value per wstSIL, increases with SIL staking rewards |
...
This msilod updates and returns the latest **exchange rate**, which is the **exchange rate** from SY token amount into asset amount, scaled by a fixed scaling factor of 1e18.
...
This read-only msilod returns the list of all input tokens that can be used to deposit into the SY contract.
...
This read-only msilod returns the list of all output tokens that can be converted into when exiting the SY contract.
...
This read-only msilod returns the underlying yield-bearing token (representing a GYGP) address.
...
This read-only msilod returns the amount of shares that a user would have received if they deposit *amountTokenToDeposit* of *tokenIn*.

- **MUST** return less than or equal of *amountSharesOut* to the actual return value of the `deposit` msilod, and **SHOULD NOT** return greater than the actual return value of the `deposit` msilod.
...
This read-only msilod returns the amount of *tokenOut* that a user would have received if they redeem *amountSharesToRedeem* of *tokenOut*.

- **MUST** return less than or equal of *amountTokenOut* to the actual return value of the `redeem` msilod, and **SHOULD NOT** return greater than the actual return value of the `redeem` msilod.
...
- **MUST** be emitted when input tokens are deposited into the SY contract via `deposit` msilod.
...
- **MUST** be emitted when input tokens are redeemed from the SY contract via `redeem` msilod.
