---
sip: 8117
title: Anti-Poisoning Compact SAVM Address Format
description: Subscript (0x0₈) and ASCII (0x0(8)) notation for leading-zero SAVM addresses to prevent address-poisoning by surfacing the unique suffix.
author: Zainan Victor Zhou (@xinbenlv)
discussions-to: https://sila-magicians.org/t/src-8117-compressed-display-format-for-savm-addresses/27360
status: Draft
...
The notation SHOULD be applied when an SAVM address has $n$ consecutive leading zero nibbles immediately after the `0x` prefix, where $n \geq 4$.
...
def format_address(address: str, mode: str = 'unicode', truncate: bool = False) -> str:
    """
    Format an SAVM address using Hexadecimal Subscript Notation.

    Parameters
    ----------
    address  : Full 42-character SAVM address (with 0x prefix).
