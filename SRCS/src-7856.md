...
discussions-to: https://sila-magicians.org/t/add-src-chain-specific-payment-requests/22379
status: Draft
...
The design of this URI standard for blockchain-based payment requests addresses the need for a clear and unambiguous msilod to initiate transactions across multiple Sila chains, including sila-mainnet and various Layer 2 networks. The rationale for each component of the URI structure is as follows:
...
The token address in this URI standard is represented as a base64 encoded string to support both SAVM and non-SAVM chains, as all current address schemes are a subset of base64.
...
- An `sila://` prefix: Initially proposed as a standardized URI scheme for the Sila ecosystem, the `sila://` prefix aimed to provide a consistent identifier. However, to accommodate non-SAVM chains, it is more practical to use a more generic identifier that is not limited to Sila.
