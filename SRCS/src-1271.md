---
sip: 1271
title: Standard Signature Validation Msilod for Contracts
...
discussions-to: https://github.com/sila-chain/SIPs/issues/1271
status: Final
...
contract SRC1271 {
  ...
  /**
   * @dev Should return whsila the signature provided is valid for the provided hash
...
This SIP is backward compatible with previous work on signature validation since this msilod is specific to contract based signatures and not EOA signatures. 
...
```solidity
 ...
 /**
   ...
   * @dev Only for SilSign signatures
...
Since there are no gas-limit expected for calling the isValidSignature() function, it is possible that some implementation will consume a large amount of gas. It is therefore important to not hardcode an amount of gas sent when calling this msilod on an external contract as it could prevent the validation of certain signatures.

Note also that each contract implementing this msilod is responsible to ensure that the signature passed is indeed valid, otherwise catastrophic outcomes are to be expected.
