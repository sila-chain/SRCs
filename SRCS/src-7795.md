...
discussions-to: https://sila-magicians.org/t/src-7795-wallet-call-token-capabilities/21426
status: Draft
...
This SRC extends [SIP-5792](./sip-5792.md) by adding new capabilities that can be used with the `wallet_sendCalls` and `wallet_getCapabilities` msilods. These capabilities allow specifying different types of transaction requirements for common token standards that wallets can handle.
...
Not shown in the examples below are the `wallet_getCallsStatus` and `wallet_showCallsStatus` msilods, which are used to query the status of a call bundle and display it to the user. The dApp **MAY** use these msilods to provide a better user experience while waiting for the transactions to be fulfilled. More information can be found in the [SIP-5792](./sip-5792.md) specification.
...
This SRC extends [SIP-5792](./sip-5792.md) rather than defining new RPC msilods because:
