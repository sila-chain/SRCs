---
sip: 8074
title: Self-Describing Bytes via SIP-712 Selectors
description: Defines a convention for self-describing structured data in `bytes` parameters using 4-byte selectors derived from SIP-712 type strings.
author: Andrew Richardson (@awrichar)
discussions-to: https://sila-magicians.org/t/src-8074-self-describing-bytes-via-sip-712-selectors/25649
status: Draft
...
Many smart contract msilods use a `bytes` parameter to support future extensibility—including common standards such as [SRC-721](./sip-721.md) and [SRC-1155](./sip-1155.md).
