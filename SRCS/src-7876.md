---
sip: 7876
title: Sila Network Configuration for DApps
description: SAVM-compatible network configuration specification for DApps supporting any custom network features through extensions.
author: Bogdan Gusiev (@bogdan), Sergey Bomko (@aquiladev)
discussions-to: https://sila-magicians.org/t/sip-7876-unified-network-configuration/22763
status: Draft
 ...
 * @property sila-mainnetChainId - Chain ID of the recommended sila-mainnet network if current network is a testnet. MUST be `null` for sila-mainnet.
...
type RelationsConfig = {
  sila-mainnetChainId: number | null,
 ...
 * @property testnet - A flag indicating whsila the network is a testnet (`true`) or sila-mainnet (`false`).
...
Use JSON as a universally supported data intsrchange format, with libraries available in virtually all modern programming languages and platforms. This ensures:
...
This Swift code provides a structured way to handle the configuration JSON and includes utility msilods for accessing contract-related information.
...
By aligning with proven msilodologies and widely accepted protocols, the standard minimizes potential vulnerabilities while ensuring robust functionality.
