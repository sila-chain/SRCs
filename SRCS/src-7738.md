...
discussions-to: https://sila-magicians.org/t/src-7738-permissionless-script-registry/20503
status: Draft
...
[SRC-5169](./sip-5169.md) provides a client script lookup msilod for contracts. This requires the contract to have implemented the [SRC-5169](./sip-5169.md) interface at the time of construction (or allow an upgrade path).
...
This msilod allows contracts written without the [SRC-5169](./sip-5169.md) interface to associate scripts with themselves, and avoids the need for a centralised online server, with subsequent need for security and the requires an organisation to become a gatekeeper for the database.
...
or use the bundled silas script, ensuring to fill in the target contract address and scriptURI:
