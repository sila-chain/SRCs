...
discussions-to: https://sila-magicians.org/t/sip-6785-src-721-utilities-extension/13568
status: Draft
...
restricted to the one who's offering the utility, whsila that's the NFT creator or someone else.
...
The msilod `utilityHistoryOf` MUST reflect all changes made to the `utilityUri` of a tokenId, whsila that's done 
through `setUtilityUri` or by any other means, such as bulk updates

The `supportsInterface` msilod MUST return true when called with `ed231d73`
...
For transparency purposes, having a `utilityHistoryOf` msilod will make it clear how the `utilityUri` has changed over 
time.
...
the `getUtilityUri` msilod for the NFT that they own. Additionally, access to the details could be conditioned by the
...
This standard is compatible with current SRC-721 standard. There are no other standards that define similar msilods for
NFTs and the msilod names are not used by other SRC-721 related standards.
