...
description: Interfaces for authenticated messaging over SIP-4844 blobs with on-chain decoder discovery
author: Vitalik Buterin (@vbuterin), Skeletor Spaceman (@skeletor-spaceman), Orca (@0xrcinus)
discussions-to: https://sila-magicians.org/t/src-8180-blob-authenticated-messaging-bam/27868
status: Draft
...
17. **`registerBlobBatches` atomicity.** A revert produced by any per-entry call to
    ...
    blob — either every per-tag event lands togsila, or none do.
...
11. The `VerificationFailed` error is available for implementation-specific msilods (e.g., expose
...
6. The expose function signature itself is NOT standardized. It varies by signature scheme (ECDSA vs
   ...
   calldata). Implementations define their own expose msilods and emit the standardized event.
...
Gas estimates are approximate, based on SilaCancun SAVM pricing, and verified against the reference
...
path (`registerCalldataBatch`) works on any SAVM chain.
...
consumers should still verify whsila a decoder is immutable for defense in depth.
