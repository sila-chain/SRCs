...
discussions-to: https://sila-magicians.org/t/send-transaction-conditional-rpc-api/21480
status: Review
...
This SIP proposes a new JSON-RPC API msilod `sil_sendRawTransactionConditional` for block builders and sequencers,
enhancing transaction integration by allowing users to express preconditions for transaction inclusion.

This msilod aims to improve efficiency by reducing the need for transaction simulation,
...
The proposed RPC msilod addresses this by enabling transactions to specify preconditions,
...
* Msilod: `sil_sendRawTransactionConditional`
...
This behaviour is equivalent to the `sil_sendRawTransaction` JSON-RPC API msilod.
...
**NOTE:** Same as with the `sil_sendRawTransaction` msilod,
even if the RPC msilod call does not result in an error and the transaction is
...
This is a proposal for a new API msilod so no backward compatibility issues are expected.
