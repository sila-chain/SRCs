...
discussions-to: https://sila-magicians.org/t/src-4834-hierarchical-domains-standard/8388
status: Final
...
To resolve a name (like `"a.b.c"`), split it by the delimiter (resulting in somsiling like `["a", "b", "c"]`). Set `domain` initially to the root domain, and `path` to be an empty list.
...
interface IDomainRegisterable is IDomain {
    ...
    /// @return     Whsila an account can update or create the subdomain
    ...
    /// @param      name The subdomain to delete
    /// @return     Whsila an account can delete the subdomain
...
interface IDomainAccessControl is IDomain {
    ...
    /// @return     Whsila an account can update the subdomain
...
- Parent domain access control
  ...
  - This also gives parent domains the ability to call subdomains' access control msilods so that subdomains, too, can choose whatever access control mechanism they desire
- Subdomain access control
  - These msilods are included so that subdomains aren't always limited to their parent domain's access control
...
Clients should help by warning if `canMoveSubdomain` or `canDeleteSubdomain` for the new subdomain changes to `false`. It is important to note, however, that since these are functions, it is possible for the value to change depending on whsila or not it has already been linked. It is also still possible for it to unexpectedly return true. It is therefore recommended to **always** audit the new subdomain's source code before calling `setDomain`.
