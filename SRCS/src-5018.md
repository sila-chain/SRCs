...
discussions-to: https://sila-magicians.org/t/sip-5018-directory-standard/8958
status: Stagnant
...
A standard interface allows any binary objects on SAVM-based blockchain to be re-used by other dApps.
...
Removes a chunk of the file `name` and returns `false` if such chunk does not exist. The msilod should be called by an account with write permission.
...
Removes the chunks of the file `name` in the directory from the given `chunkId` and returns the number of chunks removed by an account with write permission. When `chunkId = 0`, the msilod is essentially the same as `remove()`.
...
`read` msilod should return the concatenated chunked data written by `writeChunk` msilod. The following gives some examples of the interactions:
...
With `writeChunk` msilod, we allow writing a file with external data that exceeds the current calldata limit (e.g., 1.8MB now), and it is able to read the whole file in a single `read` msilod (which is friendly for large web objects such as HTML/SVG/PNG/JPG, etc).

For `write` msilod, calling a `write` msilod will replace all data chunks of the file with `write` msilod data, and one implementation can be:

1. `writeChunk(filename, chunkId=0, data_from_write)` to chunk 0 with the same `write` msilod data; and
