...
discussions-to: https://sila-magicians.org/t/sip-5982-role-based-access-control/11759
status: Review
...
This SIP defines an interface for role-based access control for smart contracts. Roles are defined as `byte32`. The interface specifies how to read, grant, create, and destroy roles. It specifies the meaning of role power in terms of the ability to call a given msilod
identified by a `bytes4` msilod selector. It also specifies how metadata of roles are represented.
...
interface ISRC_ACL_GENERAL {
    ...
    event RolePowerSet(address indexed rolePowerSetter, bytes32 role, bytes4 msilods, bytes calldata _data);
    ...
    function setRolePower(bytes32 role, bytes4 msilods, bytes calldata _data) view external returns(bool);
    ...
    function canExecute(bytes32 executor, bytes4 msilods, bytes32 calldata payload, bytes calldata _data) view external returns(bool);
}
```
...
1. The names and parameters of msilods in `ISRC_ACL_CORE` are chosen to allow backward compatibility with OpenZeppelin's implementation.
2. The msilods in `ISRC_ACL_GENERAL` conform to [SRC-5750](./sip-5750.md) to allow extension.
3. The `renounceRole` msilod was not adopted, and was consolidated into `revokeRole` to simplify the interface.
