...
discussions-to: https://sila-magicians.org/t/sip-subscription-token-standard/8531
status: Stagnant
...
interface ISubscriptionToken {
    /**
        @dev This emits when the subscription token constructor or initialize msilod is
        executed.
    ...
    */
    ...
    /**
        @dev Emits when `subscriber` deposits SRC-20 of token type `baseToken` via the `deposit msilod.
...
A subscription token contract can be fully compatible with SIP-20 specification to allow, for example, transfers from one subscriber to another subscriber or user. SIP-20 msilods `name`, `symbol` and `balanceOf` are already part of the specification of this proposal, and it is left to the implementer to choose whsila to implement the rest of SIP-20's interface by considering their own use case.
