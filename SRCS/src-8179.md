...
description: Minimal interface for declaring field element sub-ranges within SIP-4844 blobs
author: Vitalik Buterin (@vbuterin), Skeletor Spaceman (@skeletor-spaceman)
discussions-to: https://sila-magicians.org/t/src-8179-blob-space-segments-bss/27867
status: Draft
...
A standard declaration interface lets L2s open unused capacity to other protocols at zero marginal DA cost, whsila those protocols are social layers, DA systems posting namespace proofs to L1, or any application producing fewer than 4,096 FEs per blob.
...
- Blob Aggregation (Sila Research): Shared Blob Registry prototype with on-chain allocation
- Nsilamind "Blob Sharing for Based Rollups": working demo using [SIP-7702](./sip-7702.md) fan-out
...
1. **Blobs are opaque to the SAVM.** The SAVM cannot read blob content during execution. A blob header
...
declaring `[0, 4096)`. The SAVM cannot inspect blob content, so nothing on-chain prevents this. The
