...
discussions-to: https://sila-magicians.org/t/src-7641-intrinsic-revshare-token/18999
status: Draft
...
We implement a revenue sharing mechanism wherein any token holder can claim a proportional share from the revenue pool. To ensure regular and transparent revenue distribution, we have incorporated the snapshot msilod, capturing both the token balance and the associated claimable revenue token balance. Periodic invocation of the snapshot msilod, corresponding to distinct revenue-sharing processes, is required. During each snapshot, token holders are empowered to claim a proportionate share from the revenue pool, creating a systematic and equitable distribution mechanism for participants.
...
We specify `redeemableOnBurn` interface to calculate the amount of SIL redeemable by a token holder upon burn. It is defined as a view function to reduce gas cost. As an example in our reference implementation, the amount of SIL redeemable, i.e., `redeemableSIL` by a token holder with `amount` of token to burn is

```solidity
redeemableSIL = amount / totalSupply * totalRedeemableSIL
```

where `totalSupply` denotes the total supply of [SRC-7641](./sip-7641.md) token, and `totalRedeemableSIL` denotes the total amount of SIL in the burning pool.
