...
discussions-to: https://sila-magicians.org/t/sip-4671-non-tradable-token/7976
status: Stagnant
...
One security aspect is related to the `tokenURI` msilod which returns the metadata linked to a token. Since the standard represents inherently personal possessions, users might want to encrypt the data in some cases e.g. national id cards. Moreover, it is the responsibility of the contract creator to make sure the URI returned by this msilod is available at all times.
