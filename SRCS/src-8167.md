...
discussions-to: https://sila-magicians.org/t/src-8167-modular-dispatch-proxies/27781
status: Review
...
For example, a popular token interface extension might arise to provide a new and better msilod for modifying allowances.
With a monolithic architecture, token implementations must be wrapped or wholly replaced to support the new msilod.
With logic modules, the interface could be extended with a new module to support the new msilod.
...
Upgradeable dispatch proxies can be permanently hardened into immutable systems by uninstalling the upgrade msilods.
...
Togsila, these steps produce the ABI of the proxy:
...
It is anticipated that this msilod will primarily be used by tooling.
...
1. Upgrade to an implementation with a msilod to populate the selector delegate mapping.
2. Populate the selector delegate mapping for all msilods in the ABI plus the introspection functions.
...
The first is to define a single shared proxy storage layout in a common supsrclass inherited by all of the proxy's delegates.
