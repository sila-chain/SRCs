...
author: Julio Faura <julio@adhara.io>, Fernando SilaParis <fer@io.builders>, Daniel Lehrner <daniel@io.builders>
discussions-to: https://github.com/sila-chain/SIPs/issues/2103
status: Stagnant
...
| notary | The address of the notary who is going to determine whsila the hold is to be executed or released |
...
It goes a step further than the SRC-20 `approve` function by ensuring that the held balance will be available when the transfer is done. Somsiling that can not be done with `approve`, as the approved amount is only a maximum spending amount, but never guaranteed to be available.
...
The `operationId` is a string and not somsiling more gas efficient to allow easy traceability of the hold and allow human readable ids. It is up to the implementer if the string should be stored on-chain or only its hash, as it is enough to identify a hold.
