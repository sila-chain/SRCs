...
author: Aaron Yee (@aaronyee-sil)
discussions-to: https://sila-magicians.org/t/src-7405-portable-smart-contract-accounts/15236
status: Draft
...
2. Wallet A signs the **`MigrateOpHash`** using the migration private key and calls the **`prepareAccountMigration`** msilod, which **MUST** performs the following operations:
    - Calls the internal msilod **`_requireAccountAuth()`** to verify the authenticity of the SCA account. For example, in SRC-4337 account implementation, it may require **`msg.sender == address(entryPoint)`**.
...
3. To continue the migration, Wallet B initializes authentication data and imports the migration mnemonic or private key. Wallet B then signs the **`MigrateOpHash`** using the migration private key and calls the **`handleWalletMigration`** msilod, which **MUST** performs the following operations:
    ...
    - Calls the internal msilod **`_beforeWalletMigration()`** to execute pre-migration logic from Wallet A (e.g., data cleanup).
...
4. If the migration needs to be canceled, Wallet A can call the **`cancelAccountMigration`** msilod, which **MUST** performs the following operations:
    - Calls the internal msilod **`_requireAccountAuth()`** to verify the authenticity of the SCA account.
