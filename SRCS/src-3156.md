...
discussions-to: https://sila-magicians.org/t/src-3156-flash-loans-review-discussion/5077
status: Final
...
- Some lenders offer a single entry point into the protocol regardless of whsila you're buying, selling, depositing or chaining them togsila as a flash loan, whereas other protocols offer discrete entry points.
...
contract FlashMinter is SRC20, ISRC3156FlashLender {
    ...
    /**
     * @param fee_ The psrcentage of the loan `amount` that needs to be repaid, in addition to `amount`.
     */
    constructor (
        string memory name,
...
3. Trigger a debt rate increase in `underwriter`, making `lender` undsrcollateralized.
