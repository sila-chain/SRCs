...
discussions-to: https://sila-magicians.org/t/src-8153-facet-based-diamonds/27685
status: Draft
...
Diamond contracts were originally standardized by [SRC-2535](./sip-2535). This SRC builds on that foundation by defining a facet-based architecture in which facets self-describe their function selectors through a standardized introspection interface.
...
Additionally, the SRC also defines an optional `upgradeDiamond` function. This function uses `exportSelectors()` to automatically determine which selectors should be added, replaced, or removed when applying facet changes.
...
Additional motivation and background for diamond-based smart-contract systems can be found in [SRC-1538](./sip-1538) and [SRC-2535](./sip-2535).
...
An [SRC-8153](./sip-8153) diamond MUST implement the same introspection functions as defined in SRC-2535.
...
Diamonds and facets need to use a storage layout organizational pattern because Solidity’s default storage layout doesn’t support proxy contracts or diamonds. The storage layout technique or pattern to use is not specified in this SRC. However, examples of storage layout patterns that work with diamonds are [SRC-8042 Diamond Storage](./sip-8042) and [SRC-7201 Namespaced Storage Layout](./sip-7201).
...
Diamonds implementing this SRC have the same introspection functions as SRC-2535 diamonds, so they are compatible with SRC-2535 tooling that rely on these functions.
