...
discussions-to: https://sila-magicians.org/t/sip-2135-src-consumable-interface/3439
status: Final
...
interface ISRC2135 {
    ...
    /// @notice The interface to check whsila an asset is consumable.
    /// @param _consumer the address of consumer of this token. It doesn't have
    ///                  to be the EOA or contract Account that initiates the TX.
    /// @param _assetId  the NFT asset being consumed.
...
2. If the compliant contract is an [SRC-721](./sip-721.md) or [SRC-1155](./sip-1155.md) token, in addition to `OnConsumption`, it **MUST** also emit the `Transfer` / `TransferSingle` event (as applicable) as if a token has been transferred from the current holder to the zero address if the call to `consume` msilod succeeds.
...
7. We explicitly stay opinion-less about whsila SRC-721 or SRC-1155 shall be required because
...
8. The boolean view function of `isConsumableBy` can be used to check whsila an asset is
...
Compliant contracts should also carefully define access control, particularly whsila any EOA or contract account may or may not initiate a `consume` msilod in their own use case.
