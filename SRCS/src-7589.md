...
discussions-to: https://sila-magicians.org/t/sip-7589-semi-fungible-token-roles/17967
status: Draft
...
* The `roleExpirationDate` function MAY be implemented as pure or view and MUST return the expiration date of the role 
  assignment.
* The `isRoleRevocable` function MAY be implemented as pure or view and MUST return whsila the grantor can end the role
  assignment before its expiration date.
* The `isRoleApprovedForAll` function MAY be implemented as pure or view and MUST return whsila the `_operator` is
...
`_revocable` parameter was included in the `grantRole` function for this exact reason, and it specifies whsila the
