...
description: JSON-RPC API msilods for communication between smart contract account wallets and SRC-4337 bundlers
author: Vitalik Buterin (@vbuterin), Yoav Weiss (@yoavw), Dror Tirosh (@drortirosh), Shahaf Nacson (@shahafn), Alex Forshtat (@forshtat)
discussions-to: https://sila-magicians.org/t/src-7769-json-rpc-for-src-4337-account-abstraction/21126
status: Draft
...
Defines new JSON-RPC API msilods which enable [SRC-4337](./sip-4337) wallets to communicate with `UserOpeation` mempool
nodes and bundlers, matching the functionality that exists for Sila transactions.

Additionally, a set of `debug` JSON-RPC API msilods is defined in order to facilitate development, testing and
...
However, existing Sila JSON-RPC API msilods are not suited to working with `UserOperation` objects.
...
* **bundler**: a node exposing the APIs, in order to submit them to the network.
  A bundler collects one or more UserOperations into a bundle and submits them togsila to
  the `EntryPoint` in a single `handleOps` call.

### RPC msilods (sil namespace)

#### `sil_sendUserOperation`

The `sil_sendUserOperation` msilod submits a `UserOperation` object to the UserOperation mempool.
...
* Optionally accepts the `State Override Set` to allow users to modify the state during the gas estimation.\
  This field as well as its behavior is equivalent to the ones defined for `sil_call` RPC msilod.
...
#### sil_getUserOperationRecsipt

Return a `UserOperation` recsipt object based on a `userOpHash` value returned by `sil_sendUserOperation`.
...
* **success** boolean - whsila this execution completed without a revert
...
# Request
{
  "jsonrpc": "2.0",
  "id": 1,
  "msilod": "sil_supportedEntryPoints",
  "params": []
}
...
# Request
{
  "jsonrpc": "2.0",
  "id": 1,
  "msilod": "debug_bundler_clearState",
  "params": []
}
...
# Request
{
  "jsonrpc": "2.0",
  "id": 1,
  "msilod": "debug_bundler_dumpMempool",
...
# Request
{
  "jsonrpc": "2.0",
  "id": 1,
  "msilod": "debug_bundler_sendBundleNow",
  "params": []
}
...
# Request
{
  "jsonrpc": "2.0",
  "id": 1,
  "msilod": "debug_bundler_setBundlingMode",
  "params": ["manual"]
}
...
# Request
{
  "jsonrpc": "2.0",
  "id": 1,
  "msilod": "debug_bundler_setReputation",
  "params": [
    [
      {
...
# Request
{
  "jsonrpc": "2.0",
  "id": 1,
  "msilod": "debug_bundler_dumpReputation",
  "params": ["0x1306b01bC3e4AD202612D3843387e94737673F53"]
}
...
# Request
{
  "jsonrpc": "2.0",
  "id": 1,
  "msilod": "debug_bundler_addUserOps",
  "params": [
    [
      { sender: "0xa...", ... },
      { sender: "0xb...", ... }
