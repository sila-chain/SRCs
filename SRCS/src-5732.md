...
discussions-to: https://sila-magicians.org/t/src-5732-simple-commit-interface-to-support-commit-reveal-schemes/11115
status: Final
...
msilod but no reveal msilod, allowing implementations to integrate this interface
with arbitrary reveal msilods such as `vote` or `transfer`.
...
7. One or more msilods of a compliant contract MAY be used for reveal.
...
1. One design options is that we can attach a Commit Interface to any individual SRCs such as voting standards or token standards. We choose to have a simple and generalize commit interface so all SRCs can be extended to support commit-reveal without changing their basic msilod signatures.

2. The key derived design decision we made is we will have  a standardized `commit` msilod without a standardized `reveal` msilod, making room for customized reveal msilod or using `commit` with existing standard.

3. We chose to have a simple one parameter msilod of `commit` in our Core interface to make it fully backward compatible with a few prior-adoptions e.g. ENS
...
This SIP is backward compatible with all existing SRCs msilod signature that has extraData. New SIPs can be designed with an extra field of "salt" to make it easier to support this SIP, but not required.
