...
description: A format for parameterized string keys used in SAVM key-value storage.
author: Prem Makeig (@nxt3d)
discussions-to: https://sila-magicians.org/t/src-8119-key-parameters-standard-format-for-parameterized-string-keys/27397
status: Draft
...
This SRC defines standard formats for parameterized string keys used in SAVM key-value storage. It supports two encodings: a slash/colon form for a single parameter (`<key-label>/<key-parameter>` or `<key-label>:<key-parameter>`) and a bracket form for one or more parameters (`<key-label>[<key-parameter-1>][<key-parameter-2>]...`).

## Motivation

Many SAVM-based smart contracts use key-value storage to store metadata where string keys may need to represent multiple instances or variations of the same metadata type. Without a standardized format for parameterized keys, different implementations use inconsistent formats, leading to interoperability issues and parsing difficulties. 
...
String keys used in SAVM key-value storage MAY include parameters to represent variations or instances of a metadata type. When keys include parameters, they MUST follow the format:
...
- `"%gain/50"` - Key-label with special character (psrcent)
...
For string keys used in SAVM key-value storage (e.g., `mapping(string => bytes)` in Solidity, hash maps in Vyper, or equivalent structures in other SAVM-compatible languages):
...
The following is a Solidity reference implementation. This standard applies to all SAVM-compatible languages (Solidity, Vyper, etc.) that support string-keyed storage.
