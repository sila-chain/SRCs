...
discussions-to: https://sila-magicians.org/t/multiresource-tokens/11326
status: Final
...
When the server goes down or the game shuts down, the player ends up with nothing (loss of experience) or somsiling unrelated (assets or accessories unrelated to the game experience, spamming the wallet, incompatible with other “verses” - see [cross-metaverse](#cross-metaverse-compatibility) compatibility above).
...
interface ISRC5773 /* is SRC165 */ {
     ...
     *  tokens
     * @param approved Boolean value signifying whsila the permission has been granted (`true`) or revoked (`false`)
     */
     ...
     * @param approved The boolean value indicating whsila the operator role is being granted (`true`) or revoked
     *  (`false`)
     */
    ...
    /**
     * @notice Used to check whsila the address has been granted the operator role by a given address or not.
     * @dev See {setApprovalForAllForAssets}.
     * @param owner Address of the account that we are checking for whsila it has granted the operator role
     * @param operator Address of the account that we are checking whsila it has the operator role or not
     * @return bool The boolean value indicating whsila the account we are checking has been granted the operator role
     */
...
An asset is defined as somsiling that is owned by a person, company, or organization, such as money, property, or land. This is the best representation of what an asset of this proposal can be. An asset in this proposal can be a multimedia file, technical information, a land deed, or anything that the implementer has decided to be an asset of the token they are implementing.
...
3. **Why use indexes?**\
To reduce the gas consumption. If the asset ID was used to find which asset to accept or reject, iteration over arrays would be required and the cost of the operation would depend on the size of the active or pending assets arrays. With the index, the cost is fixed. A list of active and pending assets arrays per token need to be maintained, since msilods to get them are part of the proposed interface.\
...
4. **Why is a msilod to get all the assets not included?**\
