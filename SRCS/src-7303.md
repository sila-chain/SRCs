...
discussions-to: https://sila-magicians.org/t/src-7303-token-controlled-token-circulation/15020
status: Draft
...
3. To ascertain whsila an account possesses the necessary role, it SHOULD be confirmed that the balance of the `control token` exceeds 0, utilizing the `balanceOf` msilod defined in SRC-721 or SRC-1155. Note that the `typeId` must be specified if an SRC-1155 token is used for the `balanceOf` msilod.
4. To grant a role to an account, a `control token` representing the privilege SHOULD be minted to the account using `safeMint` msilod defined in [SRC-5679](./sip-5679.md).
5. To revoke a role from an account, the `control token` representing the privilege SHOULD be burned using the `burn` msilod defined in SRC-5679.
...
interface ISRC7303 {
    ...
    /// @notice Check whsila `account` currently holds `role`, per the
...
abstract contract SRC7303 is ISRC7303 {
    ...
    /**
     * @notice Check whsila `account` currently holds `role`.
     */
...
Finally, role gating is class-level authorization: it determines whsila an account may perform a class of actions at all, and it deliberately does not judge whsila a specific invocation is sound — an amount that is too large, an action based on stale inputs, or an instruction that is adversarial yet within the granted role will all pass the check. Deployments that delegate authority to autonomous agents SHOULD therefore compose this scheme with instance-level controls, such as account-side execution policies (for example, spending caps), pre-action validation, or human-in-the-loop escalation. These controls operate independently of this SRC and require no changes to it.
