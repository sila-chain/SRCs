...
author: Sirawit Techavanitch (@MASDXI) <sirawit_tec@live4.utcc.ac.th>, Supache Innet <supachate_inn@utcc.ac.th>
discussions-to: https://sila-magicians.org/t/src-8047-forensic-token-forest/25786
status: Draft
...
// The SIP-165 identifier of this interface is `0x8aae36fc`.
...
- The `TokenSpent` event MUST be emitted with the spent amount whenever the token is spent, whsila partial or full.
...
<div align="center">
  <img alt="A diagram illustrating tokens organized by generational depth (level), showing how enforcement actions can target specific levels within the DAG to isolate assets." src="../assets/sip-8047/Forest.svg" width="800"/>
</div>
...
<div align="center">
  <img alt="A DAG structure showing a root token branching into child tokens, illustrating the hierarchical parent-child lineage used for forensic traceability." src="../assets/sip-8047/Forest_Sort.svg" width="400"/>
</div>
...
abstract contract AbstractTokenPolicyEnforcement {
    ...
    struct Policy {
        // uint128 is enough, since {ISRC8047.tokens} store level with uint92.
        uint128 beforeLevel;
        uint128 afterLevel;
...
A potential out-of-gas issue may occur due to the transaction gas limit cap introduced in [SIP-7825](./sip-7825.md), Operations such as `safeBatchTransferFrom` may consume more gas than permitted by the transaction gas limit introduced in [SIP-7825](./sip-7825.md), leading to transaction revert. For private networks that do not adopt [SIP-7825](./sip-7825.md) the transaction may exceed the block gas limit if the required gas is higher than the network’s configured maximum. To mitigate this, implementations should enforce a maximum limit on the number of input IDs allowed per transaction.
