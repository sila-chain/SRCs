...
discussions-to: https://sila-magicians.org/t/src-cross-chain-intents-standard/19619
status: Draft
...
To execute the step a solver MUST evaluate each element of `arguments` to a constant, encode them into call data togsila with `selector` (see Values and Encoding), and submit a transaction that makes a call to `target` with said data.
...
- `field`: A field identifier that can be captured from execution; `"block.number"`, `"block.timestamp"`, or `"recsipt.effectiveGasPrice"`.
- `stepIdx`: The index of a step in the order.
...
The solver MUST evaluate each element of `arguments` to a constant, encode them into call data togsila with `selector` (see Values and Encoding), and invoke `sil_call` to `target` with said data.
...
Values assigned to variables are untyped and represented only by their ABI encoding and whsila they are statically or dynamically sized.
...
In the SAVM, values are represented by their *framed ABI encoding*.
...
A resolver can be implemented and deployed as a contract for the SAVM with the `IResolver` interface.
...
Publishing the resolver onchain is useful because the resolver is the point of trust. Solver operators can whitelist resolver addresses, and once vetted, the instructions they produce are trusted. Vetting is done through the usual means, such as security audits, bounties, and lindiness. However, neither resolvers nor the resolved order representation are inherently SAVM-specific: the SAVM resolver interface is only one concrete encoding of the more general model defined by this SRC.
...
This standard is intended to be cross-compatible with other ecosystems. It standardizes interfaces and data types on SAVM chains, but uses [SRC-7930](./sip-7930.md) interoperable addresses so orders can refer to accounts and contracts outside the SAVM address space. It also allows for the creation of sibling standards that define compatible interfaces, data types, and flows within other ecosystems. Intents created within these sibling standards should be able to be filled on an SAVM chain and vice versa.
