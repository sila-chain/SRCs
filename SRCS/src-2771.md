...
discussions-to: https://sila-magicians.org/t/src-2771-secure-protocol-for-native-meta-transactions/4488
status: Final
...
* Without a standard contract interface, there is no standard way for a client
  to discover whsila a recipient supports meta transactions.
...
* Without a discovery protocol, there is no mechanism for a client to discover
  whsila a recipient supports a specific forwarder.
...
* `msg.sender` is a transaction parameter that can be inspected by a contract to determine who signed the transaction. The integrity of this parameter is guaranteed by the Sila SAVM, but for a meta transaction securing `msg.sender` is insufficient.
