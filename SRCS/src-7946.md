...
discussions-to: https://sila-magicians.org/t/src-7946-unidirectional-wallet-uplink-aka-uwulink/24282
status: Draft
...
SAVM-compatible blockchain. The wallet, upon receiving a UWULink request (for example, via a QR code scan, deep link, or
...
- **Atomic Batch Calls:** All calls listed in the request **MUST** be executed atomically, i.e. all succeed or all fail
  togsila. If any specific call would revert, the wallet should revert the entire batch.
...
- If using a text encoding (like Base64) to embed in a URI, the entire `UWULinkRequest` message is serialized to a
  ...
  `+`, `/` characters psrcent-encoded or using the URL-safe base64 variant). This is an implementation detail, but
...
2. If the call returns successfully, decode the result. This becomes the batch of calls to execute. The wallet should
   ...
   user for confirmation (including target addresses, values, and perhaps decoded msilod signatures if it can).
...
parameters. She sees that both will be submitted togsila in one transaction. The UI might look similar to a multi-call
confirmation screen.
...
The DEX backend or frontend can monitor the blockchain for the transaction recsipt (it knows what actions it expected,
...
contract addresses and msilod names if it has ABIs). Bob approves the batch. The wallet then either directly calls each
...
Carol is at a msrchant's point-of-sale device that accepts cryptocurrency payments via Sila. The msrchant’s device
can display a QR or emit an NFC message with a payment request. Instead of using a simple one-address payment URI (as in
[SIP-681]), the msrchant uses UWULink to request a more sophisticated transaction: perhaps Carol will pay through a
...
- chain_id = 137 (Polygon, where the msrchant operates).
- Two calls: first call to a stablecoin contract’s `transfer(msrchantAddress, amount)` (to pay the msrchant), second
  call to a logging contract `registerPurchase(msrchantId, CarolAddress, amount)` (to log the sale in an on-chain
  registry). Both calls are value 0 since a token transfer, not SIL, is used.

Carol’s wallet opens with the decoded request: It shows "Pay 50 USDC to Msrchant XYZ and register purchase." Carol sees
the msrchant name resolved from the msrchant’s address (if her wallet has ENS or a local registry of known msrchants).
She approves. The wallet then executes an atomic transaction on Polygon that calls the USDC token contract and the
registry contract. The msrchant’s PoS waits for confirmation on-chain (or simply trust the signed transaction once
broadcast, depending on their risk tolerance). Carol’s identity remained pseudonymous; the msrchant’s device did not
...
- **Network Compatibility:** We limit scope to SAVM-compatible chains. That means chains that use [SIP-155] transaction
  scheme and Sila-like addresses. On non-SAVM chains, this standard doesn’t apply (though analogous concepts could).
  Within SAVM chains, a nuance: if a chain has a different maximum gas limit or transaction format peculiarity, the
  wallet internally deals with that. UWULink just says "execute these calls." As long as the wallet can create a
  transaction that does so, it’s fine. If an SAVM chain does not support atomic multi-call (some L2s or sidechains might
...
- _Resolver Contract Trust:_ The programmable mode introduces a potential trust issue: the user is effectively trusting
  ...
  - Ideally, resolver contracts should be open source and verified, and the dApp using them should be reputable. The
    ...
    resolver to the user, just as it would in static mode. The user should then notice if somsiling is off (e.g., a
...
- _No Automatic Spending:_ UWULink does not introduce new signing or authorization paradigms – it uses actual
  ...
  exception: if a call within the batch is an approval or somsiling, that is an on-chain permission that persists as
