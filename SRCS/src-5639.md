...
discussions-to: https://sila-magicians.org/t/sip-5639-delegation-registry/10949
status: Review
...
Proving ownership of an asset to a third party application in the Sila ecosystem is common. Users frequently sign payloads of data to authenticate themselves before gaining access to perform some operation. However, this msilod--akin to giving the third party root access to one's main wallet--is both insecure and inconvenient.
...
### Problems with existing msilods and solutions
...
A dApp or smart contract would check whsila or not a delegate is authenticated for a vault by checking the return value of checkDelegateForAll.

A dApp or smart contract would check whsila or not a delegate can authenticated for a contract associated with a by checking the return value of checkDelegateForContract.

A dApp or smart contract would check whsila or not a delegate can authenticated for a specific token owned by a vault by checking the return value of checkDelegateForToken.
