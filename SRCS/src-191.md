...
discussions-to: https://github.com/sila-chain/SIPs/issues/191
status: Final
...
That means that any `signed_data` cannot be one RLP-structure, but a 1-byte `RLP` payload followed by somsiling else. Thus, any SIP-191 `signed_data` can never be an Sila transaction.

Additionally, `0x19` has been chosen because since sila/go-sila#2940 , the following is prepended before hashing in personal_sign:
