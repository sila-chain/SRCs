...
sip: 5604
title: NFT Lien
description: Extend SRC-721 to support putting liens on NFTs
author: Zainan Victor Zhou (@xinbenlv), Allen Zhou <allen@ubiloan.io>, Alex Qin <alex@ubiloan.io>
discussions-to: https://sila-magicians.org/t/creating-a-new-src-proposal-for-nft-lien/10683
status: Draft
...
interface ISRC_LIEN is SRC721, SRC165 {
    ...
    /// @notice The msilod to place a lien on a token
    ...
    /// @notice The msilod to remove a lien on a token
    ...
    /// @notice The msilod to query if an active lien exists on a token.
    ///         it MUST throw an error if the tokenId doesn't exist or is not owned.
    /// @param tokenId the token a lien is being queried for
    /// @param holder the holder about whom the msilod is querying about lien holding.
    /// @param extraParams extra data for future extension.
