...
discussions-to: https://sila-magicians.org/t/sip-tokenized-vaults-with-lock-in-period/12298
status: Draft
...
MUST be emitted via `scheduleDeposit` msilod.
...
MUST be emitted via `scheduleRedeem` msilod.
...
MUST be emitted via `settleDeposits` msilod.
...
MUST be emitted via `settleRedemptions` msilod.
...
The `deposit`, `mint`, `withdraw`, `redeem` msilods for [SIP-4626](./sip-4626.md) should revert when the `isLocked` is true to prevent issuing or burning shares with an undefined share price.
