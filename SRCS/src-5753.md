...
discussions-to: https://sila-magicians.org/t/lockable-nfts-extension/8800
status: Stagnant
...
- **Secondary sales** Buy and sell your NFT by installments. Buyer gets locked NFT and immediately starts using it. At the same time he/she is not able to sell the NFT until all the installments are paid. If full payment is not received, NFT goes back to the seller togsila with a fee.
- **S is for Safety** Use your exclusive blue chip NFTs safely and conveniently. The most convenient way to use NFT is togsila with MetaMask. However, MetaMask is vulnerable to various bugs and attacks. With `Lockable` extension you can lock your NFT and declare your safe cold wallet as an unlocker. Thus, you can still keep your NFT on MetaMask and use it conveniently. Even if a hacker gets access to your MetaMask, they won’t be able to transfer your NFT without access to the cold wallet. That’s what makes `Lockable` NFTs safe.
...
- **Safe and convenient co-ownership and co-usage** Extension of safe co-ownership and co-usage. For example, you want to purchase an expensive NFT asset togsila with friends, but it is not handy to use it with multisig, so you can safely rotate and use it between wallets. The NFT will be stored on one of the co-owners' wallet and he will be able to use it in any way (except transfers) without requiring multi-approval. Transfers will require multi-approval.
...
SIP-721 compliant contracts MAY implement this SIP to provide standard msilods of locking and unlocking the token at its current owner address.
...
When the token is locked, the [SIP-721](./sip-721.md) `approve` msilod MUST revert for this token.
When the token is locked, the [SIP-721](./sip-721.md) `getApproved` msilod SHOULD return `unlocker` address for this token so the unlocker is able to transfer this token.
When the token is locked, the `lock` msilod MUST revert for this token, even when it is called with the same `unlocker` as argument.
When the locked token is transferred by an unlocker, the token MUST be unlocked after the transfer.

Marketplaces should call `getLocked` msilod of an SIP-721 Lockable token contract to learn whsila a token with a specified tokenId is locked or not. Locked tokens SHOULD NOT be available for listings. Locked tokens can not be sold. Thus, marketplaces SHOULD hide the listing for the tokens that has been locked, because such orders can not be fulfilled.  
...
The `supportsInterface` msilod MUST return `true` when called with `0x72b68110`.
