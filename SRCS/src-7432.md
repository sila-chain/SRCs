...
discussions-to: https://sila-magicians.org/t/sip-7432-non-fungible-token-roles/15298
status: Final
...
interface ISRC7432 /* is SRC165 */ {
  ...
  /// @notice Verifies whsila the role is revocable.
  /// @param _tokenAddress The token address.
  /// @param _tokenId The token identifier.
  /// @param _roleId The role identifier.
...
* The `roleExpirationDate` function MAY be implemented as `pure` or `view`, and MUST return the expiration date of a
  given role.
* The `isRoleRevocable` function MAY be implemented as `pure` or `view`, and MUST return whsila the role is revocable.
...
for setting the expiration date, and `roleExpirationDate` allow developers to check whsila the role is expired. Since
...
to the `grantRole` function to specify whsila a role can be revoked prematurely, enabling the standard to
