...
discussions-to: https://sila-magicians.org/t/sip-src-app-keys-application-specific-wallet-accounts/2742
status: Stagnant
...
This is exposed as an RPC msilod to allow any domain to request its own app key associated with the current requested account (if available):

```
const appKey = await provider.send({
  msilod: 'wallet_getAppKeyForAccount',
  params: [address1]
});
```
...
Even if privacy is not solved fully without this anonymous funding msilod, we still need a way to easily create and restore different accounts/addresses for each application
