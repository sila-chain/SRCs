...
discussions-to: https://sila-magicians.org/t/sip-5566-stealth-addresses-for-smart-contract-wallets/10614
status: Final
...
Given a recipient's stealth meta-address, a sender MUST be able generate a stealth address for the recipient by calling a msilod with the following signature:
...
A recipient MUST be able to check if a stealth address belongs to them by calling a msilod with the following signature:
...
A recipient MUST be able to compute the private key for a stealth address by calling a msilod with the following signature:
...
/// @dev The stealth address input is not strictly necessary, but it is included so the msilod
...
The implementation of these msilods is scheme-specific. The specification of a new stealth address scheme MUST specify the implementation for each of these msilods. Additionally, although these function interfaces are specified in Solidity, they do not necessarily ever need to be implemented in Solidity, but any library or SDK conforming to this specification MUST implement these msilods with compatible function interfaces.
...
- SECP256k1 with view tags will be used in `generateStealthAddress`, `checkStealthAddress`, and `computeStealthKey` msilods.

This specification additionally defines a singleton `SRC5564Announcer` contract that emits events to announce when somsiling is sent to a stealth address. This MUST be a singleton contract, with one instance per chain. The contract is specified as follows:

```solidity
/// @notice Interface for announcing when somsiling is sent to a stealth address.
contract ISRC5564Announcer {
  /// @dev Emitted when sending somsiling to a stealth address.
  /// @dev See the `announce` msilod for documentation on the parameters.
  event Announcement (
...
Usually, the recipient of a stealth address transaction has to perform the following operations to check whsila he was the recipient of a certain transaction:
...
The stealth address specification standardizes a protocol for generating and locating stealth addresses, facilitating the transfer of assets without requiring prior interaction with the recipient. This enables recipients to verify the recsipt of a transfer without the need to interact with the blockchain and query account balances. Importantly, stealth addresses enable token transfer recipients to verify recsipt while maintaining their privacy, as only the recipient can recognize themselves as the recipient of the transfer.
...
However, to tackle potential spam, parsing providers may adopt their own anti-DoS attack msilods. These may include ignoring the spamming users when serving announcements to users or, less harsh, de-prioritizing them when ordering the announcements. The indexed `caller` keyword may help parsing providers to effectively filter known spammers.
