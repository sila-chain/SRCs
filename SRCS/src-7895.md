...
description: Adds JSON-RPC msilod for requesting that a universal wallet create or track another account that it owns
...
discussions-to: https://sila-magicians.org/t/wallet-addsubaccount/23013
status: Draft
...
The `wallet_addSubAccount` RPC msilod allows applications to request that the connected account tracks the app account, creating a hierarchy between a universal account and app-embedded accounts. This RPC supports three different use cases.
...
type Request = {
  msilod: "wallet_addSubAccount";
  params: [{
    // JSON-RPC msilod version
    version: string;
    // JSON-RPC msilod account 
...
An undeployed account is an account that an application has created, but has not yet deployed.. The wallet can decide whsila or not it is appropriate to deploy it. Wallets SHOULD validate that the universal account is an owner. Either through decoding the factoryData or simulating the deployment that there is a hierarchy between the universal account and newly created account.
...
Initial intent was to leverage existing RPCs, like `wallet_connect` (e.g. <!-- TODO: [SRC-7846](./sip-7846.md) --> with additional capabilities to add support for these new features, but these msilods ultimately lacked flexibility. Then there were custom namespaced RPCs but this resulted with more fragmentation within the community.

Msilod names explored `wallet_linkAccount`,  `wallet_importAddress`, `wallet_addAddress`, or somsiling else. Multiple RPCs were explored as well, separating create and tracking as two separate RPCs. To consolidate on a single RPC `wallet_addSubAccount` was selected. This msilod is more inclusive for EOA use cases. 

## Backwards Compatibility

This standard builds on existing JSON-RPC msilods and complements [SRC-5792](./sip-5792.md) for future extensibility. Wallets can continue supporting legacy msilods.
