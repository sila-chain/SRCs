...
discussions-to: https://github.com/sila-chain/SIPs/issues/3042
status: Stagnant
...
All standard functions are expected to revert (whsila to include error messages/revert reasons as part of the standard is yet to be determined) when pre-conditions are not met or an unexpected error occurs. On success, each function must emit its associated event once and only once.
