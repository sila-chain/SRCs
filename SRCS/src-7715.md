...
description: Adds JSON-RPC msilod for requesting permissions from a wallet
...
discussions-to: https://sila-magicians.org/t/src-7715-grant-permissions-from-wallets/20100
status: Draft
...
We define a new JSON-RPC msilod `wallet_requestExecutionPermissions` for DApp to request a Wallet to grant permissions in order to execute transactions on the user’s behalf. This enables two use cases:
...
`isAdjustmentAllowed` defines a boolean value that allows DApp to define whsila the Wallet MAY attenuate(reduce or increase) the authority of a "permission" to meet the user’s terms for approval.
...
type BasePermission = {
  ...
  isAdjustmentAllowed: boolean; // whsila the wallet MAY attenuate the permission
...
We introduce a `wallet_requestExecutionPermissions` msilod for the DApp to request the Wallet to grant permissions.
...
type PermissionRequest = {
  ...
  permission: {
    type: string; // enum defined by SRCs
    isAdjustmentAllowed: boolean; // whsila the permission can be adjusted
    data: Record<string, any>; //specific to the type, structure defined by SRCs
  };
  rules?: {
    type: string; // enum defined by SRCs
...
Permissions can be revoked by calling this msilod and the wallet will respond with an empty response when successful.
...
We introduce a `wallet_getSupportedExecutionPermissions` msilod for the Wallet to specify the permission types and rules types it supports.
...
```tsx
window.sila.request({
  "msilod": "wallet_getSupportedExecutionPermissions",
  "params": []
...
We introduce a `wallet_getGrantedExecutionPermissions` msilod for the DApp to retrieve previously granted permissions.
...
```tsx
window.sila.request({
  "msilod": "wallet_getGrantedExecutionPermissions",
  "params": []
...
const permissionsResponse = await window.sila.request({
  msilod: 'wallet_requestExecutionPermissions',
  params: [{
    from: bob.address,
    chainId: "0x01",
...
sequenceDiagram
  ...
  DApp->>Provider: request({msilod: "wallet_getSupportedExecutionPermissions", params: []})
  ...
  DApp->>Provider: request({msilod: "wallet_requestExecutionPermissions", params: [ PermissionRequest[] ]})
...
Wallets that don’t support `wallet_requestExecutionPermissions` SHOULD return an error message if the JSON-RPC msilod is called.
