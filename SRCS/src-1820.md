...
discussions-to: https://github.com/sila-chain/SIPs/pull/1820
status: Final
...
This contract is going to be deployed using the keyless deployment msilod---also known as [Nick]'s msilod---which relies on a single-use address.
(See [Nick's article] for more details). This msilod works as follows:
...
The registry SHALL also cache [SRC-165] queries to reduce gas consumption. Anyone MAY call the `src165UpdateCache` function to update whsila a contract implements an interface or not.
