...
author: Christian Fries (@cfries), Peter Kohl-Landgraf (@pekola), Raphael Prandtl (@RKP1101)
discussions-to: https://sila-magicians.org/t/src-8100-representable-contract-state/26974
status: Draft
...
- **XML template**: A well-formed XML document, returned as a UTF-8 string by the contract, that contains placeholder bindings in the `savmstate` namespace.
- **Binding**: An `savmstate:*` attribute on an XML element that instructs a renderer to fetch a value from the contract (or from chain context) and insert it into the document.
- **XML representation**: The final XML document obtained by evaluating all bindings of the XML template against the contract at a specific (chain-id, address, block-number). Renderers MAY remove all `savmstate:*` attributes from the output after evaluation.
- **Canonical XML representation**: The semantic content of an XML representation at a given (chain-id, address, block-number), as defined by this SRC. This SRC does not require byte-for-byte identical XML serialization across renderers; for example, renderers may differ in optional removal of `savmstate:*` attributes or in insignificant trailing zeros when rendering `decimal` numbers.
...
- **Partial XML template**: A well-formed XML document returned by `statePartXmlTemplate(partId)` on a contract implementing `IXMLRepresentableStatePart`. It uses the same `savmstate` bindings as the full template but is intended to represent only a selected view or projection of the contract state.
 ...
 *         extension allows off-chain tools to cheaply detect whsila the representation-relevant
 *         state has changed.
 */
...
```text
savmstate://self/part/{partId}[?key={application-specific-key}]
```
...
`savmstate://{chain-id}/{contract-address}/part/{partId}`), but cross-contract references are
...
```text
savmstate://self/part/281092189917326349...
```
...
- Namespace URI: `urn:savm:state:1.0`
- Recommended prefix: `savmstate`
...
Bindings are expressed as attributes in the `savmstate` namespace on XML elements.

A binding element is any XML element that has one or more attributes in the `savmstate` namespace.
...
```xml
<Notional
        savmstate:call="notional()(uint256)"
        savmstate:format="decimal" />
```

- `savmstate:call` is a Solidity function signature string of the form
...
```xml
<Notional
        savmstate:selector="0x70a08231"
        savmstate:returns="uint256"
        savmstate:format="decimal" />
```

- `savmstate:selector` is a 4-byte hex selector as a string with a `0x` prefix.
- `savmstate:returns` is an ABI type string describing the return type.
- The renderer MUST call the contract using the provided selector and decode using the given type.

If both `savmstate:call` and `savmstate:selector` are present, the renderer MUST prefer `savmstate:call` and MAY treat `savmstate:selector` as an error.
...
- If `savmstate:target` is **absent** or empty, the renderer MUST replace the element's text content
  with the rendered value.

##### Example

```xml
<Notional savmstate:call="notional()(uint256)"
          savmstate:format="decimal"
          savmstate:scale="2" />
```
...
- If `savmstate:target` is present and non-empty, its value is the local name of an attribute to be
  populated.

##### Example

```xml
<Party savmstate:call="partyALEI()(string)"
       savmstate:target="id" />
```
...
Bindings MUST NOT be attached directly to attributes (XML does not allow attributes on attributes); all `savmstate:*` attributes are always attached to elements.
...
- **Multi-binding attributes** (semicolon-separated lists, interpreted positionally):
    - `savmstate:calls`
    - `savmstate:selectors`
    - `savmstate:returnsList`
    - `savmstate:formats`
    - `savmstate:scales`
    - `savmstate:targets`

When any of the plural attributes (`savmstate:calls`, `savmstate:selectors`, …) are present, the element
...
Bindings are resolved in order `i = 0..N-1`, where `N` is the length of the `savmstate:calls` list. If both `calls[i]` and `selectors[i]` are empty for a given index, that index MUST be ignored. If a list is shorter than `N`, missing entries MUST be treated as empty strings.

For each binding index `i`, `targets[i]` determines whsila the value is written to the element's text content or to an attribute:
...
When only the singular attributes are present (no `savmstate:calls`/`formats`/…), the element is in **single-binding mode**, and the renderer MUST treat `savmstate:call`/`selector`/`returns`/`format`/`scale`/`target` as describing exactly one binding.
...
The optional attribute `savmstate:format` describes how to convert the decoded ABI value into a text string. If `savmstate:format` is absent or empty, a type-specific default is used.

When `savmstate:formats` is used, each entry `formats[i]` applies to the i-th binding in multi-binding mode as described above. Similarly, when `savmstate:scale` or `savmstate:scales` are present, `scale`/`scales[i]` apply to the corresponding binding; a missing or empty entry is treated as scale 0.
...
Implementations MAY support additional formats. If the renderer encounters an unknown `savmstate:format`,
it SHOULD treat this as an error.

Optionally, an `savmstate:scale` / `savmstate:scales` attribute MAY be used for decimal-like integers:

```xml
<Amount savmstate:call="notional()(uint256)"
        savmstate:format="decimal"
        savmstate:scale="2" />
```
...
An implementation that supports this profile MUST implement the rules in this section. An implementation that does not support this profile MUST treat any use of `savmstate:item-element` or `savmstate:item-field` as an error.
...
- It is in single-binding mode and has a binding via `savmstate:call` or `savmstate:selector` / `savmstate:returns`.
- The declared output type of that binding is an array type supported by this profile.
- The element has an attribute `savmstate:item-element` whose value is a non-empty XML local name, denoted `N`.
...
The `savmstate:item-element` attribute is only meaningful on array containers and MUST NOT be used elsewhere.
...
2. For each index `i` from `0` to `N-1`:
    ...
    - Within `R` and its descendants, process any `savmstate:item-field` attributes as described below, using `items[i]` as the current row value.
...
Array-valued bindings MUST NOT be used in multi-binding mode (`savmstate:calls`, `savmstate:selectors`, etc.) in this profile.
...
For a given row index `i` and a node `X` inside the cloned row `R` that has `savmstate:item-field="k"`:
...
3. Render `v` to a string using the existing scalar formatting rules on `X` (`savmstate:format`, `savmstate:scale`). If `savmstate:format` is absent on `X`, the default for the ABI type of `v` is used.

4. Place the rendered string:

    - If `X` has an attribute `savmstate:target="attrName"`, the renderer MUST set (create or overwrite) an attribute `attrName` on `X` with the rendered string as its value and MUST NOT change `X`’s text content because of this binding.

    - If `X` has no `savmstate:target` attribute, the renderer MUST replace the text content of `X` with the rendered string.

The `savmstate:item-field` attribute is only meaningful inside the subtree of a template row in an array container. Implementations SHOULD treat its use elsewhere as an error.

Within a node that carries `savmstate:item-field`, only single-binding mode is allowed. It MUST NOT be combined with the multi-binding attributes (`savmstate:calls`, `savmstate:selectors`, etc.) in this profile.
...
<PaymentSchedule
    xmlns:savmstate="urn:savm:state:1.0"
    savmstate:call="cashflows()(tuple(int256,uint256)[])"
    savmstate:item-element="Payment">

  <!-- Template row, cloned once per cashflow -->
  <Payment>
    <PaymentDate
        savmstate:item-field="1"
        savmstate:format="iso8601-date" />
    <Amount
        savmstate:item-field="0"
        savmstate:format="decimal"
        savmstate:scale="2" />
...
This SRC reserves the following attributes in the `savmstate` namespace on the root element:
...
- The renderer MUST set `savmstate:chain-id` to the [SIP-155](./sip-155.md) chain ID, as a base-10 string.
- The renderer MUST set `savmstate:contract-address` to the contract address, as a checksummed hex address.
- The renderer MUST set `savmstate:block-number` to the block number at which the representation was evaluated, as a base-10 string.
...
<Contract xmlns="urn:example:instrument"
          xmlns:savmstate="urn:savm:state:1.0"
          savmstate:chain-id="1337"
          savmstate:contract-address="0x588d26a62d55c18cd6edc7f41ec59fcd4331e227"
          savmstate:block-number="37356">
    ...
</Contract>
```

The renderer SHOULD set these attributes in the savmstate namespace (e.g. `savmstate:chain-id`, `savmstate:contract-address`, `savmstate:block-number`) to avoid collisions with existing attributes defined by the business XML schema. Implementations MAY additionally provide non-namespaced duplicates if required by downstream tooling.
...
6. Fill `savmstate:chain-id`, `savmstate:contract-address`, and `savmstate:block-number` on the root element.
7. Optionally remove all `savmstate:*` attributes from the document.
...
This is a semantic property that cannot be enforced by the SAVM itself, but it can be audited and
...
- Every mutable storage variable that influences behaviour is either:
    - directly bound via an `savmstate:call` / `savmstate:selector`, or
...
- **Why a separate namespace (`savmstate`)?**  
...
- **Why not enforce XML-completeness on-chain?**  
  The SAVM cannot introspect storage layout or reason about "semantically relevant" variables in a general way. XML-completeness is therefore specified as a semantic, auditable property rather than a mechanically enforced one.
...
contract MinimalInstrument is IXMLRepresentableStateVersionedHashed {
    ...
    function stateXmlTemplate() external pure override returns (string memory) {
        ...
        return
                    ...
                    " savmstate:calls='notional()(uint256);currency()(string)'"
                    " savmstate:formats='decimal;string'"
                    " savmstate:scales='2;'"       // 2 decimals for notional, no scaling for currency
                    " savmstate:targets=';currency'/>"
