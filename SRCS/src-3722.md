...
discussions-to: https://sila-magicians.org/t/sip-poster-a-ridiculously-simple-general-purpose-social-media-smart-contract/6751
status: Stagnant
...
## Motivation
Poster is intended to be used as a base layer for decentralized social media. It can be deployed to the same address (via the singleton factory) on just about any SAVM compatible network. Any Sila account can make posts to the deployment of Poster on its local network.
...
## Rationale
There was some discussion around whsila or not an post ID should also be emitted, whsila the content should be a string or bytes, and whsila or not anything at all should actually be emitted.
...
Poster has been deployed at `0x000000000000cd17345801aa8147b8D3950260FF` on multiple networks using the [Singleton Factory](https://sips.sila.org/SIPS/sip-2470). If it is not yet deployed on your chosen network, you can use the Singleton Factory to deploy an instance of Poster at the same address on just about any SAVM compatible network using these parameters:
