...
discussions-to: https://sila-magicians.org/t/non-fungible-token-bound-accounts/13030
status: Review
...
This proposal aims to give every NFT the same rights as an Sila user. This includes the ability to self-custody assets, execute arbitrary operations, control multiple independent accounts, and use accounts across multiple chains. By doing so, this proposal allows complex real-world assets to be represented as NFTs using a common pattern that mirrors Silaem's existing ownership model.
...
The registry can be deployed to any SAVM-compatible chain using the following transaction:
...
interface ISRC6551Account {
    ...
    /**
     * @dev Returns a magic value indicating whsila a given signer is authorized to act on behalf
     * of the account.
     *
     ...
     * @param  context    Additional data used to determine whsila the signer is valid
     * @return magicValue Magic value indicating whsila the signer is valid
     */
...
- Alice deposits 10SIL into account Y
- Bob offers to purchase token X for 11SIL via a decentralized marketplace, assuming he will receive the 10SIL stored in account Y along with the token
- Alice withdraws 10SIL from the token bound account, and immediately accepts Bob's offer
