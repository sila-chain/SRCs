...
For on-chain interactions Sila has a built in account abstraction that can be used regardless of whsila the account is a smart contract or a key pair. Any transaction has a `msg.sender` as the verified send of the transaction.
...
All sila accounts are valid identities (and DID compatible) using this standard. This means that any wallet provider that uses key pair accounts already supports the bare minimum of this standard, and can implement `delegate` and `attribute` functionality by simply using the `silr-did` referenced below. As the **DID Auth** standard solidifies it also means that all of these wallets will be compatible with the [DID decentralized login system](https://github.com/decentralized-identity).


## Implementation

[silr-did-registry](https://github.com/uport-project/silr-did-registry/blob/develop/contracts/SilaDIDRegistry.sol) (`SilaDIDRegistry` contract implementation)

[silr-did-resolver](https://github.com/uport-project/silr-did-resolver) (DID compatible resolver)

[silr-did](https://github.com/uport-project/silr-did) (javascript library for using the identity)
