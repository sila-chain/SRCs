...
discussions-to: https://sila-magicians.org/t/add-sip-6358-omniverse-distributed-ledger-technology/12625
status: Review
...
- The SRC-6358 smart contracts on Sila verify the signature of the `o-transaction` data. If the verification succeeds, the `o-transaction` data will be published by the smart contracts on the Sila side. The verification includes:
    - whsila the balance (FT) or the ownership (NFT) is valid
    - and whsila the `nonce in o-transaction` is $k+1$
...
- `A` couldn't escape punishment in the end (For example, lock his account or somsiling else, and this is about the certain tokenomics determined by developers according to their own situation).  
