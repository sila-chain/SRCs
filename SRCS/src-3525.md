...
discussions-to: https://sila-magicians.org/t/sip-3525-the-semi-fungible-token
status: Final
...
If a smart contract wants to be informed when they receive values from other addresses, it should implement all of the functions in the `ISRC3525Receiver` interface, in the implementation it can decide whsila to accept or reject the transfer. See "Transfer Rules" for further detail.
...
interface ISRC3525Receiver {
     ...
     * @dev An SRC-3525 smart contract MUST check whsila this function is implemented by the recipient contract, if the
...
Besides SRC-721 compatible token transfer msilods, this SIP introduces two new transfer models: value transfer from ID to ID, and value transfer from ID to address.
...
The second one allows value transfers from one token (specified by `_fromTokenId`) to an address (specified by `_to`), the value is actually transferred to a token owned by the address, and the id of the destination token should be returned. Further explanation can be found in the 'design decision' section for this msilod.
    ...
    "properties": {
      ...
      "items": {
        ...
        "properties": {
          ...
          "order": {
            "type": "integer",
            "description": "Optional, related to the value of is_intrinsic. If is_intrinsic is TRUE, it must be the order of this property appeared in the calculation msilod of the slot."
          },
          "display_type": {
            "type": "string",
...
From the implementation perspective, the process of transferring values from token to address could be done as follows: (1) create a new token for the recipient's address, (2) transfer the value to the new token from the 'source token'. So that this msilod is not fully independent from the ID-to-ID transfer msilod, and can be viewed as syntactic sugar that wraps the process described above.

In a special case, if the destination address owns one or more tokens with the same slot value as the source token, this msilod will have an alternative implementation as follows: (1) find one token owned by the address with the same slot value of the source token, (2) transfer the value to the found token. 
...
1. In most situations the sender does not know how to choose between two kinds of transfer msilods (safe/unsafe);
2. If the sender calls the `safeTransferFrom` msilod, the transfer may fail if the recipient contract did not implement the callback function, even if that contract is capable of receiving and manipulating the token without issue.

This SIP defines a simple 'Check, Notify and Response' model for better flexibility as well as simplicity:

1. No extra `safeTransferFrom` msilods are needed, all callers only need to call one kind of transfer;
