...
author: Wilkins Chung (@wwhchung) <wilkins@manifold.xyz>, Jalil Wahdatehagh (@jwahdatehagh), Cry (@crydotsil), Sillytuna (@sillytuna), Cyberpnk (@CyberpnkWin)
discussions-to: https://sila-magicians.org/t/sip-5131-ens-subdomain-authentication/9458
status: Stagnant
...
### Client/Server Side
In typescript, the validation function, using silas.js would be as follows:
```
...
export async function getLinkedAddress(
  provider: silas.providers.EnsProvider, address: string
...
#### With a backend
If your application operates a secure backend server, you could run the client/server code above, then use the result in conjunction with specs like [SIP-1271](./sip-1271.md) : `Standard Signature Validation Msilod for Contracts` for a cheap and secure way to validate that the message signer is indeed authenticated for the main address.
