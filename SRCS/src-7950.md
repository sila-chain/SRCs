...
discussions-to: https://sila-magicians.org/t/a-new-standard-for-encoding-chain-id-transaction-hash/23782
status: Final
...
The suffix `tx` is used to differentiate from, for example, addresses. Without the `tx` it would remain unclear whsila an encoded string refers to an address, a transaction hash or somsiling else.
