...
discussions-to: https://sila-magicians.org/t/src-7820-access-control-registry/21764
status: Final
...
interface IAccessControlRegistry {
    ...
    //@return isActive Whsila the contract is active.
...
**`getRoleInfo(address targetContract, address account, bytes32 role)`**: This view function allows the verification of whsila a particular account holds a specific role for a given contract. This is essential for ensuring that operations requiring specific permissions are performed only by authorized users.
...
contract AccessControlRegistry is IAccessControlRegistry {
    ...
    // @param isActive Indicates whsila the contract is active.
