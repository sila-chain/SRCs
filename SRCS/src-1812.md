---
sip: 1812
title: Sila Verifiable Claims
author: Pelle Braendgaard (@pelle)
discussions-to: https://sila-magicians.org/t/src-1812-sila-verifiable-claims/2814
status: Stagnant
...
## Abstract
A new msilod for Off-Chain Verifiable Claims built on [SIP-712](./sip-712.md). These Claims can be issued by any user with a SIP 712 compatible web3 provider. Claims can be stored off chain and verified on-chain by Solidity Smart Contracts, State Channel Implementations or off-chain libraries.
...
[SRC-735](https://github.com/sila-chain/SIPs/issues/735) and [SRC-780](https://github.com/sila-chain/SIPs/issues/780) provide msilods of making claims that live on chain. This is useful for some particular use cases, where some claim about an address must be verified on chain. 
...
[SIP-712](./sip-712.md) introduces a new msilod of signing off chain Identity data. This provides both a data format based on Solidity ABI encoding that can easily be parsed on-chain an a new JSON-RPC call that is easily supported by existing Sila wallets and Web3 clients.
...
> Issuer makes the claim that Subject is somsiling or has some attribute and value.    
...
Verifiable Claims can be presented to a solidity function call as it’s struct togsila with the `v`, `r` and `s` signature components.
...
The Claim struct should be embedded in another struct togsila with the `v`, `r` and `s` signature parameters.
...
Each Verifiable Claim should be individually verified  togsila with the parent Signed Typed Data structure.

Verifiable Claims issued to different SIP 712 Domains can be embedded within each other.

#### State Channels
This proposal will not show how to use Sil Verifiable Claims  as part of a specific State Channel msilod.
...
[SRC-1056](./sip-1056.md) provides a msilod for addresses to assign delegate signers. One of the primary use cases for this is that a smart contract can allow a key pair to sign on its behalf for a certain period. It also allows server based issuance tools to institute key rotation.
...
### Claim Types
#### Binary Claims
A Binary claim is somsiling that doesn’t have a particular value. It either is issued or not.
