...
discussions-to: https://sila-magicians.org/t/sip-7531-resolving-staked-src-721-ownership-recognition/15967
status: Review
...
interface ISRC7531Rights {
  ...
  /**
   * @dev Returns whsila the NFT supports a specific right.
   * @param right The right to check.
   * @return Whsila the NFT supports the right.
   */
...
### The `rightsHolderOf` Msilod:

The `rightsHolderOf` msilod provides a solution to this challenge. By maintaining a record of the original owner or the rightful holder of certain privileges associated with the NFT, this msilod ensures that the underlying utility of the NFT is preserved, even when the NFT itself is held in a pool.
...
2. Enhanced Flexibility: The msilod offers greater flexibility for NFT owners, allowing them to participate in staking and other DeFi activities without relinquishing the intrinsic benefits of their NFTs.

3. Compatibility and Interoperability: By introducing a new msilod instead of altering the existing ownerOf function, this SIP ensures backward compatibility with existing [SRC-721](./sip-721.md) contracts. This is crucial for maintaining interoperability across various platforms and applications in the NFT space.
