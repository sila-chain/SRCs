...
author: Leigh Cronian (@cybsrcentry) <leigh.cronian@cybsrcentry.co.uk>, Chris Johnson <chris@virtuals.io>
discussions-to: https://sila-magicians.org/t/src-8196-ai-agent-authenticated-wallet/27987
status: Last Call
...
- **Layer 1 (Register)**: [SRC-8004](./sip-8004.md) - on-chain identity and registration  
- **Layer 2 (Verify)**: [SRC-8126](./sip-8126.md) - verification/risk scoring  
...
|-------|--------------------------|---------------------------|----------------------------------------|
| 1     | Register                 | [SRC-8004](./sip-8004.md) | "Does this agent exist on-chain?"      |
| 2     | Verify                   | [SRC-8126](./sip-8126.md) | "Is this agent trustworthy and free of malicious signals?" |
| 3     | Execute                  | This SRC                  | "Is this action authorized right now?" |
...
| `agentId`              | `uint256`     | Yes      | SRC-8004 agent id; key for the SRC-8126 `getLatestRiskScore` lookup |
...
Each audit entry MUST include `previousHash` for integrity. Implementations MAY store entries off-chain (e.g. IPFS) with periodic Merkle roots anchored on-chain (for example, to SRC-8004's Validation Registry).
...
- Separation of concerns: identity (SRC-8004) and verification (SRC-8126) decoupled from execution, with verification enforced via policy 
- `policyHash` in SRC-712 signatures binds actions immutably  
