...
discussions-to: https://sila-magicians.org/t/src-8092-associated-accounts/26858
status: Draft
...
### Core Concepts
Each Association between two accounts denotes the participating addresses as `initiator` and `approver`. These accounts can be on disparate chains with different architectures made possible by a combination of [SRC-7930](./sip-7930.md) Interoperable Addresses and an enumeration of signature key types. To accommodate non-SAVM account types, addresses are recorded in the association as raw bytes.
...
- `interfaceId` is the 4-byte interface or msilod selector for the `data` field (optional).
...
### Validation
Clients or contracts determining whsila a SignedAssociationRecord is valid at the time of consumption MUST check all of the following validation steps:
...
### Revocation
Onchain Association stores MUST implement a revocation msilod. This msilod MUST allow either party of an Association to revoke a valid, active association by submitting a revocation request. 
