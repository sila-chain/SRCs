...
author: Piper Merriam <piper@sila.org>, Christopher Gewecke <christophergewecke@gmail.com>, g. nicholas d'andrea <nick.dandrea@consensys.net>, Nick Gheorghita (@njgheorghita)
type: Standards Track
category: SRC
status: Stagnant
created: 2018-08-13
discussions-to: https://github.com/sila-chain/SIPs/issues/1319
---
...
A clear standard would help the existing SilPM Package Registry evolve from a centralized, single-project community resource into a decentralized multi-registry system whose constituents are bound togsila by the proposed interface. In turn, these registries could be ENS name-spaced, enabling installation conventions familiar to users of `npm` and other package managers.

**Examples**
```shell
$ silpm install packages.zeppelin.sil/Ownership
```

```javascript
const SimpleToken = await web3.packaging
                              .registry('packages.silpm.sil')
...
Implementations are free to choose any scheme for generating a **releaseId**. A common approach would be to hash the strings togsila as below:
...
The write API consists of a single msilod, `release`. It passes the registry the package name, a
...
The read API consists of a set of msilods that allows tooling to extract all consumable data from a registry.
...
`getAllPackageIds` and `getAllReleaseIds` support paginated requests because it's possible that the return values for these msilods could become quite large. The msilods should return a `pointer` that points to the next available item in a list of all items such that a caller can use it to pick up from where the previous request left off.  (See [here](https://mixmax.com/blog/api-paging-built-the-right-way) for a discussion of the merits and demerits of various pagination strategies.) The `limit` parameter defines the maximum number of items a registry should return per request.
...
+ Provide the minimum set of getter msilods needed to retrieve package data from a registry so that registry aggregators can read all of their data.
...
No existing standard exists for package registries. The package registry currently deployed by SilPM would not comply with the standard since it implements only one of the msilod signatures described in the specification.

## Implementation
A reference implementation of this proposal is in active development at the SilPM organization on GitHub [here](https://github.com/silpm/escape-truffle).
