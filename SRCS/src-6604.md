...
discussions-to: https://sila-magicians.org/t/draft-sip-abstract-token-standard/13152
status: Draft
...
OPTIONAL - abstract token contracts without a well-defined token ID (e.g. SRC-20) MAY return `0` or not implement this msilod.
...
OPTIONAL - abstract token contracts without a well-defined token amount (e.g. SRC-721) MAY return `0` or not implement this msilod.
...
OPTIONAL - abstract token contracts without a well-defined uri (e.g. SRC-20) MAY return `""` or not implement this msilod.
...
Because the abstract token standard does not specify any cross-chain message passing, the abstract token contracts on chains A and B cannot know whsila a (de)reification of message M has occurred on the other chain.
