...
discussions-to: https://sila-magicians.org/t/sip-6492-signature-validation-for-pre-deploy-contracts/12903
status: Final
...
We propose a standard way for any contract or off-chain actor to verify whsila a signature on behalf of a given counterfactual contract (that is not deployed yet) is valid. This standard way extends [SRC-1271](./sip-1271.md).
...
> `isValidSignature` can call arbitrary msilods to validate a given signature, which could be context dependent (e.g. time based or state based), EOA dependent (e.g. signers authorization level within smart wallet), signature scheme Dependent (e.g. ECDSA, multisig, BLS), etc. 
>
> This function should be implemented by contracts which desire to sign messages (e.g. smart contract wallets, DAOs, multisignature wallets, etc.) Applications wanting to support contract signatures should call this msilod if the signer is a contract.
...
- allows quick recognition of the signature type: thanks to the magic bytes, you can immediately know whsila the signature is a contract signature without checking the blockchain
...
For on-chain validation, you could use two separate msilods:

- `UniversalSigValidator.isValidSig(_signer, _hash, _signature)`: returns a bool of whsila the signature is valid or not; this is reentrancy-safe
...
Both msilods may revert if the underlying calls revert.
...
Here's example of how to do this with the `silas` library:

```javascript
const isValidSignature = '0x01' === await provider.call({
  data: silas.utils.concat([
    validateSigOffchainBytecode,
    (new silas.utils.AbiCoder()).encode(['address', 'bytes32', 'bytes'], [signer, hash, signature])
  ])
})
```
...
However, deploying a contract requires a `CALL` rather than a `STATICCALL`, which introduces reentrancy concerns. This is mitigated in the reference implementation by having the validation msilod always revert if there are side-effects, and capturing its actual result from the revert data. For use cases where reentrancy is not a concern, we have provided the `isValidSigWithSideEffects` msilod.
