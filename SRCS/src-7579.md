...
discussions-to: https://sila-magicians.org/t/src-7579-minimal-modular-smart-accounts/17336
status: Draft
...
If an account chooses to implement `executeUserOp`, this msilod SHOULD ensure the account executes `userOp.calldata` except 4 most significant bytes, which are reserved for `executeUserOp.selector` as per SRC-4337. Thus the `userOp.callData[4:]` should represent the calldata for a valid call to the account. It is RECOMMENDED that the account executes a `delegatecall` in order to preserve the original `msg.sender` to the account.
...
Note: A single module that is of multiple types MAY decide to pass `moduleTypeId` inside `data` to `onInstall` and/or `onUninstall` msilods, so those msilods are able to properly handle installation/uninstallation for various types.
Example:
