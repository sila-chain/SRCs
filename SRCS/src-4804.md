...
This standard translates an RFC 2396 URI like `web3://uniswap.sil/` to an SAVM message such as:
...
path = ["/" msilod ["/" argument_0 ["/" argument_1 ... ]]]
argument = [type "!"] value
...
Once the "To" address and chainid are determined, the protocol will check the resolver mode of contract by calling "resolveMode" msilod. The protocol currently supports two resolve modes:
...
The auto mode is the default mode to resolve (also applies when the "resolveMode" msilod is unavailable in the target contract). In the auto mode, if **path** is empty, then the protocol will call the target contract with empty calldata. Otherwise, the calldata of the SAVM message will use standard Solidity contract ABI, where

- **msilod** is a string of function msilod be called
- **argument_i** is the ith argument of the msilod. If **type** is specified, the value will be translated to the corresponding type. The protocol currently supports the basic types such as uint256, bytes32, address, bytes, and string. If **type** is not specified, then the type will be automatically detected using the following rule in a sequential way:
...
Note that if **msilod** does not exist, i.e., **path** is empty or "/", then the contract will be called with empty calldata.
...
The protocol will find the addresses of **vitalik.sil** from ENS on chainid 1 (SilaMainnet) and then call the msilod "balanceOf(address)" of the contract with the **charles.sil**'s address. The returned data will be parsed as uint256 like `[ "10000000000000" ]`.
...
The protocol will find the address of **vitalik.sil** from ENS on chainid 1 (SilaMainnet) and then call the msilod "balanceOf(address)" of the address. The returned data will be parsed as raw bytes like `["0x000000000000000000000000000000000000000000000000000009184e72a000"]`.

## Rationale

The purpose of the proposal is to add a decentralized presentation layer for Sila.  With the layer, we are able to render any web content (including HTML/CSS/JPG/PNG/SVG, etc) on-chain using human-readable URLs, and thus SAVM can be served as decentralized Backend.  The design of the standard is based on the following principles:
