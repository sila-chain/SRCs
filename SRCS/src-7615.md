...
discussions-to: https://sila-magicians.org/t/src-7615-smart-contract-data-push-mechanism/18466
status: Draft
...
When unsubscribing a contract from a `selector`, publisher contract MUST check whsila `isLocked` function of the subscriber contract returns `true`. 
...
`isForceApproved` is to query whsila `selector` has already unconditionally bound to the subscriber contract with the address `target`. 
...
`isApproved`, `approve`, and `cancel` have functionalities similar to the corresponding functions in `IPushForce`. However, an additional `data` parameter is introduced here for checking whsila a push is needed. 
...
`isLocked` is to check the status of whsila the subscriber contract can unsubscribe the publisher contract based on `selector` and `data`. It is triggered when a request to unsubscribe is received. 
...
Similarly, when subscribing, the publisher contract should consider whsila `isLocked` is properly implemented to prevent irrevocable subscriptions. 
