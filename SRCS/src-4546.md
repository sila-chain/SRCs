...
interface SilaReceiver {
  function acceptSilaDeposit(address depositor, uint amount) external returns (bool);
}
```
...
contract WrappedDeposit {
  ...
  function depositSila(address to) public payable {
    _assertContract(to);
    require(SilaReceiver(to).acceptSilaDeposit(msg.sender, msg.value));
