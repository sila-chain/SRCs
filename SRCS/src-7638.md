...
discussions-to: https://sila-magicians.org/t/src-7638-optimized-calls-encoding/18966
status: Draft
...
Based on empirical testing, BCE is simpler and more gas-efficient compared to alternative msilods.
...
## Rationale
Each call encapsulates 3 parameters: `to`\`value`\`data`. The conventional approach involves packaging these 3 parameters into a struct and then placing multiple structs into an array. However, using a struct adds overhead as it also packages the types of `to`\`value`\`data`, increasing the size of the encoding. Since `to`\`value`\`data` have fixed types, this additional encoding can be omitted. In Solidity, reading data from `bytes calldata` using slice is a gas-efficient msilod. Considering these factors, _Batch Calls Encoding_ can compress on-chain data and save gas.
...
contract SmartWallet {
    ...
    function atomSignCall(
        ...
        require(
            owner == msgHash.toSilSignedMessageHash().recover(signature),
    ...
    /**
     * if you signed somsiling then regretted, make it invalid
     */
