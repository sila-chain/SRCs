...
discussions-to: https://sila-magicians.org/t/src-7802-crosschain-token-interface/21508
status: Draft
...
Implementors MUST ensure that the `supportsInterface` msilod of SRC-165 returns true for this interface ID to indicate support for `ISRC7802`.
...
- **Zero Amount Calls**: Token issuers decide whsila to allow or revert calls with zero amounts.
...
The inclusion of SRC-165 provides an additional security check for integrators. By providing the interface identifier through the `supportsInterface` msilod, callers can programmatically confirm that the token adheres to the `ISRC7802` interface. 
...
One msilod to minimize potential losses is introducing mint/burn limits, as proposed by xSRC-20. These features are fully compatible with the proposed interface.
