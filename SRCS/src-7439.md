...
discussions-to: https://sila-magicians.org/t/prevent-ticket-touting/15269
status: Final
...
3. How to design a smooth ticketing process？
   ...
   * Easy to refund. When somsiling extreme happens and you need to cancel the show. Handling ticket refunds can be a straightforward process.
 
   * Easy to redeem. Before the show, the ticket agency can verify the ticket by the signature to confirm if the audience is genuine. `TokenStatus` needs to be equal to `sold`, and `expireTime` can distinguish whsila the audience has arrived at the correct session. After verification is passed, the ticket agency can change the `TokenStatus` to `Redeemed`.
