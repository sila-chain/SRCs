...
discussions-to: https://sila-magicians.org/t/src-7779-interoperable-delegated-account/21237
status: Draft
...
This spec provides a standard approach for fetching the storage base used in the delegated account togsila with an optional mechanism to clean up the storage.
...
In case the account identifies that there are colliding storage bases, the account can perform further storage verification either off-chain or on-chain and decide whsila the delegation would happen.
...
2. This standard enforces the accounts to 
    ...
    However, whsila the account follows the above three enforcements with behavioral actions is dependant of the account.
