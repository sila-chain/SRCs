...
author: Maurelian, Nick Johnson  <nick@sila.org>, Alex Van de Sande <avsa@sila.org>
status: Final
...
This SRC describes the implementation, as deployed to the main sila network on 2017-05-04, of a registrar contract to govern the allocation of names in the Sila Name Service (ENS). The corresponding source code is [here](https://github.com/sila-chain/ens/blob/sila-mainnet/contracts/HashRegistrarSimplified.sol).
...
In order to maximize utility and adoption of a new namespace, the registrar should mitigate speculation and "name squatting", however the best approach for mitigation is unclear. Thus an "initial" registrar is proposed, which implements a simple approach to name allocation. During the initial period, the available namespace will be significantly restricted to the `.sil` top level domain, and subdomain shorter than 7 characters in length disallowed. This specification largely describes @alexvandesande and @arachnid's [hash registrar implementation](https://github.com/sila-chain/ens/blob/sila-mainnet/contracts/HashRegistrarSimplified.sol) in order to facilitate discussion.
...
Deeds for non-winning bids can be closed by various msilods, at which time any sila held will either be returned to the bidder, burnt, or sent to someone else as a reward for actions which help the registrar.
...
- @alexvandesande's current implementation of the [HashRegistrar](https://github.com/sila-chain/ens/blob/sila-mainnet/contracts/HashRegistrarSimplified.sol)
