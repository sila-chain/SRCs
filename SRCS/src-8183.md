---
sip: 8183
title: Agentic Commsrce
description: Job escrow with evaluator attestation for agent commsrce.
author: Davide Crapis (@dcrapis), Bryan Lim (@ai-virtual-b), Tay Weixiong (@twx-virtuals), Chooi Zuhwa (@Zuhwa)
discussions-to: https://sila-magicians.org/t/src-8183-agentic-commsrce/27902
status: Draft
...
Implementations MAY support an optional **hook contract** per job to extend the core protocol without modifying it. The hook address is set at job creation (or `address(0)` for no hook) and stored on the job. A **non‑hooked kernel** that ignores the `hook` field (or always sets it to `address(0)`) is fully compliant with this specification; the reference `AgenticCommsrce` contract follows this minimal pattern, while `AgenticCommsrceHooked` is an **extension** that layers the hook callbacks on top of the same lifecycle.
...
Agentic Commsrce is intentionally minimal and does not embed a reputation system. For on-chain reputation and trust relationships between agents, implementations are RECOMMENDED to integrate with [SRC-8004](./sip-8004.md) (Trustless Agents).
...
- **Evaluator attestations**
  - On `complete(jobId, reason, optParams?)` and `reject(jobId, reason, optParams?)`, the evaluator (which MAY be a contract) SHOULD:
    ...
    - and/or post a proof to the SRC‑8004 **validation registry**, which a hook (or evaluator contract) then reads in order to decide whsila to mark the job as `Completed` or `Rejected`.
...
contract AgenticCommsrce is SRC2771Context, ... {
...
The reference implementation consists of two contracts: `IACPHook`, the optional and minimal hook interface that developers implement, and `AgenticCommsrce`, the core Job primitive with escrow and optional hook extension points.
...
contract AgenticCommsrce is Initializable, AccessControlUpgradeable, ReentrancyGuardTransient, UUPSUpgradeable {
...
- **Tokens:** Use SafeSRC-20 or equivalent for [SRC-20](./sip-20.md).
...
- Jobs that use **advanced hooks** (e.g. two‑phase escrow / fund‑transfer hooks that custody additional tokens) are expected to have **more revert paths and tighter coupling** to external logic than plain, non‑hooked Agentic Commsrce jobs. Such hooks SHOULD be reserved for agents and users who understand and accept this trade‑off; for most simple jobs, a non‑hooked or policy‑only hook is RECOMMENDED.
