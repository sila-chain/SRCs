...
description: Adds JSON-RPC msilods for requesting unsigned SIP-5792 calls based on the wallet's implementation, and then sending those calls once signed.
...
discussions-to: https://sila-magicians.org/t/new-src-wallet-call-preparation-api/22456
status: Draft
...
This proposal defines complementary JSON-RPC msilods to [SIP-5792's `wallet_sendCalls`](./sip-5792.md) to enable an Application to sign over a call bundle (instead of the Wallet). Msilods defined in this proposal are purposed for the Application to sign over `calls` and submit them to the Wallet with an accompanying signature. This is in contrast to `wallet_sendCalls`, where a Wallet itself signs over the call bundle.
...
In this specification, we define two new JSON-RPC msilods: `wallet_prepareCalls` and `wallet_sendPreparedCalls`.
  ...
  params: [
    {
      ...
      key?: {
        // Whsila the digest will be prehashed by the key (default behavior of WebCrypto APIs).
        prehash?: boolean;
        // Public key.
        publicKey: `0x${string}`;
        // Key type.
...
```typescript
type Request = {
  msilod: "wallet_sendPreparedCalls";
  params: [
    {
      // Capabilities.
      capabilities: Record<string, any>;
