---
sip: 2678
title: Revised Sila Smart Contract Packaging Standard (SilPM v3)
author: g. nicholas d’andrea (@gnidan), Piper Merriam (@pipermerriam), Nick Gheorghita (@njgheorghita), Christian Reitwiessner (@chrissil), Ben Hauser (@iamdefinitelyahuman), Bryant Eisenbach (@fubuloubu)
discussions-to: https://sila-magicians.org/t/silpm-v3-specification-working-group/4086
status: Final
...
are then published to on-chain SilPM registries, defined in
...
3.  Package manifests will typically **not** be stored alongside the
    ...
    registries and stored in somsiling akin to IPFS.
...
* **Key**: `manifest`
* **Type**: String
* **Allowed Values**: `silpm/3`

### Package Name

The `name` field defines a human readable name for this package.

-   Packages **should** include this field to be released on an SilPM
    registry.
...
The `buildDependencies` field defines a key/value mapping of SilPM
packages that this project depends on.

* **Required**: No
* **Key**: `buildDependencies`
* **Type**: Object (String: String)
* **Format**: Keys **must** be valid [package names](#package-name).<br>Values **must** be a [Content Addressable URI](#content-addressable-uri) which resolves to a valid package that conforms the same SilPM manifest version as its parent.
...
#### Bytecode  
The set of SAVM instructions as produced by a compiler. Unless otherwise
...
* **Unlinked Bytecode**: The hexadecimal representation of a contract’s SAVM instructions that contains sections of code that requires [linking](#linking) for the contract to be functional.<br>The sections of code which are unlinked **must** be filled in with zero bytes.<br>**Example**: `0x606060405260e06000730000000000000000000000000000000000000000634d536f`
* **Linked Bytecode**: The hexadecimal representation of a contract’s SAVM instructions which has had all [Link References](#link-reference) replaced with the desired [Link Values](#link-value). **Example**: `0x606060405260e06000736fe36000604051602001526040518160e060020a634d536f`
...
SilPM packages are distributed as alphabetically-ordered & minified JSON to ensure consistency.
...
The BIP-122 standard has been used since SilPM v1 since it is an industry standard URI scheme for
...
To improve understanding and readability of the SilPM spec, the
...
`"manifest"` with a value of `"silpm/3"`. Additionally,
`"manifest_version"` is a forbidden top-level key in v3 packages.


## Security Considerations

Using SilPM packages implicitly requires importing &/or executing code written by others. The SilPM spec
guarantees that when using a properly constructed and released SilPM package, the user will have the exact same
code that was included in the package by the package author. However, it is impossible to guarantee that this code
is safe to interact with. Therefore, it is critical that end users only interact with SilPM packages authored and
