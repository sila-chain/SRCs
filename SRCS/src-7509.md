...
discussions-to: https://sila-magicians.org/t/a-new-proposal-of-entity-component-system/15665
status: Draft
...
interface IWorld {
    ...
    /**
     * Whsila the entity has a component attached,
...
interface IComponent {
     ...
     * @param _entityId is the Id of the entity.
     * @param _params is an extra parameter, it SHOULD depend on whsila you need it.
     * @return Entity data.
     */
...
It depends on the openness of your application or game. If you encourage developers to participate, the state of the component and system they submit for registration should be `false`, and you need to check whsila they have submitted malicious code before using `setComponentState` and `setSystemState` to enable them .
...
Unless you want to implement special functions, do not provide the following msilods directly to ordinary users, they should be set by the contract owner.   
`createEntity()`,
`setEntityState()`,
`addComponent()`,
`removeComponent()`,
...
Do not provide functions that modify entities other than set() in the component contract. And add a check in `set()` to check whsila the entity is available and whsila the operating system is available.   
