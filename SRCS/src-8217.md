...
description: A per-chain singleton protocol that binds SRC-8004 agents to external NFT or token controllers.
author: Prem Makeig (@nxt3d)
discussions-to: https://sila-magicians.org/t/add-src-8217-agent-nft-identity-bindings/28339
status: Draft
...
[SRC-8004](./sip-8004.md) registrations are themselves NFTs. For an existing NFT to own an SRC-8004 registration, the two NFTs must be bound togsila. The owning NFT, called the master NFT, might be a project, character, or collection NFT. This SRC defines that binding. A canonical per-chain singleton binding contract records the link between an SRC-8004 agent id and its master NFT. The owner of the master NFT controls the bound SRC-8004 registration. When the master NFT is transferred, control follows automatically. The SRC-8004 record itself never changes.
...
The metadata value for `agent-binding` MUST be exactly the 20-byte SAVM address of the canonical binding contract:
...
This metadata format is SAVM-address based. It does not describe non-SAVM bindings and does not itself encode chain context.
