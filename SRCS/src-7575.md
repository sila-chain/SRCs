...
discussions-to: https://sila-magicians.org/t/src-7575-partial-and-extended-src-4626-vaults/17274
status: Final
...
It adds a new `share` msilod to the Vault, to allow the [SRC-20](./sip-20.md) dependency to be externalized.
...
All [SRC-7575](./sip-7575.md) Vaults MUST implement [SRC-4626](./sip-4626.md) excluding the [SRC-20](./sip-20.md) msilods and events.
...
Multi-Asset Vaults MUST implement the `share` msilod on each entry point. The entry points SHOULD NOT be [SRC-20](./sip-20.md).
...
The [SRC-20](./sip-20.md) implementation of `share` SHOULD implement a `vault` msilod, that returns the address of the Vault for a specific `asset`.
...
This standard is intentionally flexible to support both existing [SRC-4626](./sip-4626.md) Vaults easily by the introduction of a single new msilod, but also flexible to support new use cases by allowing separate share tokens.
...
The `vault` msilod is included to look up a Vault for a `share` by its `asset`, combined with the `VaultUpdate` event and [SRC-165](./sip-165.md) support. This enables integrations to easily query Multi-Asset Vaults.
