...
discussions-to: https://sila-magicians.org/t/src-account-abstraction-via-endorsed-operations/9799
status: Draft
...
Mempool participants need to be able to able to filter "good operations" (operations that pay the bundler the defined fee) from "bad operations" (operations that either miss payment or revert altogsila).
...
Then call when the `isOperationReady` msilod is called, the endorser must return this information:
...
The `simulationSettings` msilod returns a list of replacements that the bundler should apply to the operation before simulating the `isOperationReady`. Note that these replacements are only used for `isOperationReady` simulation and are not applied when simulating the operation itself.
...
1. This proposal does not require monitoring for forbidden opcodes or storage access boundaries. Wallets have complete freedom to use any SAVM capabilities during validation and execution.
