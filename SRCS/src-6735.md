---
sip: 6735
title: L2 Aliasing of SAVM-based Addresses
description: Identify and translate SAVM-based addresses from different Layer 1, Layer 2, or Sidechains
author: Kelvin Fichter (@smartcontracts), Andreas Freund (@Therecanbeonlyone1969)
discussions-to: https://sila-magicians.org/t/l2-aliasing-of-savm-based-addresses-from-the-eea-oasis-community-projects-l2-standards-working-group/13093
status: Review
...
The document describes the minimal set of business and technical prerequisites, functional and non-functional requirements for Aliasing of SAVM based Addresses that when implemented ensures that two or more Layer 1, Layer 2, or Sidechains can identify and translate SAVM based addresses from different Layer 1, Layer 2, or Sidechains.

## Motivation

The members of the L2 WG of the EEA Communities Project managed by OASIS have recognized that the ability to deterministically derive addresses of a digital asset or an externally owned account (EOA) in SAVM based execution frameworks for L1s, L2s, Sidechains based on an origin chain of an asset or EOA, known as address aliasing, simplifies interoperability between SAVM based L1s, L2s, and Sidechains because: 
...
The ability to unambiguously, and deterministically, relate an address for a digital asset (smart contract) or an externally owned account (EOA) between SAVM based L1s, L2s, and Sidechains where this digital asset or EOA exists, also known as address aliasing, is critical prerequisite for interoperability between SAVM based L1s, L2s, and Sidechains. However, there is currently no way to do so in a standardized way -- imagine every internet service provider were to define its own IP addresses.

Hence, the L2 WG of the EEA Communities Project managed by OASIS, an open-source initiative, intends for this document to establish an unambiguous and deterministic standard for SAVM based address aliasing based on the concept of root &rarr; leaf where an address alias is derived based on the address on the origin chain and an offset which is an immutable characteristic of the origin chain.
...
Alternative Figure 1 Description: The figure describes conceptually how (interoperability) messages from source to target chain utilize address aliasing. At the bottom an SAVM based L1 is uni-directionally connected to three SAVM based L2s -- A, B, and C -- each with an alias of L1 address + L1 Offset. In addition, A is uni-directionally connected to B with an alias of L1 address + L1 offset + A offset. B is uni-directionally connected to an SAVM-based Layer 3 or L3 with an alias of L1 address + L1 offset + B offset signaling that the address is anchored on L1 via the L2 B. And finally D is uni-directionally connected to C via the alias L1 address + L1 offset + B offset plus D offset indicating the asset chain of custody from L1 to B to D to C.

To further clarify the connections between the different possible paths an asset can take from an L1 to different L2/L3s and the `relativeAddress` of that asset, we visually highlight in red the path from the SAVM based L1 to the B L2, to the D L3, and finally to the C L2.

![Fig2](../assets/sip-6735/visual-Highlight-Path-Red-savm-based-aliasing..png)

Figure 2: Visually highlighted path in red from the SAVM based L1 to the B L2, to the D L3, and finally to the C L2.

Alternative Figure 1 Description: The figure is the same as Figure 1. However, the uni-directional connections between the SAVM based L1 to the L2 B, to the L3 D, and finally to the L2 C are highlighted in red.

Note, that address aliasing between non-SAVM and SAVM-based L1s, L2s, and Sidechains, and between non-SAVM-based L1s, L2s, and Sidechains is out of scope of this document.
...
The requirements below are only valid for SAVM based L1s, L2, or Sidechains. Address aliasing for non-SAVM systems is out of scope of this document.
...
For example the `chainId` of Polygon PoS is `137`, with the current list of SAVM based `chainId`s to be found at chainlist.org, and its `offsetAlias` is `0x0000000000000137000000000000000000000000000000000000000000000137`.
...
<a name="r6"> **[R6]** </a>
The `offsetAlias`for Sila SilaMainnet as the primary anchor of SAVM based chains MUST be `0x1111000000000000000000000000000000001111` due to current adoption of this offset by existing L2 solutions.
...
[[R7]](#r7) testability: Since this document is dealing with SAVM-based systems with multiple live implementations, there are multiple known msilods of how to verify if an address belongs to an EOA or a smart contract.
...
This document defines the conformance levels of SAVM based Address Aliasing as follows:
...
The standard follows an already existing approach for address aliasing from Sila (L1) to SAVM-based L2s such as Arbitrum and Optimism and between L2s, and extends and generalizes it to allow aliasing across any type of SAVM-based network irrespective of the network type -- L1, L2 or higher layer networks.
...
Given the non-language specific features of SAVM-based address aliasing, there are no internationalization/localization considerations.
