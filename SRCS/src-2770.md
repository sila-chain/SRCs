...
discussions-to: https://sila-magicians.org/t/src-2770-meta-transactions-forwarder-contract/5391
status: Withdrawn
...
guaranteed by the Sila SAVM, but for a meta-transaction verifying
...
## Specification
The Forwarder contract operates by accepting a signed typed data togsila with it's SRC-712 signature,
...
Registration MUST be performed in advance by a call to the following msilod:
```
...
The following msilod performs an SRC-712 signature check on a request:
```
function verify(
...
In order for the Forwarder to perform an operation, the following msilod is to be called: 
```
function execute(
   ForwardRequest forwardRequest,
   bytes32 domainSeparator,
...
```
Regardless of whsila the inner call succeeds or reverts, the nonce is incremented, invalidating the signature and preventing a replay of the request.

Note that `gas` parameter behaves according to SAVM rules, specifically SIP-150. The forwarder validates internally that
