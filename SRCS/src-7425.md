...
discussions-to: https://sila-magicians.org/t/sip-7425-tokenized-reserve/15297
status: Draft
...
interface TokenizedReserve is SRC4626{
	...
	* - SHOULD be only msilod to deposit to SRC4626 vault
	...
	* - SHOULD be only msilod for withdrawing underlying token from SRC4626 vault
	* @param assets Amount being deposited
	...
	* - MUST be only msilod to withdraw SRC20 tokens owned by the reserve (excluding share tokens deposited in a proposal )
	* - MUST be owner
