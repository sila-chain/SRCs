...
discussions-to: https://sila-magicians.org/t/sip-1967-standard-proxy-storage-slots/3185
status: Final
...
This SIP standardises the **storage slot** for the logic contract address, instead of a public msilod on the proxy contract. The rationale for this is that proxies should never expose functions to end users that could potentially clash with those of the logic contract.
...
/**
 * @dev This abstract contract provides a fallback function that delegates all calls to another contract using the SAVM
