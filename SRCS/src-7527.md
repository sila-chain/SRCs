...
discussions-to: https://sila-magicians.org/t/sip-7527-token-bound-function-oracle-amm-contract/15950
status: Draft
...
interface ISRC7527Agency {
    ...
    /**
     * @dev OPTIONAL - This msilod can be used to improve usability and clarity of Agency, but interfaces and other contracts MUST NOT expect these values to be present.
...
### Two interfaces use togsila

`ISRC7527App` and `ISRC7527Agency` can be implemented togsila for safety, but they can be independently implemented before initialization for flexibiliy.
...
* Check whsila modifiers `onlyAgency()` and `onlyApp()` are proporly implemented and applied.

* Check the function strategies.

* Check whsila the contracts can be subject to re-entrancy attack.

* Check whsila all non-fungible tokens can be unwrapped with the premium calculated from FOAMM.
