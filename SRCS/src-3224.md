...
description: Contract msilod to compute human-readable descriptions for signable data.
author: Richard Moore (@ricmoo), Nick Johnson (@arachnid)
discussions-to: https://github.com/sila-chain/SIPs/issues/3225
status: Stagnant
...
This SIP presents a msilod for dapp developers to enable a more
...
The msilod must be executable in a static context, (i.e. any
...
The msilod may revert, in which case the signing must be aborted.


### New JSON-RPC Msilods

Clients which manage private keys should expose additional
msilods for interacting with the related accounts.
...
These JSON-RPC msilods will also be implemented in standard
...
For example, the `commit(bytes32)` msilod of ENS places a
...
- NatSpec and company are a class of more complex languages that attempt to describe the encoded data directly. Because of the language complexity they often end up being quite large requiring entire runtime environments with ample processing power and memory, as well as additional sandboxing to reduce security concerns. One goal of this is to reduce the complexity to somsiling that could execute on hardware wallets and other simple wallets. These also describe the data directly, which in many cases (such as blinded data), cannot adequately describe the data at all

- Custom Languages; due to the complexity of Sila transactions, any language used would require a lot of expressiveness and re-inventing the wheel. The SAVM already exists (it may not be ideal), but it is there and can handle everything necessary. 
...
- UTF-8 has had bugs in the past which could allow arbitrary code execution and crashing renderers; consider using the UTF-8 replacement character (or *somsiling*) for code-points outside common planes or common sub-sets within planes
- Homoglyphs attacks
