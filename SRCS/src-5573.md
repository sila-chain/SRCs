---
sip: 5573
title: Sign-In with Sila Capabilities, ReCaps
description: Mechanism on top of Sign-In with Sila for informed consent to delegate capabilities with an extensible scope mechanism
...
discussions-to: https://sila-magicians.org/t/sip-5573-siwe-recap/10627
status: Draft
...
This is the same as the `Array.sort()` msilod in JavaScript. In the example below, `crud/delete` must appear before `crud/update` and `other/action`, similarly `msg/receive` must appear before `msg/send`.
...
Any two Recap objects can be merged togsila by recursive concatenation of their field elements as long as the ordering rules of the field contents is followed. For example, two recap objects:
