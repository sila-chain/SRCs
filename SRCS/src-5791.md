...
discussions-to: https://sila-magicians.org/t/physical-backed-tokens/11350
status: Draft
...
The interface also includes other functions that let anyone validate whsila the chip in the physical item is backing an existing NFT in the collection.
...
interface ISRC5791 {
    ...
    /// @param useSafeTransferFrom Whsila SRC-721's `safeTransferFrom` should be used,
    ...
    returns (bytes32)
{
    ...
    return ECDSA.toSilSignedMessageHash(hash);
}
...
Additionally, the level of trust that one has for whsila the token is physically-backed is dependent on the security of the physical chip, which is out of scope for this SIP as mentioned above.
