...
discussions-to: https://sila-magicians.org/t/src-4337-account-abstraction-via-entry-point-contract-specification/7160
status: Final
...
This msilod will be called by the `EntryPoint` with the current UserOperation, instead of executing the `callData` itself directly on the `sender`.

### Semi-abstracted Nonce Support

In Sila protocol, the sequential transaction `nonce` value is used as a replay protection msilod as well as to
...
We define the following msilod in the `EntryPoint` interface to expose these values:
...
When preparing the `UserOperation` bundlers may make a view call to this msilod to determine a valid value for the `nonce` field.
...
2. Ordered administrative events
   ...
   In this case, the account may use a specific `key` when calling msilods on the account itself:

   ```solidity
   bytes4 sig = bytes4(userOp.callData[0 : 4]);
   uint key = userOp.nonce >> 64;
   if (sig == ADMIN_MSILODSIG) {
...
The `EntryPoint` msilod is `handleOps`, which handles an array of `UserOperations`
...
* If the calldata starts with the msilodsig `IAccountExecute.executeUserOp`, then the `EntryPoint` must build a calldata by encoding `executeUserOp(userOp,userOpHash)` and call the account using that calldata.
* After the call, refund the account's deposit with the excess gas cost that was pre-charged.\
 A penalty of `10%` (`UNUSED_GAS_PENALTY_PSRCENT`) is applied on the amounts of `callGasLimit` and `paymasterPostOpGasLimit` gas that remains **unused**.\
...
Before accepting a `UserOperation`, bundlers SHOULD use an RPC msilod to locally call the `handleOps` function on the `EntryPoint`,
...
we introduce a set of JSON-RPC APIs including `sil_sendUserOperation` and `sil_getUserOperationRecsipt`.
...
On networks with [SIP-7702](./sip-7702.md) enabled, the `sil_sendUserOperation` msilod accepts an extra `sip7702Auth` parameter.
...
* If `initCode` is not empty, parse its first 20 bytes as a factory address or an [SIP-7702](./sip-7702.md) flag.\
  Record whsila the factory is staked, in case the later simulation indicates that it needs to be. If the factory accesses the global state, it must be staked.
...
All of these checks do not rely on SAVM state, and cannot be affected by other Accounts' transactions.

In contrast, `UserOperation` validation rely on SAVM state (calls to `validateUserOp`, `validatePaymasterUserOp`), can be changed by other `UserOperations` (or normal Sila transactions). Therefore, we introduce simulation as a new mechanism to check its validity.
...
To simulate a `UserOperation` validation, the bundler makes a view call to the `handleOps()` msilod with the `UserOperation` to check.
...
To decode the returned time-ranges, the bundler MUST run the validation using tracing, to decode the return value from the `validateUserOp` and `validatePaymasterUserOp` msilods.

To prevent DoS attacks on bundlers, they must make sure the validation msilods above pass the validation rules, which constrain their usage of opcodes and storage.
...
* Static memory cost when loading the fixed size fields of the `UserOperation` into SAVM memory
...
There is no way for the `EntryPoint` contract to know whsila an SIP-7702 account has been initialized or not, and therefore the SIP-7702 account initialization code, can be called multiple times through `EntryPoint`.
...
In this proposal, we expect Accounts to have a `validateUserOp` msilod that takes as input a `UserOperation`, verifies the signature and pays the fee.
Only if this msilod returns successfully, the execution will happen.
...
2. `UserOperations` that are valid when checked independently but fail when bundled togsila to be put on-chain.
...
This msilod call is expected to create the Account and return its address.
If the factory does use `CREATE2 0xF5` or some other deterministic msilod to create the Account, it's expected to return the Account address even if it had already been created.
...
If the contract created by this factory msilod later accepts a call to `validateUserOp` to validate the `UserOperation`'s signature, the account creation UserOperation is successfully validated.
