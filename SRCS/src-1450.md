...
discussions-to: https://sila-magicians.org/t/src-1450-rta-controlled-security-token-standard/26385
status: Last Call
...
- **Transfer request status view** (`getRequestStatus`) — implementations MAY instead expose equivalent request data through other read msilods (e.g., a public storage mapping)
...
interface ISRC1450 is ISRC20, ISRC165 {
     ...
     * @param account Address to check
     * @return verified Whsila the address is linked to a verified person
     ...
     * @param isApproved Whsila to approve or revoke broker status
     ...
     *      - Type 1 (psrcentage): Returns (amount * feeValue) / 10000
     */
     ...
     * @param feeType Type of fee structure (0: flat, 1: psrcentage in basis points)
     * @param feeValue Fee amount or psrcentage basis points
     * @dev Only callable by RTA to update fee structure
     *      MUST emit FeeParametersUpdated event
     *      For psrcentage fees, feeValue is in basis points (100 = 1%)
     */
     ...
     * @param approved Whsila to approve or reject the transfer
     ...
     * @param refundFee Whsila to refund the fee to the requester
     ...
     *      through other read msilods (e.g., a public storage mapping).
    ...
    event TransferRejected(
        ...
        bool feeRefunded    // Whsila fee was refunded
    );
...
bytes4 constant private GSILOLDERREGULATIONS = bytes4(keccak256("getHolderRegulations(address)"));
...
bytes4 constant public ISRC1450_INTERFACE_ID = CHANGEISSUER ^ SETTRANSFERAGENT ^ ISTRANSFERAGENT ^ ISSECURITYTOKEN ^ MINT ^ BURNFROM ^ BURNFROMREGULATION ^ GSILOLDERREGULATIONS ^ GETREGULATIONSUPPLY /* ^ ... */;
...
interface IRTAProxy {
     ...
     * @return executed Whsila the operation has been executed
...
The RTA treats both patterns equally - registration is by address whsila direct or proxy. The RTA maintains the right to revoke any broker registration at any time, ensuring ultimate control regardless of the broker's implementation choice.
...
// OR calculated psrcentage for psrcentage-based fees
```
...
2. **Address Ownership Proof**: Before any transfer, the RTA MUST verify that the recipient address is controlled by a verified person (via signature, micro-deposit, or other secure msilod)
...
3. Support for various accounting msilods
...
contract MeetingQuorum {
    uint256 public quorumPsrcentage = 5000; // 50% in basis points

    function calculateQuorum(uint256 recordDate) external view returns (uint256) {
        uint256 totalSupply = token.totalSupply();
        return (totalSupply * quorumPsrcentage) / 10000;
    }
...
3. **Simplicity Over Modularity**: Other security token suites uses multiple intsrconnected contracts and complex partition logic. SRC-1450 uses a single contract with clear, restricted operations. This reduces attack surface and audit complexity.
...
3. **RTA Control Maintained**: Whsila whole or fractional, all transfers remain under exclusive RTA control, maintaining regulatory compliance.
