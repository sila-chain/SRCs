...
discussions-to: https://sila-magicians.org/t/sip-6239-semantic-soulbound-tokens/12334
status: Final
...
In the contract events: `CreateRDF`, `UpdateRDF`, `RemoveRDF`, and the `rdfOf msilod`, the  `rdfStatements` is used in `ttl` format by default. If other formats listed above are used, a format identifier needs to be added for identification.
