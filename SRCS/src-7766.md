---
sip: 7766
title: Signature Aggregation for SRC-4337
description: An SRC-4337 improvement to aggregation of all UserOperation signatures in a bundle
author: Vitalik Buterin (@vbuterin), Yoav Weiss (@yoavw), Dror Tirosh (@drortirosh), Shahaf Nacson (@shahafn), Alex Forshtat (@forshtat)
discussions-to: https://sila-magicians.org/t/src-7766-signature-aggregation-for-account-abstraction/21123
status: Withdrawn
...
* To accept the `UserOperation`, the bundler must call `validateUserOpSignature()` to validate the `UserOperation` signature.
  This msilod returns an "alternate signature" that should be used during bundling.\
...
* Note that the above msilods are helper msilods for the bundler.
...
* Implementations of a `validateSignatures()` function MUST verify the aggregated signature's validity
  ...
  This msilod is called on-chain by `handleAggregatedOps()`
