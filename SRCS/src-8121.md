...
description: A specification for cross-chain function calls using hooks with SRC-7930 interoperable addresses.
author: Prem Makeig (@nxt3d)
discussions-to: https://sila-magicians.org/t/src-8121-delegated-metadata-resolution-via-hooks/27424
status: Draft
...
For systems that store `bytes` values, hooks MUST be ABI-encoded. Use different hook selectors depending on whsila the optional function selector is included:
...
if (value.startsWith("0x037f43ed") || value.startsWith("0x6113bfa3")) {
    ...
    const targetContract = new silas.Contract(
        address,
    ...
    const credential = silas.utils.defaultAbiCoder.decode([returnType], resultBytes);
