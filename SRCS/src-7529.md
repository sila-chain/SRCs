...
discussions-to: https://sila-magicians.org/t/add-sip-dns-over-https-for-contract-discovery-and-etld-1-association/15996
status: Draft
...
It is RECOMMENDED that SAVM address strings adhere to [SRC-1191](./sip-1191.md) so that the browser client can checksum the validity of the address and its target network before making an RPC call. 
...
```solidity
{
  ...
  /// @notice an authenticated msilod to add an eTLD+1 domain
  ...
  /// @notice an authenticated msilod to remove an eTLD+1 domain
