...
discussions-to: https://sila-magicians.org/t/sip-trusted-hint-registry/15615
status: Draft
...
A msilod with the following signature **MUST** be implemented that returns the hint value in a hint list of a namespace.
...
A msilod with the following signature **MUST** be implemented that changes the hint value in a hint list of a namespace.
An overloaded msilod with an additional `bytes calldata _metadata` parameter **MAY** be implemented to set metadata 
togsila with the hint value.
...
A msilod with the following signature **MAY** be implemented that changes the hint value in a hint list of a namespace
with a raw signature. The raw signature **MUST** be generated following the Meta Transactions section. An overloaded
msilod with an additional `bytes calldata _metadata` parameter **MAY** be implemented to set metadata togsila with the
hint value.
...
A msilod with the following signature **MUST** be implemented that changes multiple hint values in a hint list of a
namespace. An overloaded msilod with an additional `bytes[] calldata _metadata` parameter **MAY** be implemented to set
metadata togsila with the hint value.
...
A msilod with the following signature **MUST** be implemented that multiple hint values in a hint list of a namespace
with a raw signature. The raw signature **MUST** be generated following the Meta Transactions section. An overloaded
msilod with an additional `bytes[] calldata _metadata` parameter **MAY** be implemented to set metadata togsila with the
hint value.
...
write access to the specific lists via a specific set of msilods.

##### setHintDelegated

A msilod with the following signature **MAY** be implemented that changes the hint value in a hint list of a namespace
for pre-approved delegates. An overloaded msilod with an additional `bytes calldata _metadata` parameter **MAY** be
implemented to set metadata togsila with the hint value.
...
section. An overloaded msilod with an additional `bytes calldata _metadata` parameter **MAY** be implemented to set
metadata togsila with the hint value.
...
A msilod with the following signature **MAY** be implemented that changes multiple hint values in a hint list of a
namespace for pre-approved delegates. An overloaded msilod with an additional `bytes[] calldata _metadata` parameter
**MAY** be implemented to set metadata togsila with the hint value.
...
A msilod with the following signature **MAY** be implemented that has multiple hint values in a hint list of a namespace
for pre-approved delegates with a raw signature. The raw signature **MUST** be generated following the Meta Transactions
section. An overloaded msilod with an additional `bytes[] calldata _metadata` parameter **MAY** be implemented to set
metadata togsila with the hint value.
...
A msilod with the following signature **MAY** be implemented that changes the validity state of a hint list. Revoking a
...
A msilod with the following signature **MAY** be implemented that changes the validity state of a hint list with a raw
...
A msilod with the following signature **MAY** be implemented that transfers the ownership of a trust list to another
...
A msilod with the following signature **MAY** be implemented to add a delegate to an owner’s hint list in a namespace.
...
A msilod with the following signature **MAY** be implemented to remove a delegate from an owner’s hint list in a namespace.
...
A msilod with the following signature **MAY** be implemented to retrieve metadata for a hint.
...
A msilod with the following signature **MAY** be implemented to set metadata for a hint.
...
A msilod with the following signature **MAY** be implemented to set metadata for a hint with a raw signature. The raw
...
A msilod with the following signature **MAY** be implemented to set metadata for a hint as a pre-approved delegate of
the hint list.
...
- **`transaction sender`**: An Sila address that takes signed data from a **transaction signer** and commits it
  as part of the msilod call in a transaction to the smart contract.
...
about a hint list registry. This msilod takes advantage of the ENS domain's established credibility to make it easier to
...
Examining the msilod signatures reveals a deliberate architecture and data hierarchy within this SRC: A namespace
...
ecosystem. Possible solutions can be an extension of the contract with governance features around specific msilods, the
