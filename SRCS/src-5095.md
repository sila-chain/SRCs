...
discussions-to: https://sila-magicians.org/t/sip-5095-principal-token-standard/9259
status: Stagnant
...
Note that some implementations will require pre-requesting to the Principal Token before a withdrawal may be performed. Those msilods should be performed separately.
...
Note that some implementations will require pre-requesting to the principal token contract before a withdrawal may be performed. Those msilods should be performed separately.
...
MUST be emitted when Principal Tokens are burnt and underlying is withdrawn from the contract in the `SIP5095.redeem` msilod.
...
The `convertToUnderlying` msilod is an estimate useful for display purposes,
