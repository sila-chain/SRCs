...
discussions-to: https://sila-magicians.org/t/sip-5169-client-script-uri-for-token-contracts/9674
status: Final
...
This msilod avoids the need for building secure and certified centralized hosting and allows scripts to be hosted anywhere: IPFS, GitHub or cloud storage.
...
const { BigNumber, Wallet } = require('silas');
const { silas, network, getChainId } = require('hardhat');

describe('SRC5169', function () {
  before(async function () {
    this.SRC5169 = await silas.getContractFactory('SRC5169');
  });
...
While centralized storage for a decentralized functionality goes against the silos of web3, fully decentralized solutions may come with speed, price or space penalties. This SIP handles this by allowing the function `ScriptURI` to return multiple URIs, which could be a mix of centralized, individually hosted and decentralized locations.
