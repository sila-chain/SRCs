...
discussions-to: https://sila-magicians.org/t/sip-7730-proposal-for-a-clear-signing-standard-format-for-wallets/20403
status: Draft
...
- Fields in the data to sign are tied to primitive types only, but those can be displayed in many different ways. For instance, integers can be displayed as psrcentages, dates, etc...
...
The `context` key is used to provide binding context information for this file. It can be seen as a set of *constraints* on the structured data being reviewed, indicating whsila the SRC-7730 file is valid for this data. A wallet MUST ensure these constraints are met before SRC-7730 formatting information is applied to the data being signed. 
...
The current version of the specification is `2` at the time of writing. The reference schema for version `2` is [here](../assets/sip-7730/src7730-v2.schema.json).
...
Current specification covers SAVM smart contract calldata:
- Defined in Solidity
- The schema is the function ABI (derived from the matched `display.formats` function fragment)
- The container structure is an SAVM Transaction serialized in RLP encoding
...
The `@` root names values coming from the container that wraps the structured data (for example, an SAVM transaction or an SIP‑712 message). These values are container-specific; see the [Reference](#reference) section for the canonical list of container fields (e.g. `@.from`, `@.to`, `@.value`, `@.chainId`).  

*Examples*
- `@.value` — native currency value of an SAVM transaction.  
...
* Merge togsila objects sharing the same `path` value, overriding parameters of the included file with those of the including file.
...
```
Note that the keys under `context.contract` would be merged togsila to construct a full contract binding object. The field formatter `value` parameter `threshold` is overridden with value `0xFFFFFFFFFFFFFFFFFF`.
...
The current version of this specification only supports two types of binding context, SAVM smart contracts and SIP-712 domains. 
...
#### SAVM smart contract binding context (denoted 'calldata')

The `contract` sub-key is used to introduce an SAVM smart contract binding context, with the following constraints expressed as sub-keys of `contract`.
    ...
    "display": {
        "definitions": {
            "sendAmount": {
                ...
                "params": {
                    "tokenPath": "fromToken",
                    "nativeCurrencyAddress": "$.metadata.constants.addressAsSil"
                }
            }
        }
    }
...
* In `bundled` mode, elements of the arrays gets bundled togsila - array_0[0] array_1[0] array_0[1] array_1[1] .. array_0[N] array_1[N]
...
| `prefix`      | A boolean indicating whsila an SI prefix should be appended, defaults to `False` |
| *Examples*    | --- |
...
| **`raw`**       |                                                                                              |
|-----------------|----------------------------------------------------------------------------------------------|
| *Description*   | Display address as an [SIP-55](./sip-55.md) formatted string. Truncation is device dependent |
| *Parameters*    | None                                                                                         |
| *Examples*      | ---                                                                                          |
...
| **`addressName`**       |                    |
|-------------------------|--------------------|
| *Description*           | Display address as a trusted name if a trusted source exists, an SIP-55 formatted address otherwise. See [next section](#address-types-and-sources) for a reference of trusted sources |
| *Parameters*            | --- |
...
| Scheme        | Description                       |
| ------------- | --------------------------------- |
| fhsavm         | FHSAVM full homomorphic encryption |
...
### User Operations (SIP-4337)

Clear signing [SIP-4337](./sip-4337.md) User Operations is supported using the `PackedUserOperation` SIP-712 signature format. See [example-userops-sip-712.json](../assets/sip-7730/example-userops-sip712.json) for a reference implementation.
...
The `interpolatedIntent` approach provides users with a clear, natural language summary of complex multi-step operations without requiring them to mentally piece togsila separate transaction descriptions. The `intent` approach provide a more controllable wallet UI for space limited wallets.
...
[
    ...
    {
        "function": "swapExactTokensForSIL",
...
In the future, this specification could be extended to structured data like Meta Transaction in [SIP-2771](./sip-2771.md), User Operations in [SIP-4337](./sip-4337.md), and batch transaction payloads in [SIP-5792](./sip-5792.md).
...
| [example-account-execute.json](../assets/sip-7730/example-account-execute.json) | Example of SIP-4337 smart wallet execute function clear signing |
| [example-userops-sip-712.json](../assets/sip-7730/example-userops-sip712.json) | Example of SIP-4337 User Operation clear signing |
