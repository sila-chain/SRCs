...
discussions-to: https://sila-magicians.org/t/sip-5375-nft-authorship/10182
status: Final
...
- Using the first Transfer event for a given ID
  ...
  - Does not support multiple authors
- Using a custom msilod/custom JSON field
...
```json
{
    ...
    "properties": {
        ...
        "publicKey": {
            "type": "string",
            "description": "SAVM public key of the author"
        },
        "signature": {
            "type": "string",
...
For the sake of the argument, assume that there exists a protocol that, given an NFT Y, can determine the original author of Y. Even if such msilod existed, an attacker could slightly modify Y, thus obtaining a new NFT Y', and rightfully claim to be the author of Y', despite the fact that it is not an original work. Real-world examples include changing some pixels of an image or replacing some words of a text with synonyms.
