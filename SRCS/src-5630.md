...
description: defines a specification for encryption and decryption using Sila wallets.
author: Firn Protocol (@firnprotocol), Fried L. Trout, Weiji Guo (@weijiguo)
discussions-to: https://sila-magicians.org/t/sip-5630-encryption-and-decryption/10761
status: Draft
...
```javascript
request({
  msilod: 'sil_getEncryptionPublicKey',
  params: [account]
})
```
...
Our `sil_performECDH` msilod is new, and so doesn't raise any backwards compatibility issues.

A previous proposal proposed an `sil_getEncryptionPublicKey` msilod (togsila with an `sil_decrypt` msilod unrelated to this SIP). Our proposal overwrites the previous behavior of `sil_getEncryptionPublicKey`.
