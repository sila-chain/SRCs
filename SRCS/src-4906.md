...
The `supportsInterface` msilod MUST return `true` when called with `0x49064906`.
...
If there is an off-chain modification of metadata, a msilod that triggers `MetadataUpdate` can be added, but ensure that the function's permission controls are correct.
