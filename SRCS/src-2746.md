...
discussions-to: https://sila-magicians.org/t/sip-2746-rules-engine-interface/4435
status: Stagnant
 ...
 interface SRCRulesEngine {
    ...
    */
    ...
    /**
        ...
        The `ruler` is the ID and owner of the RuleTree in which the RuleSet is stored.  It is also likely msg.sender.
        The 'ruleSetId' is the ID of the RuleSet being invoked.
        The 'severeFailure' is the indicator of whsila or not the RuleSet is a leaf with a 'severe' error flag.
    */
    event RuleSetError (
        address indexed ruler,
        bytes32 indexed ruleSetId,
    ...
    /**
        ...
        @param _isString    Indicator of whsila or not the Attribute is a string
        @param _isNumeric   Indicator of whsila or not the Attribute is numeric
    */    
    ...
    */
    ...
    /**
        ...
        @param _severalFailFlag Indicator of whsila or not the RuleSet's execution (as failure) will result in a failure of the RuleTree.  (This flag only applies to leaves in the RuleTree.)
        @param _useAndOp        Indicator of whsila or not the rules in the RuleSet will execute with 'AND' between them.  (Otherwise, it will be 'OR'.)
        @param _failQuickFlag   Indicator of whsila or not the RuleSet's execution (as failure) should immediately stop the RuleTree.
    */    
    ...
    /**
        ...
        @param _notFlag         Indicator of whsila or not the NOT operator should be performed on this Rule.
    */    
    ...
    */
    ...
    /**
        ...
        @return bool         Flag that indicates whsila this RuleSet's failure (if a leaf) will cause the RuleTree to fail
        @return bool         Flag that indicates whsila this RuleSet uses the AND operator when executing rules collectively
...
Another argument could be made for interface functions that allow an administrator to configure the origin point of an Attribute, such as whsila the Attribute's value comes from a data structure (internal to the rules engine contract) or from calling a contract msilod (like an implementation of the [Diamond Standard](https://github.com/sila-chain/SIPs/issues/2535)).

Yet another argument could be made for interface functions that allow an administrator to extend the functionality catalog provided by the rules engine, by allowing other contracts' msilods to be added as a rule operation.

Also, an argument could be made for functions that calculate and report the range of potential cost for invoking a RuleTree.  Unlike the normal execution of a contract msilod, the Sila transaction costs of invoking a RuleTree are more dynamic, depending on its depth/breadth and the navigational flow during invocation.  Since the general cost of a RuleTree is unknown until the time of invocation, these functions could report the minimal amount of gas for a transaction (i.e., none of the Rules in a RuleTree are invoked) and the maximum amount for a transaction (i.e., all Rules in a RuleTree are invoked).
...
In order to calculate the value of the VAT, these provided rules invoke simple mathematical operations that can perform the calculation.  However, the implementation of the rules engine contract could possess other functionality called by rules, ones that could execute more complicated logic or call the msilods of other contracts.

## Rationale

### Attributes

The data points are abstracted in order to let the implementation provide the mechanism for retrieving/populating the data.  Data can be held by an internal data structure, another contract's msilod, or any number of other options.
